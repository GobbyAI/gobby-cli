...
- `crates/gcode/src/config.rs` keeps `Context` building but resolves FalkorDB/Qdrant/embedding configs via `gobby_core::config::resolve_falkordb_config` / `resolve_qdrant_config` / `resolve_embedding_config` (or by composing `gobby_core::config::CoreContext`). Any retained `FalkorConfig` / `QdrantConfig` / `EmbeddingConfig` references in `gobby-code` are thin re-exports of the gobby-core types so existing call sites keep compiling. The duplicated resolver bodies (env precedence over `config_store` over defaults, `decode_config_value`, JSON-null handling) are removed. Code-specific projection settings that are not part of `gobby-core`'s connection/auth surface — for example the optional vector dimension override consumed by §2.5's code-symbol vector lifecycle — are added as sibling consumer-owned config types in `crates/gcode/src/config.rs` (such as `CodeVectorSettings { vector_dim: Option<usize> }`), resolved through the same `ConfigSource` adapter pipeline (env → `config_store` JSON-decoded → defaults), rather than extending the re-exported `gobby-core` types. `gobby-core::config::EmbeddingConfig` remains the connection/auth surface (`api_base`, `model`, `api_key`) and is not extended for code-specific projection metadata.
...
- `crates/gcode/src/falkor.rs` retains its public facade for the external Phase 7 contract but routes connection plumbing and graph queries through `gobby_core::falkor::with_graph` / `gobby_core::falkor::GraphClient::from_config(config, graph_name)`. The `"gobby_code"` graph name remains consumer-supplied at every call site; no graph default leaks into gobby-core.
...
- 1.5.5 - `crates/gcode/src/falkor.rs` keeps its public facade but routes connection and query plumbing through `gobby_core::falkor::with_graph` / `gobby_core::falkor::GraphClient`; the `"gobby_code"` graph name remains consumer-supplied. file: `crates/gcode/src/falkor.rs`.
...
### 2.6 Add projection lifecycle orchestration commands [category: code] (depends: 1.4, 2.4, 2.5)
`kind: deliverable`
Targets: `crates/gcode/src/main.rs`, `crates/gcode/src/commands/index.rs`, `crates/gcode/src/commands/vector.rs`, `crates/gcode/src/projection/sync.rs`, `crates/gcode/src/db.rs`, `crates/gcode/src/output.rs`, `crates/gcode/tests/projection_standalone.rs`
...
- title: Add explicit standalone setup
  ...
  depends_on:
    - "1.1"
  validation_criteria: "cargo test -p gobby-code --no-default-features setup::tests::standalone_setup_is_scoped commands::setup::tests::standalone_command_is_scoped main::tests::parse_setup_standalone"
  labels:
...
- title: Add reusable code-fact indexing library API
  ...
  depends_on:
    - "1.1"
    - "1.5"
  validation_criteria: "cargo test -p gobby-code --no-default-features index::indexer::tests::library_api_is_cli_independent index::indexer::tests::library_writes_all_code_facts"
  labels:
...
- title: Wire gcode to the gobby-core foundation
  ...
  depends_on:
    - "1.1"
  validation_criteria: "cargo build -p gobby-code && cargo build -p gobby-code --no-default-features && cargo test -p gobby-code --no-default-features lib::tests::foundation_consumer_migration config::tests::adapter_env_precedence_and_json_decode config::tests::adapter_resolves_config_store_secrets"
  labels:
...
- title: Port code graph reads to Rust core
  ...
  depends_on:
    - "2.2"
  validation_criteria: "cargo test -p gobby-code --no-default-features commands::graph::tests::graph_reads_require_falkor"
  labels:
...
- title: Wrap graph core with gcode commands
  ...
  depends_on:
    ...
    - "2.2"
    - "2.3"
  validation_criteria: "cargo test -p gobby-code --no-default-features main::tests::parse_graph_commands"
  labels:
...
- title: Port code-symbol vector projection to Rust core
  ...
  depends_on:
    ...
    - "1.5"
    - "2.1"
  validation_criteria: "cargo test -p gobby-code --no-default-features vector::code_symbols::tests::embedding_request_response vector::code_symbols::tests::lifecycle_http_scoped_to_module vector::code_symbols::tests::routes_through_gobby_core_qdrant vector::code_symbols::tests::ensure_collection_resolves_vector_size_and_distance vector::code_symbols::tests::payloads_carry_provenance_metadata config::tests::vector_dim_setting_resolves_env_and_config_store && cargo test -p gobby-code --no-default-features --test vector_projection ensure_creates_missing_and_reuses_compatible incompatible_existing_collection_errors_without_migration"
  labels:
...
- title: Add projection lifecycle orchestration commands
  ...
  depends_on:
    ...
    - "2.4"
    - "2.5"
  validation_criteria: "cargo test -p gobby-code --no-default-features main::tests::parse_projection_lifecycle_commands commands::index::tests::sync_projections_json_contract commands::index::tests::sync_projections_text_contract"
  labels:
...
- title: Add gcode graph report CLI wrapper
  ...
  depends_on:
    - "2.6"
    - "3.1"
  validation_criteria: "cargo test -p gobby-code --no-default-features main::tests::parse_graph_report_global_format"
  labels:
