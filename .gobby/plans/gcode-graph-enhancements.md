...
- Standalone setup is an explicit user action, for example `gcode setup --standalone`, that creates only the minimal gcode-owned app schema needed for indexing, graph/vector sync state, and search in a selected database/schema namespace.
...
### 2.5 Port code-symbol vector projection into the Rust core [category: code] (depends: 1.1)
`kind: deliverable`
...
- Resolve embedding configuration through the standard Gobby config chain: env overrides first, then PostgreSQL `config_store`, then hardcoded defaults where a default is valid. Attached mode reaches `config_store` through Gobby bootstrap or the daemon database broker, not through a project `.env`.
...
- Ensure Qdrant collections using the existing `code_symbols_<project_id>` names. No migration or collection rename is allowed.
- Build vector payloads from indexed symbol facts: project ID, file path, symbol ID, name, kind, language, signature/docstring where present, source range, and optional existing summary text.
- Delete stale vectors by `project_id` plus `file_path` before file-level re-upsert.
- Upsert all current symbols for a file after embedding succeeds.
...
- 2.5.3 - Collection naming remains `code_symbols_<project_id>` and does not require migration. test: `crates/gcode/src/vector/code_symbols.rs::tests::collection_name_compatibility`.
...
### 2.6 Add projection lifecycle orchestration commands [category: code] (depends: 2.4, 2.5)
`kind: deliverable`
Targets: `crates/gcode/src/main.rs`, `crates/gcode/src/commands/index.rs`, `crates/gcode/src/commands/vector.rs`, `crates/gcode/src/projection/sync.rs`, `crates/gcode/src/db.rs`
...
`gcode index --sync-projections` writes PostgreSQL code facts, then synchronously syncs graph and vector projections for the affected files. It is the daemon-triggered indexing path during migration. CLI JSON output is stable enough for Python shell-out shims: each projection reports `status`, `synced_files`, `synced_symbols`, `degraded`, and typed error details when available.

Hard lifecycle commands fail non-zero when their explicitly requested backing service is unavailable or misconfigured. Search/index paths that can return useful PostgreSQL-only results may return typed degradation instead, but they must make skipped projection work visible in JSON.
...
- 2.6.3 - JSON output for lifecycle commands is stable and includes typed degradation/error fields. test: `crates/gcode/src/commands/vector.rs::tests::lifecycle_json_contract`.
...
- Gobby repo transition tests prove Python shims shell out to `gcode` and do not instantiate Python graph/vector projection code.
...
- title: Port code-symbol vector projection to Rust core
  ...
  depends_on:
  - '1.1'
  validation_criteria: "crates/gcode/src/vector/code_symbols.rs::tests::embedding_request_response"
  labels:
  - covers:unknown:2.5:2.5.1
  - covers:unknown:2.5:2.5.2
  - covers:unknown:2.5:2.5.3
  - covers:unknown:2.5:2.5.4
  - covers:unknown:2.5:2.5.5
  tdd: true
  source_section: '2.5'
  implementation_domain: backend
  assigned_agent: backend-developer
- title: Add projection lifecycle orchestration commands
  ...
  - '2.4'
  - '2.5'
  validation_criteria: "crates/gcode/src/main.rs::tests::parse_projection_lifecycle_commands"
  labels:
