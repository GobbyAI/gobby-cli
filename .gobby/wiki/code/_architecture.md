...
- [[code/modules/crates/gcode|crates/gcode]] - The `crates/gcode` subsystem implements gcode, a Postgres-backed, multi-language code intelligence CLI and library that indexes source code and exposes a contract-defined command surface for searching, grepping, graph analysis, vector/embedding lifecycle management, freshness and projection synchronization, and AI-assisted documentation generation.
...
- [[code/modules/crates/gcode/assets|crates/gcode/assets]] - Provides curated JSON reference data cataloging well-known Elixir dependency and Ruby require roots so the indexer can recognize standard library and third-party package boundaries during language-aware import resolution.
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:2]
[crates/gcode/assets/import_roots/ruby_require_roots.json:2]
...
- [[code/modules/crates/gcode/assets/import_roots|crates/gcode/assets/import_roots]] - This subsystem provides curated JSON reference data mapping well-known Elixir dependency and Ruby require names to recognized import roots, enabling the code indexer to resolve external library imports.
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:2]
[crates/gcode/assets/import_roots/ruby_require_roots.json:2]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:3]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:4]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:5]
- [[code/modules/crates/gcode/contract|crates/gcode/contract]] - The gcode contract subsystem defines the machine-readable CLI contract specification, declaring the tool's commands, global flags, positionals, value constraints, JSON output keys, and error codes for consumers and the daemon.
[crates/gcode/contract/gcode.contract.json:2]
...
- [[code/modules/crates/gcode/src|crates/gcode/src]] - The `gcode/src` subsystem implements the gcode CLI and library that indexes source code into a Postgres-backed store and projects it to graph and vector backends, exposing freshness-gated search, grep, symbol/graph navigation, code-wiki documentation generation, and deployment setup commands.
[crates/gcode/src/cli.rs:21-44]
[crates/gcode/src/cli/tests.rs:5-213]
...
- [[code/modules/crates/gcode/src/cli|crates/gcode/src/cli]] - The CLI subsystem parses and validates gcode command-line arguments—covering search, grep, graph, codewiki, setup, and projection commands along with global flags and output format resolution.
[crates/gcode/src/cli/tests.rs:5-213]
...
- [[code/modules/crates/gcode/src/commands|crates/gcode/src/commands]] - The crates/gcode/src/commands subsystem implements the gcode CLI's command layer, dispatching and executing user-facing operations—indexing, search, grep, symbol lookup, code-graph queries, vector and embeddings lifecycle management, project status, setup, and citation-grounded documentation generation—against the indexed codebase and supporting daemon services.
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
...
- [[code/modules/crates/gcode/src/commands/codewiki|crates/gcode/src/commands/codewiki]] - The codewiki subsystem generates a grounded, hierarchical documentation wiki (file, module, repo, architecture, onboarding, hotspot, ownership, and changelog pages) from a codebase's indexed symbols and call/import graph, combining structural and AI-assisted text generation with citation grounding, ownership/blame analysis, incremental snapshot-based reuse, and safe Markdown output.
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
[crates/gcode/src/commands/codewiki/build_parts/hotspots.rs:5-131]
[crates/gcode/src/commands/codewiki/build_parts/modules.rs:4-144]
- [[code/modules/crates/gcode/src/commands/codewiki/build_parts|crates/gcode/src/commands/codewiki/build_parts]] - This subsystem generates the individual CodeWiki documentation parts—architecture, changes, per-file, hotspots, module, onboarding, and index-snapshot documents—from indexed code analysis.
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
[crates/gcode/src/commands/codewiki/build_parts/hotspots.rs:5-131]
[crates/gcode/src/commands/codewiki/build_parts/modules.rs:4-144]
- [[code/modules/crates/gcode/src/commands/graph|crates/gcode/src/commands/graph]] - The graph command subsystem implements the CLI's code-graph operations—lifecycle actions (sync, clear, rebuild) and read queries (overview, callers, usages, imports, neighbors, blast radius)—including symbol resolution, typed contract errors, and text/JSON payload formatting with graceful degradation when the graph service is unavailable.
...
- [[code/modules/crates/gcode/src/commands/grep|crates/gcode/src/commands/grep]] - The grep subsystem provides pattern matching for the grep command, locating regex match spans within text while enforcing word and identifier boundary rules and reporting invalid pattern errors.
...
- [[code/modules/crates/gcode/src/commands/snapshots|crates/gcode/src/commands/snapshots]] - Stores snapshot fixtures that capture expected outputs for the index command's projection and indexing tests. 
- [[code/modules/crates/gcode/src/config|crates/gcode/src/config]] - The config subsystem resolves runtime context—project identity, daemon URL, and service configurations (FalkorDB, Qdrant, embeddings, code-vector/indexing settings)—from layered, prioritized configuration sources spanning environment variables, config stores, and standalone files.
...
- [[code/modules/crates/gcode/src/db|crates/gcode/src/db]] - The db subsystem manages PostgreSQL connectivity for gcode, resolving database URLs from environment, bootstrap files, gcore config, and daemon broker sources, and provides read/sync queries over indexed project, file, symbol, import, and call graph data.
...
- [[code/modules/crates/gcode/src/dispatch|crates/gcode/src/dispatch]] - The dispatch subsystem maps parsed command requests to the minimal set of backing services required for execution, resolving service configuration only when needed.
...
- [[code/modules/crates/gcode/src/graph|crates/gcode/src/graph]] - The graph subsystem builds and queries the code-index property graph—syncing files, symbols, imports, and call relationships into a Neo4j/Cypher-style store via safe typed queries, serving read APIs (callers, usages, blast radius, neighbors, overviews), and generating project-level graph analysis reports.
...
- [[code/modules/crates/gcode/src/graph/code_graph|crates/gcode/src/graph/code_graph]] - The code_graph subsystem manages persistence and querying of the code knowledge graph, providing connection handling, lifecycle orchestration, payload models, and read/write operations for syncing files and traversing symbol relationships like callers, usages, imports, and blast radius.
[crates/gcode/src/graph/code_graph/connection.rs:7-12]
[crates/gcode/src/graph/code_graph/lifecycle.rs:18-21]
[crates/gcode/src/graph/code_graph/payload.rs:10-19]
[crates/gcode/src/graph/code_graph/read.rs:45-90]
[crates/gcode/src/graph/code_graph/tests.rs:7-21]
- [[code/modules/crates/gcode/src/graph/report|crates/gcode/src/graph/report]] - Generates project graph reports by loading graph snapshots, querying and summarizing structural analytics such as hotspots, target frequencies, and bridge-edge hypotheses, and rendering them into markdown for the repository architecture overview.
...
- [[code/modules/crates/gcode/src/index|crates/gcode/src/index]] - The index subsystem discovers, classifies, parses, and hashes source files across many languages to extract symbols, imports, call sites, and content chunks—resolving local versus external references and persisting the resulting code facts incrementally to storage.
...
- [[code/modules/crates/gcode/src/index/import_resolution|crates/gcode/src/index/import_resolution]] - The import_resolution subsystem builds per-language import-resolution contexts from project manifests and parses import statements across multiple languages to bind imported symbols and classify callees as internal or external dependencies.
[crates/gcode/src/index/import_resolution/context.rs:19-37]
[crates/gcode/src/index/import_resolution/helpers.rs:1-3]
...
- [[code/modules/crates/gcode/src/index/import_resolution/parser|crates/gcode/src/index/import_resolution/parser]] - Parses language-specific import statements across Go, Rust, Java, C#, PHP, Kotlin, Python, JavaScript, Swift, Ruby, Dart, and Elixir to extract and register import bindings for cross-file symbol resolution.
[crates/gcode/src/index/import_resolution/parser/go_rust.rs:12-40]
[crates/gcode/src/index/import_resolution/parser/java_csharp.rs:8-60]
[crates/gcode/src/index/import_resolution/parser/mod.rs:29-54]
...
- [[code/modules/crates/gcode/src/index/indexer|crates/gcode/src/index/indexer]] - The indexer subsystem orchestrates parsing and indexing of discovered, explicit, and overlay files—probing freshness, routing supported versus content-only paths, and persisting code facts and projections through a pluggable sink while reconciling and cleaning up stale or deleted entries.
...
- [[code/modules/crates/gcode/src/index/parser|crates/gcode/src/index/parser]] - Call-site extraction during code indexing, detecting function and method invocations through both AST traversal and language-aware textual scanning to resolve callees within a file.
...
- [[code/modules/crates/gcode/src/index/parser/calls|crates/gcode/src/index/parser/calls]] - Extracts function and method call references from source code—via AST traversal, language-specific textual scanning (notably Dart), and identifier tokenization—then resolves callees to same-file symbols while filtering out keywords and shadowed local bindings.
[crates/gcode/src/index/parser/calls/ast.rs:17-96]
[crates/gcode/src/index/parser/calls/dart_textual.rs:8-55]
[crates/gcode/src/index/parser/calls/resolution.rs:6-10]
[crates/gcode/src/index/parser/calls/shadowing.rs:6-23]
[crates/gcode/src/index/parser/calls/text.rs:4-20]
- [[code/modules/crates/gcode/src/projection|crates/gcode/src/projection]] - Synchronizes indexed code facts to downstream graph and vector projection targets, tracking per-file sync state and producing status reports that capture success, degradation, and error conditions.
...
- [[code/modules/crates/gcode/src/search|crates/gcode/src/search]] - The search subsystem provides Postgres-backed code search—full-text symbol and content queries with visibility filtering, path/glob and BM25 scoring, graph-based result expansion and boosting, and reciprocal-rank-fusion merging—to rank and resolve relevant code symbols and snippets.
...
- [[code/modules/crates/gcode/src/search/fts|crates/gcode/src/search/fts]] - Full-text search execution over symbols, content, and graph data, building SQL queries with path/visibility filtering, BM25 ranking, snippet generation, result counting, and symbol resolution.
[crates/gcode/src/search/fts/common.rs:16]
[crates/gcode/src/search/fts/content.rs:13-21]
[crates/gcode/src/search/fts/counts.rs:10-66]
[crates/gcode/src/search/fts/graph.rs:16-50]
[crates/gcode/src/search/fts/symbols.rs:15-18]
- [[code/modules/crates/gcode/src/setup|crates/gcode/src/setup]] - Provisions and validates a standalone Postgres code-index schema by defining table/index contracts, generating and executing DDL with safe quoted identifiers, redacting connection secrets, and enforcing compatibility and destructive-operation guards during setup.
...
- [[code/modules/crates/gcode/src/vector|crates/gcode/src/vector]] - The `vector` subsystem provides semantic vector indexing, lifecycle management, and similarity search for code symbols, embedding symbol text via configurable backends and persisting/querying it through Qdrant collections.
...
- [[code/modules/crates/gcode/src/vector/code_symbols|crates/gcode/src/vector/code_symbols]] - The code_symbols subsystem manages the embedding, Qdrant collection lifecycle, and semantic search of source-code symbols, generating vector embeddings from symbol text and synchronizing project/file symbol vectors into per-project Qdrant collections to support semantic code search.
[crates/gcode/src/vector/code_symbols/embedding.rs:21-23]
[crates/gcode/src/vector/code_symbols/lifecycle.rs:29-37]
[crates/gcode/src/vector/code_symbols/qdrant.rs:17-23]
[crates/gcode/src/vector/code_symbols/repository.rs:6-18]
[crates/gcode/src/vector/code_symbols/search.rs:8-14]
- [[code/modules/crates/gcore|crates/gcore]] - The gcore subsystem provides Gobby's shared infrastructure foundation—resolving layered configuration and secrets, provisioning and health-checking backing services (PostgreSQL, FalkorDB, Qdrant) and AI capability routing, and offering core clients for graph, vector, and hybrid search plus indexing, schema validation, and hub bootstrapping.
...
- [[code/modules/crates/gcore/assets|crates/gcore/assets]] - The gcore assets subsystem provides Docker Compose service definitions and a custom PostgreSQL image build that provision the crate's backing infrastructure—FalkorDB, Qdrant, and a search/audit-enabled PostgreSQL—for local development and deployment.
[crates/gcore/assets/docker-compose.services.yml:5-117]
[crates/gcore/assets/postgres-pgsearch/version.json:2]
...
- [[code/modules/crates/gcore/assets/postgres-pgsearch|crates/gcore/assets/postgres-pgsearch]] - Packages a versioned, multi-architecture PostgreSQL container image with pre-configured full-text search (pg_search) and audit logging (pgaudit) extensions plus initialization scripts and audit-export tooling for the gcore crate.
[crates/gcore/assets/postgres-pgsearch/version.json:2]
...
- [[code/modules/crates/gcore/assets/postgres-pgsearch/initdb.d|crates/gcore/assets/postgres-pgsearch/initdb.d]] - Provides PostgreSQL initialization scripts that bootstrap the pg_search and pgAudit extensions for the search-enabled database container. 
- [[code/modules/crates/gcore/assets/postgres-pgsearch/scripts|crates/gcore/assets/postgres-pgsearch/scripts]] - The subsystem provides shell tooling for exporting PostgreSQL audit data for the pgsearch asset. 
- [[code/modules/crates/gcore/src|crates/gcore/src]] - The gcore subsystem provides the foundational runtime layer for the platform—resolving layered configuration, secrets, and project context; brokering AI capabilities across local-daemon and direct-provider routes with capability probing, rate limiting, and degradation handling; and managing storage and analytics backends (FalkorDB graph, Qdrant vectors, Postgres) alongside indexing, hybrid search, standalone provisioning, and schema validation.
[crates/gcore/src/ai/daemon.rs:19-24]
[crates/gcore/src/ai/mod.rs:30-34]
[crates/gcore/src/ai/probe.rs:20-23]
[crates/gcore/src/ai/text.rs:9-15]
[crates/gcore/src/ai/transcription.rs:11-14]
- [[code/modules/crates/gcore/src/ai|crates/gcore/src/ai]] - The `ai` subsystem provides AI inference capabilities—text generation, embeddings, vision/image description, and audio transcription—by routing requests through either a local daemon or direct provider endpoints, with capability probing, profile-based model binding, retry/backoff, and timeout handling to select and execute the effective route per capability.
[crates/gcore/src/ai/daemon.rs:19-24]
[crates/gcore/src/ai/mod.rs:30-34]
[crates/gcore/src/ai/probe.rs:20-23]
[crates/gcore/src/ai/text.rs:9-15]
[crates/gcore/src/ai/transcription.rs:11-14]
- [[code/modules/crates/gcore/src/config|crates/gcore/src/config]] - Configuration subsystem that resolves layered settings—from environment variables, config stores, and YAML—into typed configuration values for FalkorDB, Qdrant, embedding, indexing, and AI capability routing across the system.
...
- [[code/modules/crates/gcore/src/provisioning|crates/gcore/src/provisioning]] - The provisioning subsystem orchestrates local service setup by managing standalone YAML configuration, generating embedding and Docker Compose bootstrap assets, launching and health-checking backing services (Postgres, Qdrant, FalkorDB), and resolving or provisioning the shared hub database identity.
...
- [[code/modules/crates/gcore/src/qdrant|crates/gcore/src/qdrant]] - The Qdrant subsystem defines scoped collection naming with validation and governs the vector store integration contract—covering schema enforcement, batched upserts, filtered deletions, point counts, and typed HTTP error handling with degradation semantics.
...
- [[code/modules/crates/ghook|crates/ghook]] - ghook is the hook-dispatch CLI that bridges agent CLIs (Claude, Codex, Droid, Gemini) to the gobby daemon by parsing hook payloads, wrapping them in versioned, schema-validated envelopes, enriching them with terminal context, and translating daemon responses into appropriate hook actions while handling diagnostics, queuing, and failure suppression.
...
- [[code/modules/crates/ghook/schemas|crates/ghook/schemas]] - Defines the JSON Schema contracts for ghook's diagnostic output and inbox envelope payloads, establishing versioned validation structures for hook diagnostics and queued message envelopes exchanged with the gobby daemon.
[crates/ghook/schemas/diagnose-output.v2.schema.json:2]
[crates/ghook/schemas/inbox-envelope.v1.schema.json:2]
...
- [[code/modules/crates/ghook/src|crates/ghook/src]] - ghook is the CLI hook client that captures terminal and CLI-tool context, wraps hook events into envelopes, and dispatches them to the gobby daemon—classifying success/failure responses into hook actions while handling planned-shutdown suppression, diagnostics, statusline forwarding, and reliable enqueue/quarantine delivery.
[crates/ghook/src/cli_config.rs:11-18]
[crates/ghook/src/detach.rs:23-43]
[crates/ghook/src/diagnose.rs:15-32]
[crates/ghook/src/envelope.rs:24-32]
[crates/ghook/src/main.rs:38-42]
- [[code/modules/crates/gloc|crates/gloc]] - The gloc crate provides a configurable CLI tool that resolves backend, client, model, and alias settings to launch AI client binaries (Claude, Codex) against pluggable LLM backends, including local Ollama models with readiness checks.
...
- [[code/modules/crates/gloc/src|crates/gloc/src]] - The gloc CLI subsystem resolves backend, client, and model configuration from aliases and templates, ensures the selected LLM backend (e.g. Ollama) is reachable and its model is ready, then builds the environment and arguments to execute the chosen client.
[crates/gloc/src/backend.rs:7-12]
[crates/gloc/src/config.rs:13-22]
[crates/gloc/src/exec.rs:9-21]
[crates/gloc/src/main.rs:16-52]
[crates/gloc/src/backend.rs:14-23]
- [[code/modules/crates/gsqz|crates/gsqz]] - The gsqz crate provides a configurable, pipeline-driven command-output compression tool that shrinks verbose shell and prose output through pattern-matched filtering, grouping, deduplication, replacement, and truncation steps while preserving meaningful content.
...
- [[code/modules/crates/gsqz/src|crates/gsqz/src]] - The gsqz crate compresses shell command output by matching commands against configurable pipelines of text-transformation primitives (filtering, grouping, deduplication, truncation, replacement, and prose compression), running as both a CLI and a config-fetching daemon client.
[crates/gsqz/src/command_split.rs:5-85]
[crates/gsqz/src/compressor.rs:7-12]
[crates/gsqz/src/config.rs:26-35]
[crates/gsqz/src/daemon.rs:11-23]
[crates/gsqz/src/main.rs:25-48]
- [[code/modules/crates/gsqz/src/primitives|crates/gsqz/src/primitives]] - The primitives subsystem provides the core text-transformation operations—deduplication, regex filtering and replacement, line grouping, pattern matching, prose compression, and truncation—that gsqz composes to reduce and reshape command output.
...
- [[code/modules/crates/gwiki|crates/gwiki]] - The gwiki crate implements a scope-aware (global/project/topic) wiki and knowledge-base CLI that ingests heterogeneous sources into a raw-first store and provides indexing, multi-backend search, AI-assisted ask/research/compile, provenance and graph analytics, and trust/health/audit reporting—all governed by a machine-readable command contract.
...
- [[code/modules/crates/gwiki/assets/skills|crates/gwiki/assets/skills]] - This subsystem provides the markdown skill definitions—audit, compile, query, and research—that specify the gwiki agent's knowledge-base workflows. 
- [[code/modules/crates/gwiki/contract|crates/gwiki/contract]] - The gwiki contract subsystem defines the machine-readable CLI/API specification—commands, flags, positionals, output shapes, dependencies, and error codes—that establishes the stable interface contract for the gwiki tool.
[crates/gwiki/contract/gwiki.contract.json:2]
...
- [[code/modules/crates/gwiki/src|crates/gwiki/src]] - The gwiki crate implements a scope-aware personal knowledge-base and wiki system that ingests diverse sources (URLs, files, audio, video, images, PDFs, documents, git repositories) into a raw-first vault, indexes them across PostgreSQL keyword, Qdrant semantic, and FalkorDB graph backends for hybrid search, and provides CLI commands for AI-assisted research, synthesis, compilation, citation/provenance auditing, link analysis, and health/trust reporting.
[crates/gwiki/src/ai/chunk.rs:20-26]
[crates/gwiki/src/ai/clients.rs:20-23]
[crates/gwiki/src/ai/translate.rs:6-29]
[crates/gwiki/src/api.rs:11-121]
[crates/gwiki/src/audit.rs:33-35]
- [[code/modules/crates/gwiki/src/ai|crates/gwiki/src/ai]] - The gwiki AI subsystem orchestrates audio transcription and segment translation by chunking media, routing requests through production and vision AI clients, and merging deduplicated, language-normalized results back into the wiki.
[crates/gwiki/src/ai/chunk.rs:20-26]
[crates/gwiki/src/ai/clients.rs:20-23]
[crates/gwiki/src/ai/translate.rs:6-29]
[crates/gwiki/src/ai/chunk.rs:29-31]
[crates/gwiki/src/ai/chunk.rs:34-43]
- [[code/modules/crates/gwiki/src/commands|crates/gwiki/src/commands]] - The gwiki commands subsystem implements the CLI command handlers that orchestrate wiki operations—indexing, searching, reading, asking, source ingestion/refresh/removal, citation-quality and review reporting, trust/health/status auditing, and setup—by coordinating retrieval, graph, embedding, and storage backends while rendering results and reporting degraded-service conditions.
...
- [[code/modules/crates/gwiki/src/commands/refresh|crates/gwiki/src/commands/refresh]] - The refresh command subsystem re-fetches and re-indexes wiki sources, selecting changed or explicitly targeted URL and local-file sources, replaying their content, updating manifests and raw vault assets, and rendering refresh status results.
...
- [[code/modules/crates/gwiki/src/compile|crates/gwiki/src/compile]] - The compile subsystem collects accepted note sources, renders them into scoped Obsidian-markdown wiki pages with provenance, and safely writes target pages while updating the wiki index under file locks.
...
- [[code/modules/crates/gwiki/src/graph|crates/gwiki/src/graph]] - The gwiki graph subsystem builds and maintains the in-memory wiki knowledge graph from document facts, providing backlink, link-suggestion, and related-path queries plus context-pack assembly, analytics/centrality computation, and graph export rendering (statements, JSON, Mermaid) with scoped ID generation and degradation warnings.
...
- [[code/modules/crates/gwiki/src/ingest|crates/gwiki/src/ingest]] - The gwiki ingest subsystem normalizes diverse content sources—local files, stdin, URLs, Wayback captures, Git repositories, MediaWiki pages, audio, images, video, PDFs, and Office documents—into raw-first stored assets with derived Markdown, provenance metadata, and search indexing.
...
- [[code/modules/crates/gwiki/src/ingest/document|crates/gwiki/src/ingest/document]] - The document ingest subsystem extracts text from HTML and Office (DOCX, PPTX, XLSX) sources, normalizes it into derived Markdown snapshots, and atomically persists those results with bounded resource limits and degradation tracking for the gwiki ingestion pipeline.
[crates/gwiki/src/ingest/document/html.rs:8-39]
[crates/gwiki/src/ingest/document/mod.rs:21-27]
[crates/gwiki/src/ingest/document/office.rs:39-52]
[crates/gwiki/src/ingest/document/render.rs:11-33]
[crates/gwiki/src/ingest/document/tests.rs:9-18]
- [[code/modules/crates/gwiki/src/ingest/pdf|crates/gwiki/src/ingest/pdf]] - Ingests PDF documents by rendering pages, extracting text-layer and vision-based OCR content, merging and sanitizing them into wiki markdown with managed asset storage and rollback support.
...
- [[code/modules/crates/gwiki/src/ingest/video|crates/gwiki/src/ingest/video]] - Ingests video files by extracting metadata, sampling and persisting frame images, transcribing audio, and rendering markdown with degradation handling and provenance tracking.
...
- [[code/modules/crates/gwiki/src/research|crates/gwiki/src/research]] - The research subsystem orchestrates AI-driven enrichment loops that resolve scope, execute investigative commands (ask, search, read, ingest), and atomically materialize deduplicated, citation-backed accepted research notes with checkpointed, audited outcome tracking.
...
- [[code/modules/crates/gwiki/src/research_loop|crates/gwiki/src/research_loop]] - The research_loop subsystem drives an iterative, model-guided research workflow that plans and executes retrieval actions (search, ask, read, ingest) over wiki and code sources, validating source references and citations before recording observations and writing verified notes while tracking degradations and budget limits.
...
- [[code/modules/crates/gwiki/src/search|crates/gwiki/src/search]] - The gwiki search subsystem provides scope-aware wiki search by combining BM25 keyword, semantic vector, and graph-link-boost backends—each with pluggable production and in-memory implementations plus graceful degradation—and fusing their ranked results via reciprocal rank fusion into a unified, provenance-annotated response.
...
- [[code/modules/crates/gwiki/src/sources|crates/gwiki/src/sources]] - The sources subsystem manages wiki source records by defining their data types and drafts, maintaining a lock-protected, deduplicated source manifest with atomic file persistence, and rendering canonicalized source entries into escaped Markdown index documents.
...
- [[code/modules/crates/gwiki/src/support|crates/gwiki/src/support]] - The gwiki support subsystem provides shared infrastructure utilities—configuration resolution, database/broker URL connectivity, index counts and options, search backends with scope and ranking, link-graph target resolution, and text, time, and environment helpers—that underpin the wiki's higher-level commands.
...
- [[code/modules/docs/contracts|docs/contracts]] - The docs/contracts subsystem defines the interface contracts and shared schemas governing the CLI tools (gcode, gwiki), research workflows, and graph data model across the repository. 
- [[code/modules/docs/guides|docs/guides]] - This subsystem provides user and developer guides, configuration references, and contract documentation for the project's tooling (gcode, gcore, ghook, gloc, gsqz, gwiki), AI integration, and release processes. 
- [[code/modules/docs/plans|docs/plans]] - The docs/plans subsystem stores the gobby project's planning documents, holding both active release plans and an archive of completed feature design and implementation specs. 
- [[code/modules/docs/plans/completed|docs/plans/completed]] - Archives completed implementation planning documents for features such as gcode skill targets, statusline phases, sandbox-tolerant hooks, and standalone schema as historical project records. 
- [[code/modules/docs/spikes|docs/spikes]] - The docs/spikes subsystem houses exploratory research notes documenting external callee resolution strategies across multiple programming languages (Bash, C/C++, Dart, Elixir, Kotlin, Lua, Ruby, and Scala). 
- [[code/modules/scripts|scripts]] - The scripts subsystem provides shell-based verification tooling for validating the repository. 
