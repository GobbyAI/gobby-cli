---
title: Architecture Overview
type: code_architecture
provenance:
- file: crates/gcode/assets/import_roots/elixir_dependency_roots.json
  ranges:
  - 2-17
- file: crates/gcode/assets/import_roots/ruby_require_roots.json
  ranges:
  - 2-11
- file: crates/gcode/build.rs
  ranges:
  - 1-8
- file: crates/gcode/contract/gcode.contract.json
  ranges:
  - 2-850
- file: crates/gcode/src/cli.rs
  ranges:
  - 21-44
  - 47-52
  - 54-63
  - 66-71
  - 74-80
  - 84-380
  - 383-445
  - 448-459
  - 462-465
  - 467-473
  - 475-477
  - 479-481
  - 483-494
  - 496-504
- file: crates/gcode/src/cli/tests.rs
  ranges:
  - 5-213
  - 216-234
  - 237-252
  - 255-270
  - 273-288
  - 291-298
  - 301-312
  - 315-348
  - 351-359
  - 362-372
  - 375-394
  - 397-415
  - 418-440
  - 443-461
  - 464-478
  - 481-488
  - 491-503
  - 506-511
  - 514-528
  - 531-559
  - 562-614
  - 617-636
  - 639-646
  - 649-658
  - 661-693
  - 696-726
  - 729-784
  - 787-796
  - 799-808
  - 811-821
  - 824-835
  - 838-850
  - 853-876
  - 879-887
  - 890-899
  - 902-913
  - 916-924
- file: crates/gcode/src/commands/codewiki/build_parts/architecture.rs
  ranges:
  - 5-114
  - 116-131
  - 134-184
- file: crates/gcode/src/commands/codewiki/build_parts/changes.rs
  ranges:
  - 5-101
  - 104-113
  - 115-138
  - 140-156
  - 158-163
- file: crates/gcode/src/commands/codewiki/build_parts/file.rs
  ranges:
  - 12-15
  - 18-143
- file: crates/gcode/src/commands/codewiki/build_parts/hotspots.rs
  ranges:
  - 5-131
  - 133-157
- file: crates/gcode/src/commands/codewiki/build_parts/modules.rs
  ranges:
  - 4-136
  - 138-149
  - 151-153
  - 155-165
- file: crates/gcode/src/commands/codewiki/build_parts/onboarding.rs
  ranges:
  - 7-52
  - 54-109
  - 111-200
  - 202-208
  - 210-212
  - 214-219
  - 225-246
  - 249-255
  - 258-268
- file: crates/gcode/src/commands/codewiki/build_parts/snapshot.rs
  ranges:
  - 6-84
  - 86-99
  - 101-134
- file: crates/gcode/src/commands/codewiki/cluster.rs
  ranges:
  - 3-54
  - 56-80
  - 89-130
  - 132-156
  - 158-168
  - 170-178
  - 180-196
  - 198-206
  - 208-226
  - 228-233
- file: crates/gcode/src/commands/codewiki/graph.rs
  ranges:
  - 4-109
  - 113-142
  - 148-163
  - 165-180
- file: crates/gcode/src/commands/codewiki/io.rs
  ranges:
  - 3-9
  - 11-28
  - 30-43
  - 46-48
  - 50-77
  - 83-93
  - 97-103
  - 105-127
  - 131-179
  - 181-192
  - 196-224
  - 227-231
  - 233-237
  - 239-247
  - 249-267
  - 269-289
  - 291-309
  - 311-314
  - 316-323
  - 325-328
  - 330-351
  - 353-388
  - 391-421
  - 424-431
  - 433-443
- file: crates/gcode/src/commands/codewiki/mod.rs
  ranges:
  - 92-97
  - 100-104
  - 106-128
  - 131-134
  - 137-140
  - 142-163
  - 166-170
  - 173-185
  - 188-194
  - 197-212
  - 215-220
  - 223-227
  - 230-235
  - 238-242
  - 245-250
  - 253-259
  - 262-268
  - 271-278
  - 281-285
  - 288-292
  - 295-299
  - 302-314
  - 317-324
  - 327-343
  - 348-355
  - 357-366
  - 369-376
  - 379-382
  - 385-391
  - '393'
  - 405-409
  - 414-422
  - 424-440
  - 443-449
  - 451-471
  - 473-598
  - 600-605
  - 607-614
  - 616-624
  - 626-648
  - 651-670
  - 673-680
  - 684-708
  - 710-886
- file: crates/gcode/src/commands/codewiki/ownership.rs
  ranges:
  - 17-20
  - 22-29
  - 32-35
  - 38-41
  - 44-50
  - 53-57
  - 60-62
  - 65-68
  - 71-76
  - 79-82
  - 84-129
  - 131-141
  - 143-161
  - 163-182
  - 184-219
  - 221-285
  - 287-289
  - 291-302
  - 304-308
  - 310-331
  - 333-363
  - 365-370
  - 372-425
  - 427-434
  - 436-445
  - 447-461
  - 463-487
  - 489-521
  - 523-525
  - 527-553
  - 555-567
  - 569-579
  - 581-625
  - 627-633
  - 635-657
  - 672-699
  - 702-726
  - 729-746
  - 749-770
  - 773-796
  - 799-818
  - 821-856
  - 859-883
  - 885-890
  - 892-911
  - 913-922
  - 924-940
  - 942-950
- file: crates/gcode/src/commands/codewiki/paths.rs
  ranges:
  - 3-14
  - 16-28
  - 30-32
  - 34-41
  - 43-98
  - 103-111
  - 113-119
  - 121-129
  - 131-133
  - 135-137
  - 139-147
  - 149-151
  - 153-155
  - 157-159
  - 161-163
  - 165-167
- file: crates/gcode/src/commands/codewiki/progress.rs
  ranges:
  - 2-7
  - 10-12
  - 14-55
- file: crates/gcode/src/commands/codewiki/prompts.rs
  ranges:
  - 11-33
  - 35-56
  - 58-72
  - 74-104
  - 106-120
  - 122-133
  - 135-174
  - 185-203
  - 206-214
  - 217-220
  - 226-235
  - 238-258
  - 261-268
  - 271-278
- file: crates/gcode/src/commands/codewiki/render.rs
  ranges:
  - 5-35
  - 37-71
  - 73-87
  - 89-112
  - 114-121
  - 123-211
  - 213-242
  - 244-294
  - 296-309
  - 311-321
  - 323-338
  - 340-410
  - 412-446
  - 448-474
  - 476-512
  - 514-557
  - 559-561
  - 563-622
  - 625-631
  - 633-697
  - 699-742
- file: crates/gcode/src/commands/codewiki/reuse.rs
  ranges:
  - 11-19
  - 21-101
  - 105-107
- file: crates/gcode/src/commands/codewiki/text.rs
  ranges:
  - 15-27
  - 30-34
  - 36-84
  - 89-103
  - 105-113
  - 115-131
  - 138-142
  - 144-161
  - 163-177
  - 186-196
  - 198-201
  - 203-218
  - 220-229
  - 231-243
  - 245-251
  - 253-255
  - 257-266
  - 277-298
  - 300-306
  - 308-328
  - 330-342
  - 344-353
  - 355-361
  - 366-379
  - 381-397
  - 399-425
  - 427-444
  - 446-459
  - 462-464
  - 468-475
  - 477-484
  - 486-533
  - 535-559
  - 565-571
  - 574-593
  - 596-616
  - 619-631
  - 634-647
  - 650-663
  - 666-681
  - 684-691
  - 694-726
  - 729-732
  - 734-740
  - 743-756
  - 759-768
  - 771-783
- file: crates/gcode/src/commands/embeddings_doctor.rs
  ranges:
  - 19-22
  - 24-32
  - 34-38
  - '40'
  - 43-55
  - 58-63
  - 66-70
  - 73-77
  - 79-95
  - 97-99
  - 101-165
  - 167-176
  - 178-195
  - 197-223
  - 225-239
  - 241-276
  - 283-295
  - 298-362
- file: crates/gcode/src/commands/graph/lifecycle.rs
  ranges:
  - 11-13
  - 15-53
  - 55-64
  - '66'
  - 68-75
  - 77-83
  - '85'
  - 87-98
  - 100-114
  - 116-128
  - 130-136
  - 138-145
  - 147-177
  - 179-200
  - 202-280
  - 282-289
  - 291-298
  - 300-348
- file: crates/gcode/src/commands/graph/payload.rs
  ranges:
  - 6-37
  - 39-44
  - 46-48
  - 50-59
  - 61-64
  - 66-69
  - 71-79
  - 81-96
- file: crates/gcode/src/commands/graph/reads.rs
  ranges:
  - 14-20
  - 22-30
  - 32-38
  - 40-48
  - 50-73
  - 75-90
  - 92-118
  - 120-133
  - 137-158
  - 160-174
  - 176-209
  - 211-262
  - 264-316
  - 318-353
  - 355-402
  - 419-421
  - 423-440
  - 442-449
  - 451-454
  - 456-464
  - 466-479
  - 481-484
  - 486-500
  - 502-511
  - 513-524
  - 526-545
  - 552-580
  - 584-610
  - 614-650
- file: crates/gcode/src/commands/graph/tests.rs
  ranges:
  - 16-30
  - 33-39
  - 42-50
  - 53-89
  - 92-106
  - 109-111
  - 113-132
  - 135-158
  - 161-170
  - 173-189
  - 192-204
  - 207-219
  - 222-235
  - 238-253
  - 256-272
  - 275-292
  - 295-312
  - 315-373
- file: crates/gcode/src/commands/grep.rs
  ranges:
  - 21-33
  - 36-40
  - 43-46
  - 49-52
  - 55-58
  - 61-68
  - 71-84
  - 87-92
  - 94-125
  - 127-234
  - 236-254
  - 256-276
  - 279-285
  - 287-352
  - 354-375
  - 377-407
  - 409-414
  - 416-439
  - 441-456
  - 458-467
  - 469-472
  - 474-497
  - 499-515
  - 517-533
  - 535-582
  - 584-597
  - 603-609
  - 611-625
  - 628-633
  - 636-647
  - 650-664
  - 667-674
  - 677-685
  - 688-703
  - 706-738
  - 741-759
  - 762-776
  - 779-799
  - 802-817
  - 820-837
  - 840-868
  - 871-879
- file: crates/gcode/src/commands/grep/grep_matcher.rs
  ranges:
  - 6-9
  - 11-44
  - 46-65
  - 67-75
  - 78-80
  - 86-92
  - 95-105
  - 108-116
  - 119-126
  - 129-136
  - 139-146
  - 149-156
  - 159-163
- file: crates/gcode/src/commands/index.rs
  ranges:
  - 10-60
  - 62-92
  - 96-104
  - 107-117
  - 119-132
  - 134-138
  - 140-195
  - 197-216
  - 218-240
  - 252-257
  - 260-262
  - 264-272
  - 274-294
  - 297-301
  - 304-309
  - 312-338
  - 341-364
- file: crates/gcode/src/commands/init.rs
  ranges:
  - 11-148
- file: crates/gcode/src/commands/scope.rs
  ranges:
  - 9-12
  - 14-27
  - 29-45
  - 47-60
  - 62-69
  - 71-109
  - 111-133
  - 135-146
  - 153-167
  - 170-182
  - 185-190
  - 193-208
- file: crates/gcode/src/commands/search.rs
  ranges:
  - 13-21
  - 25-200
  - 202-292
  - 294-299
  - 301-405
  - 407-485
  - 488-511
  - 513-593
  - 595-605
  - 607-613
  - 615-617
  - 619-631
  - 633-643
  - 645-647
  - 649-654
  - 656-659
  - 661-663
  - 665-667
  - 669-679
  - 681-685
  - 687-698
  - 700-702
  - 704-712
  - 714-716
  - 718-725
  - 727-733
  - 735-750
  - 752-754
  - 756-767
  - 769-778
  - 784-805
  - 808-819
  - 822-836
  - 839-848
  - 851-860
  - 863-874
  - 877-879
  - 882-887
- file: crates/gcode/src/commands/setup.rs
  ranges:
  - 22-94
  - 96-99
  - 101-117
  - 119-165
  - 167-186
  - 188-201
  - 203-219
  - 221-283
  - 285-296
  - 298-301
  - 303-351
  - 353-372
  - 374-390
  - 398-460
  - 463-511
  - 514-529
  - 532-541
  - 548-586
- file: crates/gcode/src/commands/status.rs
  ranges:
  - 18-42
  - 45-58
  - 60-134
  - 136-158
  - 160-185
  - 187-197
  - 200-227
  - 229-245
  - 248-256
  - 259-268
  - 271-293
  - 296-310
  - 313-316
  - 318-372
  - 375-415
  - 417-457
  - 463-473
  - 475-489
  - 492-510
- file: crates/gcode/src/commands/symbol_at.rs
  ranges:
  - 16-20
  - 23-26
  - 30-33
  - 36-47
  - 50-55
  - 57-64
  - 66-122
  - 124-171
  - 173-183
  - 185-193
  - 195-197
  - 202-218
  - 220-233
  - 235-241
  - 243-268
  - 270-275
  - 277-282
  - 284-292
  - 294-311
  - 313-323
  - 325-327
  - 329-331
  - 333-339
  - 341-349
  - 351-365
  - 367-372
  - 374-383
  - 385-410
  - 412-422
  - 429-456
  - 458-463
  - 466-476
  - 479-485
  - 488-509
  - 512-520
  - 523-528
  - 531-549
  - 552-569
  - 572-590
  - 593-616
  - 619-640
- file: crates/gcode/src/commands/symbols.rs
  ranges:
  - 21-78
  - 80-103
  - 105-126
  - 128-142
  - 144-167
  - 169-183
  - 185-200
  - 202-229
  - 231-239
  - 241-256
  - 258-302
  - 304-341
  - 343-356
  - 358-382
  - 390-417
  - 423-444
  - 447-453
  - 456-478
  - 481-490
  - 493-511
  - 514-516
  - 519-531
  - 534-557
  - 560-568
- file: crates/gcode/src/commands/vector.rs
  ranges:
  - 12-18
  - 20-24
  - 26-41
  - 43-62
  - 64-71
  - 73-83
  - 85-95
  - 98-114
  - 116-136
  - 145-159
  - 161-166
  - 168-184
  - 187-207
  - 210-268
- file: crates/gcode/src/config/context.rs
  ranges:
  - 26-31
  - '34'
  - '37'
  - 51-53
  - '55'
  - 58-63
  - 65-110
  - 112-116
  - 119-122
  - 124-134
  - '136'
  - 138-146
  - 150-173
  - 176-185
  - 188-191
  - 194-201
  - 204-211
  - 213-317
  - 319-372
  - 374-428
  - 430-438
  - 440-448
  - 450-458
  - 460-491
  - 493-500
  - 505-529
  - 531-533
  - 541-544
  - 546-582
  - 591-593
  - 595-603
- file: crates/gcode/src/config/services.rs
  ranges:
  - 20-22
  - 24-27
  - 29-39
  - 41-48
  - 51-57
  - 59-61
  - 64-67
  - 70-81
  - 83-85
  - 89-93
  - 95-99
  - 102-104
  - 108-125
  - 127-129
  - 132-135
  - 138-143
  - 150-162
  - 164-166
  - 169-178
  - 181-196
  - 198-222
  - '224'
  - 226-241
  - 244-247
  - 255-257
  - 259-261
  - 270-276
  - 278-280
  - 284-287
  - 295-301
  - 303-305
  - 309-322
  - 325-338
  - 341-354
  - 357-370
  - 373-384
  - 389-399
  - 401-416
  - 421-431
  - 433-442
  - 444-452
  - 454-469
  - 471-494
  - 501-511
  - 513-533
  - 535-545
  - 547-557
  - 559-568
  - 570-576
  - 578-587
  - 589-603
  - 605-611
  - 613-624
  - 626-635
- file: crates/gcode/src/config/tests.rs
  ranges:
  - 14-22
  - 24-38
  - 40-70
  - 80-90
  - 92-96
  - 100-140
  - 143-148
  - 152-166
  - 170-191
  - 195-229
  - 232-242
  - 246-268
  - 272-295
  - 299-313
  - 317-333
  - 336-348
  - 351-367
  - 370-389
  - 392-426
  - 429-449
  - 452-466
  - 469-500
  - 503-515
  - 518-525
- file: crates/gcode/src/contract.rs
  ranges:
  - 5-254
  - 256-258
  - 260-263
  - 265-272
  - 274-286
  - 288-294
  - 296-314
  - 316-339
  - 341-357
  - 359-369
  - 371-373
  - 375-387
- file: crates/gcode/src/db/mod.rs
  ranges:
  - 16-20
  - 27-31
  - 33-35
- file: crates/gcode/src/db/queries.rs
  ranges:
  - 8-13
  - 15-26
  - 28-38
  - 40-55
  - 57-69
  - 71-83
  - 85-97
  - 99-109
  - 111-123
  - 125-135
  - 141-156
  - 158-168
  - 170-190
  - 192-205
  - 207-236
  - 238-240
  - 242-249
  - 251-270
  - 272-281
  - 288-292
  - 296-298
- file: crates/gcode/src/db/resolution.rs
  ranges:
  - 16-18
  - 21-24
  - 27-29
  - 31-33
  - 39-48
  - 51-64
  - 67-81
  - 83-138
  - 140-156
  - 158-166
  - 168-175
  - 177-186
  - 188-211
  - 213-226
  - 228-235
  - 237-244
  - 246-255
  - 257-280
  - 282-284
  - 286-300
  - 302-323
  - 325-353
  - 362-367
  - 370-378
  - 381-388
  - 391-399
  - 402-417
  - 420-432
  - 435-452
  - 455-472
  - 475-500
  - 503-511
  - 514-521
  - 524-529
  - 532-537
  - 540-552
  - 555-572
  - 575-583
  - 586-597
  - 600-604
  - 607-613
  - 616-622
  - 625-633
  - 636-648
  - 651-665
  - 668-682
  - 685-696
  - 699-711
  - 714-722
  - 725-733
  - 736-744
  - 746-754
  - 756-761
  - 763-765
  - 767-794
- file: crates/gcode/src/dispatch.rs
  ranges:
  - '8'
  - 10-22
  - 24-28
  - 30-37
  - 39-47
  - 49-61
  - 63-65
  - 67-72
  - 74-78
  - 80-123
  - 125-206
  - 208-234
  - 236-556
- file: crates/gcode/src/dispatch/tests.rs
  ranges:
  - 5-9
  - 12-14
  - 17-22
  - 25-27
  - 30-70
  - 73-87
  - 90-107
- file: crates/gcode/src/freshness.rs
  ranges:
  - 13-16
  - 19-22
  - 24-83
  - 93-121
  - 123-144
  - 146-160
  - 162-182
  - '184'
  - 186-193
  - 195-200
  - 208-222
  - 224-226
  - 228-249
  - 251-274
  - 276-283
  - 285-293
  - 295-300
  - 302-315
  - 322-330
  - 334-343
  - 347-388
  - 392-428
- file: crates/gcode/src/git.rs
  ranges:
  - 5-9
  - 12-17
  - 19-51
  - 53-63
  - 65-77
  - 79-87
  - 93-101
  - 103-118
  - 121-132
  - 135-158
  - 161-181
- file: crates/gcode/src/graph/code_graph/connection.rs
  ranges:
  - 7-12
  - 14-40
  - 42-68
- file: crates/gcode/src/graph/code_graph/lifecycle.rs
  ranges:
  - 18-21
  - 23-44
  - 47-52
  - 54-62
  - 65-68
  - 70-77
  - 79-96
  - 98-105
  - 108-113
  - 116-122
  - 125-130
  - 132-150
  - '152'
  - 154-164
  - 166-176
  - 178-191
  - 193-211
  - 213-232
  - 234-248
  - 250-286
- file: crates/gcode/src/graph/code_graph/payload.rs
  ranges:
  - 10-19
  - 21-82
  - 84-88
  - 90-109
  - 111-113
  - 116-135
  - 137-200
  - 203-214
  - 216-243
  - 246-262
  - 264-290
  - 292-297
  - 299-316
  - 318-322
  - 324-328
  - 330-339
- file: crates/gcode/src/graph/code_graph/read.rs
  ranges:
  - 45-93
  - 95-97
  - 99-111
  - 113-128
  - 130-149
  - 151-170
  - 172-188
  - 190-206
  - 208-227
  - 229-246
  - 248-267
  - 269-286
  - 288-298
  - 300-317
  - 319-338
  - 340-356
  - 358-377
  - 379-399
  - 401-412
  - 414-436
  - 438-459
  - 461-475
  - 477-501
  - 503-525
  - 527-553
  - 555-564
  - 566-645
  - 647-673
  - 675-711
  - 713-786
  - 788-798
  - 800-810
  - 812-823
  - 825-836
  - 838-851
  - 853-862
  - 864-877
  - 879-895
  - 897-910
  - 912-928
  - 930-936
  - 938-949
- file: crates/gcode/src/graph/code_graph/tests.rs
  ranges:
  - 7-21
  - 24-33
  - 36-65
  - 68-151
  - 154-159
  - 162-189
  - 192-198
  - 201-218
  - 221-237
  - 240-245
  - 248-252
  - 255-269
  - 272-282
  - 285-324
  - 327-374
  - 377-396
  - 399-409
  - 412-439
  - 442-454
- file: crates/gcode/src/graph/code_graph/write.rs
  ranges:
  - 110-113
  - 116-118
  - 120-158
  - 160-165
  - 167-177
  - 179-195
  - 197-216
  - 218-249
  - 251-266
  - 268-277
  - 279-284
  - 286-290
  - 292-297
  - 299-301
  - 304-315
  - 317-326
  - 328-336
  - 338-342
  - 344-346
  - 348-352
  - 354-377
  - 379-383
  - 385-392
  - 394-400
  - 402-406
  - 409-412
  - 415-422
  - 425-429
  - 431-438
  - 440-455
  - 457-462
  - 464-492
  - 494-507
  - 509-511
  - 513-518
  - 520-528
  - 530-566
  - 568-586
  - 588-618
  - 620-654
  - 656-660
  - 662-694
  - 696-702
  - 704-723
  - 725-743
  - 745-765
  - 767-778
  - 780-791
  - 793-804
  - 806-864
  - 866-933
  - 935-947
  - 949-957
  - 959-975
  - 977-986
  - 988-997
- file: crates/gcode/src/graph/report/generation.rs
  ranges:
  - 21-23
  - 25-59
  - 61-63
  - 65-76
  - 78-159
- file: crates/gcode/src/graph/report/loading.rs
  ranges:
  - 18-78
  - 80-95
  - 97-111
  - 113-128
  - 130-146
- file: crates/gcode/src/graph/report/queries.rs
  ranges:
  - 7-18
  - 20-22
  - 24-26
  - 28-38
  - 40-49
  - 51-85
  - 87-104
  - 106-126
  - 128-144
- file: crates/gcode/src/graph/report/render.rs
  ranges:
  - 8-18
  - 20-99
  - 101-121
  - 123-141
  - 143-150
  - 152-164
  - 166-177
  - 179-185
- file: crates/gcode/src/graph/report/rows.rs
  ranges:
  - 11-19
  - 21-31
  - 33-39
  - 41-66
  - 68-78
  - 80-106
  - 108-112
  - 119-128
  - 131-140
  - 143-154
  - 157-162
- file: crates/gcode/src/graph/report/summary.rs
  ranges:
  - 14-17
  - 19-41
  - 43-49
  - 51-91
  - 93-100
  - 102-156
  - 158-195
  - 197-231
  - 233-237
  - 239-275
  - 277-317
  - 319-329
  - 333-347
  - 349-378
  - 380-388
  - 390-395
- file: crates/gcode/src/graph/report/tests.rs
  ranges:
  - 15-65
  - 68-84
  - 87-127
  - 129-179
  - 181-196
  - 199-225
  - 228-249
  - 252-277
  - 280-317
  - 320-342
- file: crates/gcode/src/graph/report/time.rs
  ranges:
  - 3-5
- file: crates/gcode/src/graph/report/types.rs
  ranges:
  - 10-17
  - 19-50
  - 53-68
  - 71-73
  - 75-81
  - 83-89
  - 92-97
  - 100-105
  - 108-118
  - 121-125
  - 128-136
  - 139-142
  - 145-148
  - 151-155
  - 158-162
  - 164-179
  - '181'
  - 184-192
  - 195-200
  - 202-222
  - 225-229
  - 231-244
  - 247-251
  - 253-262
  - 264-268
- file: crates/gcode/src/graph/typed_query.rs
  ranges:
  - 7-10
  - 13-21
  - 24-27
  - 30-38
  - 40-71
  - 73-75
  - 77-98
  - 100-105
  - 107-109
  - 111-113
  - 115-120
  - 122-141
  - 143-145
  - 147-164
  - 166-178
  - 180-187
  - 189-201
  - '203'
  - 211-276
  - 279-284
  - 287-297
  - 300-315
  - 318-341
  - 344-350
- file: crates/gcode/src/index/api.rs
  ranges:
  - 16-23
  - 26-34
  - 36-48
  - 50-60
  - 62-84
  - 86-108
  - 110-125
  - 127-241
  - 243-269
  - 271-298
  - 300-325
  - 327-347
  - 349-383
  - 385-387
- file: crates/gcode/src/index/chunker.rs
  ranges:
  - 19-62
  - 64-72
  - 77-90
- file: crates/gcode/src/index/hasher.rs
  ranges:
  - 7-9
  - 12-14
  - 17-27
  - 35-49
  - 52-59
- file: crates/gcode/src/index/import_resolution/context.rs
  ranges:
  - 19-37
  - 39-53
  - 56-59
  - 62-67
  - 70-73
  - 76-79
  - 82-85
  - 152-162
  - 164-189
  - 191-228
  - 230-262
  - 264-271
  - 273-282
  - 284-308
  - 310-350
  - 352-361
  - 363-376
  - 378-380
  - 382-420
  - 422-462
  - 464-503
  - 505-545
  - 547-588
  - 590-609
  - 611-617
  - 619-655
  - 657-668
  - 670-691
  - 693-698
  - 700-706
- file: crates/gcode/src/index/import_resolution/helpers.rs
  ranges:
  - 1-3
  - 5-11
  - 13-17
  - 19-47
  - 49-86
  - 88-97
  - 99-105
  - 107-134
  - 136-164
  - 167-172
  - 174-182
  - '184'
  - 186-195
  - 197-212
  - 214-303
  - 305-307
  - 309-316
  - 318-330
  - 332-334
  - 336-338
  - 340-352
- file: crates/gcode/src/index/import_resolution/parser/go_rust.rs
  ranges:
  - 12-40
  - 42-77
  - 79-106
  - 108-136
  - 138-188
  - 195-206
  - 209-219
- file: crates/gcode/src/index/import_resolution/parser/java_csharp.rs
  ranges:
  - 8-60
  - 62-118
  - 120-122
  - 124-137
- file: crates/gcode/src/index/import_resolution/parser/mod.rs
  ranges:
  - 29-54
  - 56-74
  - 76-126
  - 128-203
- file: crates/gcode/src/index/import_resolution/parser/php_kotlin.rs
  ranges:
  - 7-14
  - 16-59
  - 62-66
  - 68-104
  - 106-147
  - 149-168
  - 170-183
  - 185-191
  - 193-213
- file: crates/gcode/src/index/import_resolution/parser/python_js.rs
  ranges:
  - 11-98
  - 100-194
- file: crates/gcode/src/index/import_resolution/parser/rest.rs
  ranges:
  - 10-54
  - 56-92
  - 94-121
  - 123-176
- file: crates/gcode/src/index/import_resolution/predicates.rs
  ranges:
  - 8-21
  - 23-53
  - 55-68
  - 70-77
  - 79-81
  - 83-88
  - 94-107
  - 109-179
  - 185-201
  - 203-210
  - 212-220
  - 222-231
  - 233-241
  - 243-251
  - 258-262
  - 264-276
  - 284-288
  - 290-302
  - 304-316
  - 318-328
- file: crates/gcode/src/index/indexer/file.rs
  ranges:
  - 15-91
  - 93-108
  - 111-115
  - 117-127
  - 130-177
  - 179-229
  - 231-264
- file: crates/gcode/src/index/indexer/freshness_probe.rs
  ranges:
  - 37-81
  - 89-96
  - 98-105
  - 109-111
  - 113-115
  - 118-138
  - 141-156
  - 159-176
  - 179-195
  - 198-235
  - 238-265
- file: crates/gcode/src/index/indexer/lifecycle.rs
  ranges:
  - 16-54
  - 56-69
  - 71-81
  - 84-121
  - 125-152
  - 154-181
  - 183-229
  - 232-235
  - 237-260
  - 262-294
  - 296-305
- file: crates/gcode/src/index/indexer/overlay.rs
  ranges:
  - 32-35
  - 38-44
  - 46-82
  - 84-255
  - 257-288
  - 290-299
  - 301-321
  - 323-375
  - 377-393
  - 395-400
  - 402-407
  - 409-414
  - 416-429
  - 431-447
  - 455-462
  - 466-470
  - 474-483
- file: crates/gcode/src/index/indexer/pipeline.rs
  ranges:
  - 27-30
  - 32-45
  - 47-173
  - 175-302
  - 304-308
  - 310-324
  - 326-340
- file: crates/gcode/src/index/indexer/sink.rs
  ranges:
  - 6-34
  - 36-38
  - 41-43
  - 50-52
  - 54-60
  - 62-69
  - 71-73
  - 75-77
  - 79-86
  - 88-95
  - 97-99
- file: crates/gcode/src/index/indexer/tests.rs
  ranges:
  - 24-30
  - 32-40
  - 43-62
  - 65-84
  - 87-105
  - 108-152
  - 155-164
  - 166-235
  - 238-314
  - 318-391
  - 393-402
  - 404-410
  - 412-415
  - 417-423
  - 425-456
  - 458-485
  - 487-514
  - 516-524
  - 526-536
  - 539-561
  - 564-600
  - 603-639
  - 642-672
  - 675-701
  - 704-726
  - 729-744
  - 747-796
  - 799-827
  - 830-880
  - 883-920
  - 923-949
- file: crates/gcode/src/index/indexer/types.rs
  ranges:
  - 8-17
  - 20-25
  - 29-42
  - 45-68
  - 71-76
  - 79-84
  - 86-109
  - 111-113
  - 116-124
- file: crates/gcode/src/index/indexer/util.rs
  ranges:
  - 28-66
  - 70-93
  - 95-101
  - 103-111
  - 113-142
  - 144-154
  - 156-160
  - 162-169
  - 176-186
  - 189-194
  - 197-205
  - 209-214
  - 218-223
  - 227-232
- file: crates/gcode/src/index/languages.rs
  ranges:
  - 7-12
  - 326-338
  - 341-346
  - 349-371
  - 374-385
  - 392-396
  - 399-404
  - 407-410
  - 413-419
  - 422-428
  - 430-435
  - 437-442
- file: crates/gcode/src/index/parser.rs
  ranges:
  - 29-133
  - 135-234
  - 236-261
  - 263-324
  - 326-333
  - 335-378
- file: crates/gcode/src/index/parser/calls.rs
  ranges:
  - 23-32
  - 35-42
  - 44-55
  - 57-132
- file: crates/gcode/src/index/parser/calls/ast.rs
  ranges:
  - 17-96
  - 109-140
  - 142-154
  - 157-166
  - 169-178
  - 181-196
  - 199-213
- file: crates/gcode/src/index/parser/calls/dart_textual.rs
  ranges:
  - 8-55
  - 57-77
  - 79-168
  - 170-172
  - 174-189
  - 191-216
  - 218-223
  - 226-232
  - 234-244
  - 247-252
  - 255-259
  - 261-371
  - 373-375
  - 377-379
  - 381-391
  - 393-417
  - 419-441
  - 443-492
- file: crates/gcode/src/index/parser/calls/resolution.rs
  ranges:
  - 6-10
  - 17-21
  - 23-46
  - 48-61
  - 63-65
  - 67-90
  - 92-105
  - 107-115
  - 117-155
  - 157-162
  - 164-175
  - 177-182
- file: crates/gcode/src/index/parser/calls/shadowing.rs
  ranges:
  - 6-23
  - 25-43
  - 45-84
  - 86-96
  - 98-113
  - 115-129
  - 131-153
  - 155-218
  - 220-224
  - 226-235
  - 237-260
  - 262-273
  - 283-299
  - 302-315
  - 318-327
  - 330-339
  - 342-351
  - 354-363
- file: crates/gcode/src/index/parser/calls/text.rs
  ranges:
  - 4-20
  - 22-30
  - 32-49
  - 51-53
  - 55-57
  - 59-61
  - 63-65
  - 67-151
  - 158-163
  - 166-172
- file: crates/gcode/src/index/security.rs
  ranges:
  - 26-31
  - 34-39
  - 42-54
  - 63-89
  - 91-93
  - 96-120
  - 123-127
  - 129-148
- file: crates/gcode/src/index/semantic.rs
  ranges:
  - 15-23
  - 26-29
  - 31-36
  - 39-41
  - 43-71
  - 73-91
  - 93-110
  - 112-123
  - 125-133
  - 135-141
  - 143-163
  - 165-198
  - 200-206
  - 208-215
  - 217-223
  - 226-231
  - 234-246
  - 249-270
  - 272-277
  - 279-305
  - 307-310
  - 312-314
  - 316-331
  - 333-341
  - 343-470
  - 472-480
  - 482-519
  - 521-527
  - 529-547
  - 549-571
  - 573-605
  - 607-615
  - 626-633
  - 636-648
  - 651-660
  - 663-668
  - 671-677
  - 680-695
  - 698-742
  - 745-763
  - 767-771
  - 775-779
  - 783-788
  - 792-797
  - 802-826
  - 829-864
- file: crates/gcode/src/index/walker.rs
  ranges:
  - 35-38
  - 41-43
  - 45-51
  - 55-60
  - 62-107
  - 110-134
  - 138-148
  - 151-160
  - 163-175
  - 177-195
  - 197-213
  - 215-219
  - 222-224
  - 226-273
  - 275-289
  - 291-303
  - 305-311
  - 313-316
  - 318-326
  - 328-358
  - 360-385
  - 387-395
  - 397-422
  - 424-444
  - 446-451
  - 453-463
  - 465-471
  - 473-498
  - 504-510
  - 512-524
  - 527-566
  - 569-590
  - 593-604
  - 607-617
  - 620-629
  - 632-652
  - 655-674
  - 677-698
  - 701-716
  - 719-752
  - 755-777
  - 780-791
  - 794-812
  - 815-829
  - 832-845
  - 848-861
  - 864-877
  - 880-894
  - 897-907
  - 910-917
- file: crates/gcode/src/index_lock.rs
  ranges:
  - 15-21
  - 23-30
  - 33-36
  - 38-47
  - 49-52
  - 54-92
  - 94-125
  - 127-132
  - 134-146
  - 148-154
  - 156-160
  - 162-191
  - 200-214
  - 216-225
  - 227-233
  - 236-238
  - 241-243
  - 250-268
  - 272-298
  - 302-320
- file: crates/gcode/src/lib.rs
  ranges:
  - 34-42
  - 45-75
  - 78-142
  - 145-172
  - 175-204
- file: crates/gcode/src/main.rs
  ranges:
  - 4-6
- file: crates/gcode/src/models.rs
  ranges:
  - 18-22
  - 24-33
  - 37-50
  - 52-108
  - 112-138
  - 140-217
  - 219-222
  - 224-232
  - 236-245
  - 247-252
  - 256-266
  - 268-273
  - 277-280
  - 284-288
  - 290-298
  - 302-310
  - 312-346
  - 350-359
  - 363-381
  - 385-396
  - 399-405
  - 409-416
  - 421-429
  - 433-441
  - 445-452
  - 459-507
  - 510-523
  - 525-536
  - 539-553
  - 556-590
- file: crates/gcode/src/output.rs
  ranges:
  - 5-8
  - 11-14
  - 17-20
  - 23-26
- file: crates/gcode/src/progress.rs
  ranges:
  - 9-14
  - 16-71
- file: crates/gcode/src/project.rs
  ranges:
  - 15-18
  - 21-30
  - 35-44
  - 47-70
  - 78-115
  - 118-121
  - 126-128
  - 130-138
  - 145-152
  - 155-161
  - 164-183
  - 186-201
  - 204-226
  - 229-245
  - 248-254
  - 257-265
- file: crates/gcode/src/projection/sync.rs
  ranges:
  - 11-14
  - 17-21
  - 24-29
  - 33-37
  - 40-43
  - 46-52
  - 54-97
  - 100-103
  - 105-112
  - 114-122
  - 124-151
  - 153-205
  - 207-235
  - 237-264
  - 266-270
  - 273-285
  - 287-296
  - 299-314
  - 316-326
  - 328-335
  - 337-348
  - 355-390
- file: crates/gcode/src/savings.rs
  ranges:
  - 7-12
  - 18-29
  - 36-39
  - 42-44
  - 47-49
- file: crates/gcode/src/schema.rs
  ranges:
  - 24-52
  - 54-63
  - 65-71
  - 73-88
  - 91-93
  - 99-105
  - 112-120
  - 124-129
  - 132-137
- file: crates/gcode/src/search/fts/common.rs
  ranges:
  - '16'
  - 19-22
  - 25-29
  - 32-36
  - 38-54
  - 56-59
  - 63-69
  - 71-76
  - 78-86
  - 88-123
  - 126-135
  - 138-148
  - 150-152
  - 154-175
  - 177-184
  - 186-196
  - 198-200
  - 202-233
  - 235-250
  - 252-272
  - 274-291
  - 293-341
  - 348-354
  - 357-362
- file: crates/gcode/src/search/fts/content.rs
  ranges:
  - 13-21
  - 24-81
  - 83-138
  - 140-178
  - 180-196
  - 199-202
  - 204-210
  - 212-227
  - 229-244
  - 250-253
  - 256-261
  - 264-269
- file: crates/gcode/src/search/fts/counts.rs
  ranges:
  - 10-66
  - 69-113
  - 115-146
  - 148-164
  - 166-191
  - 193-243
  - 245-259
  - 261-273
  - 275-294
  - 296-307
  - 309-333
  - 335-359
  - 366-385
- file: crates/gcode/src/search/fts/graph.rs
  ranges:
  - 16-50
  - 52-55
  - 57-62
  - 64-69
  - 71-78
  - 80-96
  - 98-103
  - 108-147
- file: crates/gcode/src/search/fts/symbols.rs
  ranges:
  - 15-18
  - 21-26
  - 28-33
  - 36-73
  - 76-112
  - 114-190
  - 192-225
  - 227-260
  - 262-337
  - 339-371
  - 374-386
  - 388-401
- file: crates/gcode/src/search/fts/tests.rs
  ranges:
  - 17-26
  - 29-34
  - 37-43
  - 46-49
  - 52-57
  - 60-72
  - 75-81
  - 84-99
  - 102-133
  - 136-142
  - 145-151
  - 154-166
  - 173-207
  - 211-239
  - 242-257
  - 260-269
  - 272-281
  - 283-302
  - 304-308
  - 310-319
  - 321-325
  - 327-336
  - 338-342
  - 344-347
  - 349-355
  - 357-360
  - 362-364
  - 366-380
  - 382-470
  - 472-480
  - 482-499
  - 501-514
  - 516-533
  - 535-554
- file: crates/gcode/src/search/graph_boost.rs
  ranges:
  - 21-47
  - 55-86
  - 88-106
  - 113-127
  - 129-153
  - 156-160
  - 163-167
  - 170-174
  - 177-223
- file: crates/gcode/src/search/rrf.rs
  ranges:
  - '7'
  - 15-20
  - 27-34
  - 37-49
  - 52-64
  - 67-75
  - 78-81
  - 84-87
  - 90-113
- file: crates/gcode/src/setup/contracts.rs
  ranges:
  - 5-8
  - 10-14
  - 191-193
  - 195-197
- file: crates/gcode/src/setup/ddl.rs
  ranges:
  - 8-10
  - 13-16
  - 18-279
  - 281-309
  - 311-319
  - 321-338
- file: crates/gcode/src/setup/identifiers.rs
  ranges:
  - 5-15
  - 17-41
- file: crates/gcode/src/setup/postgres.rs
  ranges:
  - 12-57
  - 59-77
  - 85-101
  - 103-114
  - 116-131
  - 133-145
  - 147-179
  - 181-212
  - 214-232
  - 234-256
  - 258-262
  - 264-292
  - 294-308
- file: crates/gcode/src/setup/tests.rs
  ranges:
  - 12-55
  - 58-84
  - 87-128
  - 130-155
  - 157-162
  - 165-180
  - 183-191
  - 194-224
  - 227-231
  - 234-244
  - 247-258
  - 261-274
  - 277-297
  - 300-313
  - 316-321
  - 324-329
  - 336-409
  - 412-424
  - 426-430
  - 432-438
  - 440-445
  - 449-460
  - 464-473
- file: crates/gcode/src/setup/types.rs
  ranges:
  - '5'
  - 7-23
  - 25-29
  - 31-38
  - 41-66
  - 68-88
  - 95-110
  - 114-118
  - 121-129
  - 132-135
  - 138-147
- file: crates/gcode/src/skill.rs
  ranges:
  - 20-23
  - 26-29
  - 61-63
  - 67-72
  - 75-85
  - 88-94
  - 100-107
  - 109-114
  - 117-128
  - 131-148
  - 151-164
  - 167-188
  - 191-220
- file: crates/gcode/src/utils.rs
  ranges:
  - 4-12
  - 14-16
  - 18-22
  - 29-31
  - 34-36
  - 39-41
  - 44-48
  - 51-53
- file: crates/gcode/src/vector/code_symbols/embedding.rs
  ranges:
  - 21-23
  - 26-29
  - 31-35
  - 37-41
  - 44-47
  - 49-121
  - 123-126
  - 128-140
  - 142-145
  - 147-179
  - 181-203
  - 205-211
  - 213-216
  - 218-224
  - 226-247
  - 249-270
  - 272-287
  - 289-320
  - 331-333
  - 335-341
  - 343-354
  - 357-391
  - 394-416
  - 419-442
- file: crates/gcode/src/vector/code_symbols/lifecycle.rs
  ranges:
  - 29-37
  - 39-43
  - 45-56
  - 58-376
  - 378-389
  - 391-393
- file: crates/gcode/src/vector/code_symbols/qdrant.rs
  ranges:
  - 18-24
  - 26-28
  - 30-37
  - 39-47
  - 49-76
  - 78-99
  - 101-111
  - 113-124
  - 126-141
  - 143-152
  - 154-164
  - 166-173
  - 175-194
  - 196-217
  - 219-227
  - 229-299
  - 301-311
  - 318-323
  - 326-336
- file: crates/gcode/src/vector/code_symbols/repository.rs
  ranges:
  - 6-18
  - 20-25
  - 27-35
  - 38-43
  - 45-56
  - 59-77
- file: crates/gcode/src/vector/code_symbols/search.rs
  ranges:
  - 8-14
  - 16-26
  - '28'
  - 30-58
  - 63-81
- file: crates/gcode/src/vector/code_symbols/tests.rs
  ranges:
  - 13-34
  - 36-44
  - 47-74
  - 77-86
  - 89-94
  - 97-102
  - 105-117
  - 120-137
  - 139-153
  - 156-167
  - 170-184
  - 187-236
  - 239-256
  - 259-321
  - 324-364
  - 367-390
  - 393-422
  - 425-463
  - 466-512
  - 515-580
  - 583-653
  - 656-703
  - 705-762
  - 764-783
  - 785-796
  - 798-803
  - 805-819
  - 821-838
  - 840-849
  - 851-859
  - 862-873
  - 876-884
  - 886-915
  - 917-937
  - 939-979
- file: crates/gcode/src/vector/code_symbols/types.rs
  ranges:
  - 7-12
  - 15-18
  - 20-24
  - '26'
  - 29-57
  - 59-96
  - 100-105
  - 108-112
  - 115-118
  - 121-124
  - 127-137
  - 140-162
  - 164-203
  - 205-209
  - '211'
- file: crates/gcode/src/visibility.rs
  ranges:
  - 13-17
  - 19-21
  - 23-32
  - 34-53
  - 55-99
  - 101-149
  - 151-153
  - 155-181
  - 183-193
  - 195-205
  - 207-222
  - 224-254
  - 256-291
  - 293-319
  - 321-350
  - 352-354
  - 356-362
  - 364-383
  - 385-395
  - 397-413
  - 415-427
  - 429-450
  - 452-497
  - 499-539
  - 541-558
  - 566-587
  - 590-598
  - 601-611
- file: crates/gcore/assets/docker-compose.services.yml
  ranges:
  - 5-117
  - 119-128
- file: crates/gcore/assets/postgres-pgsearch/version.json
  ranges:
  - 2-8
- file: crates/gcore/src/ai/daemon.rs
  ranges:
  - 19-24
  - 27-31
  - 34-41
  - 44-96
  - 98-136
  - 138-144
  - 149-187
  - 189-223
  - 225-233
  - 235-237
  - 239-245
  - 247-263
  - 265-267
  - 269-271
  - 273-293
  - 295-304
  - 306-331
  - 333-350
  - 352-356
  - 358-360
  - 362-364
  - 366-402
  - 404-423
  - 437-488
  - 491-510
  - 513-530
  - 533-555
  - 558-579
  - 582-605
  - 608-638
  - 641-656
  - 659-694
  - 697-750
  - 753-770
  - 772-781
  - 783-786
  - 788-795
  - 797-799
  - 801-803
  - 805-814
  - 816-833
  - 835-848
  - 850-856
  - 858-881
  - 883-902
- file: crates/gcore/src/ai/embeddings.rs
  ranges:
  - 19-38
  - 42-92
  - 94-105
  - 107-133
  - 140-148
  - 151-166
  - 169-190
  - 193-197
  - 200-217
  - 220-242
  - 245-258
  - 261-273
- file: crates/gcore/src/ai/mod.rs
  ranges:
  - 31-35
  - 37-48
  - 50-62
  - 64-76
  - 79-82
  - 85-89
  - 91-108
  - 110-135
  - 137-142
  - 144-146
  - 148-150
  - 152-169
  - 171-201
  - 204-209
  - 211-218
  - 220-235
  - 237-248
  - 250-258
  - 260-262
  - 264-297
  - 299-310
  - 312-318
  - 320-322
  - 324-342
  - 344-347
  - 349-359
  - 361-367
  - 376-392
  - 395-417
  - 420-433
  - 436-440
  - 443-450
  - 453-466
  - 469-508
  - 511-546
  - 549-579
  - 581-594
- file: crates/gcore/src/ai/probe.rs
  ranges:
  - 20-23
  - 26-34
  - 37-42
  - 45-50
  - 53-56
  - 58-64
  - 66-78
  - 80-82
  - 84-89
  - 91-93
  - 95-97
  - 99-110
  - 112-176
  - 178-241
  - 243-251
  - 253-271
  - 274-277
  - 279-281
  - '283'
  - 285-300
  - 309-361
  - 364-377
  - 380-389
  - 392-418
  - 421-444
  - 447-466
  - 469-495
  - 497-500
  - 502-515
  - 517-530
- file: crates/gcore/src/ai/text.rs
  ranges:
  - 9-15
  - 17-35
  - 37-67
  - 69-87
  - 98-120
  - 123-134
  - 136-138
  - 140-143
  - 145-152
  - 154-171
  - 173-186
- file: crates/gcore/src/ai/transcription.rs
  ranges:
  - 11-14
  - 16-37
  - 39-73
  - 75-99
  - 101-142
  - 152-178
  - 181-201
  - 203-205
  - 207-214
  - 216-233
  - 235-248
- file: crates/gcore/src/ai/vision.rs
  ranges:
  - 14-17
  - 19-35
  - 37-63
  - 65-90
  - 92-104
  - 106-121
  - 123-156
  - 158-173
  - 175-179
  - 190-222
  - 225-234
  - 237-246
  - 248-250
  - 252-255
  - 257-264
  - 266-283
  - 285-298
- file: crates/gcore/src/ai_context.rs
  ranges:
  - 25-30
  - 32-69
  - 73-76
  - 80-86
  - 88-124
  - 127-129
  - 133-135
  - 137-141
  - 143-191
  - 193-199
  - 203-205
  - 207-217
  - 219-225
  - 232-235
  - '237'
  - 239-246
  - 252-257
  - 259-267
  - 274-283
  - 285-296
  - 299-302
  - '306'
  - 308-319
  - 323-327
  - 334-340
  - 342-344
  - 352-367
  - 369-374
  - 378-385
  - 399-402
  - 404-425
  - 427-438
  - 440-443
  - 445-457
  - 459-463
  - 465-469
  - 472-525
  - 528-548
  - 551-579
  - 582-606
  - 609-625
  - 628-637
  - 640-651
  - 654-713
  - 716-738
- file: crates/gcore/src/ai_types.rs
  ranges:
  - 9-13
  - 17-26
  - 28-34
  - 38-44
  - 46-51
  - 55-64
  - 67-74
  - 76-89
  - 91-96
  - 100-126
  - 128-191
  - 193-209
  - '211'
  - 214-231
  - 234-238
  - 240-261
  - 263-280
  - 282-295
  - 297-299
  - 306-313
  - 316-324
  - 327-341
  - 344-375
  - 378-389
  - 392-404
  - 407-419
- file: crates/gcore/src/bootstrap.rs
  ranges:
  - 33-36
  - 38-45
  - 52-54
  - 60-65
  - 71-92
  - 101-105
  - 108-113
  - 116-121
  - 124-129
  - 132-139
  - 142-149
  - 152-157
  - 160-178
- file: crates/gcore/src/cli_contract.rs
  ranges:
  - 4-12
  - 15-30
  - 32-52
  - 55-58
  - 61-68
  - 71-75
  - 78-82
  - 84-123
  - 125-141
  - 150-178
- file: crates/gcore/src/codewiki_contract.rs
  ranges:
  - 60-82
- file: crates/gcore/src/config/resolve.rs
  ranges:
  - 11-21
  - 24-75
  - 78-84
  - 87-90
  - 93-95
  - 103-112
  - 114-126
  - '130'
  - 132-143
  - 146-165
  - 168-174
  - 177-179
  - 182-189
  - 192-202
  - 205-240
  - 242-244
  - 247-254
  - 257-265
  - 268-279
  - 281-317
  - 319-341
  - 343-345
  - 347-350
  - 352-364
  - 366-375
  - 382-404
  - 406-408
  - 410-416
  - 418-435
  - 437-463
  - 465-485
  - 487-491
- file: crates/gcore/src/config/tests.rs
  ranges:
  - 9-11
  - 18-32
  - 34-47
  - 49-57
  - 61-63
  - 65-98
  - 100-104
  - 107-110
  - 112-132
  - 134-146
  - 149-152
  - 154-167
  - 169-180
  - 183-186
  - 188-198
  - 200-210
  - 213-233
  - 236-257
  - 260-284
  - 288-307
  - 310-323
  - 326-342
  - 345-354
  - 357-392
  - 395-458
  - 462-480
  - 484-505
  - 508-546
  - 549-588
  - 591-610
  - 613-659
  - 662-688
  - 691-737
  - 740-747
  - 750-763
  - 766-779
  - 783-799
  - 802-813
  - 817-834
  - 837-850
  - 853-862
  - 865-872
  - 875-888
  - 891-905
  - 907-913
  - 915-919
  - 921-953
  - 955-969
  - 971-984
  - 986-991
- file: crates/gcore/src/config/types.rs
  ranges:
  - 5-9
  - 15-18
  - 22-28
  - 32-34
  - 36-42
  - 46-52
  - 54-68
  - 71-73
  - 75-79
  - '81'
  - 85-91
  - 93-173
  - 175-190
  - 193-195
  - 197-201
  - '203'
  - 207-220
  - 224-227
  - 338-340
  - 344-347
- file: crates/gcore/src/daemon_url.rs
  ranges:
  - 28-34
  - 40-42
  - 47-59
  - 61-64
  - 72-78
  - 86-91
  - 94-98
  - 101-104
  - 107-114
  - 117-124
  - 127-130
  - 133-136
  - 139-146
  - 149-156
  - 159-164
  - 167-172
  - 175-180
  - 183-187
  - 190-192
  - 195-234
- file: crates/gcore/src/degradation.rs
  ranges:
  - 12-22
  - 24-29
  - 33-40
  - 46-53
  - 57-91
  - 93-98
  - 100-115
  - 117-132
  - 134-139
  - 150-171
  - 173-189
  - 191-195
  - 199-233
  - 240-261
  - 264-293
  - 296-309
  - 312-354
  - 357-382
  - 385-397
  - 400-417
- file: crates/gcore/src/falkor.rs
  ranges:
  - '22'
  - 28-30
  - 36-38
  - 42-44
  - 47-52
  - 55-127
  - 136-143
  - 145-172
  - 175-177
  - 180-182
  - 185-187
  - 195-198
  - 200-202
  - 207-220
  - 222-224
  - 226-241
  - 243-266
  - '275'
  - 277-283
  - 286-334
  - 337-345
  - 348-361
  - 364-389
  - 392-415
  - 418-441
  - 443-462
  - 465-474
  - 477-481
- file: crates/gcore/src/graph_analytics.rs
  ranges:
  - 7-11
  - 14-18
  - 21-24
  - 27-31
  - 34-38
  - 41-44
  - 47-51
  - 54-58
  - 61-68
  - 70-87
  - 90-94
  - 97-103
  - 105-374
  - 376-382
  - 384-390
  - 392-473
  - 475-480
  - 482-484
  - 490-518
  - 521-582
- file: crates/gcore/src/indexing.rs
  ranges:
  - 17-26
  - 28-67
  - 70-74
  - 77-91
  - 93-100
  - 104-115
  - 119-126
  - 128-137
  - 141-147
  - 150-173
  - 183-189
  - 191-208
  - 211-220
  - 223-241
  - 244-249
  - 252-262
  - 265-284
  - 287-309
  - 312-333
  - 336-357
  - 360-365
  - 368-397
- file: crates/gcore/src/layered_config.rs
  ranges:
  - 17-25
  - 32-63
  - 65-70
  - 72-77
  - 88-90
  - 94-98
  - 100-117
  - 119-130
  - 132-140
  - 143-159
  - 162-179
  - 182-198
  - 201-210
  - 213-232
  - 235-249
  - 252-261
- file: crates/gcore/src/lib.rs
  ranges:
  - 27-34
- file: crates/gcore/src/libpq.rs
  ranges:
  - 1-39
- file: crates/gcore/src/local_backend.rs
  ranges:
  - 14-20
  - 24-31
  - 35-68
  - 72-76
  - 79-108
  - 111-137
  - 140-191
  - 194-202
  - 205-213
  - 224-248
  - 250-257
  - 260-267
  - 270-288
  - 290-296
- file: crates/gcore/src/postgres.rs
  ranges:
  - 16-22
  - 25-27
  - 36-45
  - 49-58
  - 66-71
  - 73-101
  - 104-110
  - 112-119
  - 121-134
  - 136-150
  - 152-167
  - 169-182
  - 184-189
  - 191-193
  - 195-197
  - 199-209
  - 211-216
  - 219-223
  - 225-240
  - 242-247
  - 249-260
  - 262-278
  - 280-285
  - 292-310
  - 313-334
  - 337-347
  - 350-381
  - 384-391
  - 394-402
  - 405-413
- file: crates/gcore/src/project.rs
  ranges:
  - 12-24
  - 28-51
  - 53-62
  - 70-89
  - 92-113
  - 116-126
  - 129-145
  - 148-164
- file: crates/gcore/src/provisioning/bootstrap.rs
  ranges:
  - 8-15
  - 17-39
  - 41-71
  - 73-79
  - 81-134
  - 136-157
  - 159-161
  - 167-172
  - 175-179
  - 182-186
  - 189-194
  - 197-207
- file: crates/gcore/src/provisioning/docker.rs
  ranges:
  - 9-18
  - 20-41
  - 44-49
  - 52-58
  - 61-66
  - 69-73
  - 75-77
  - '79'
  - 81-98
  - 100-104
  - 106-109
  - 111-118
  - 120-148
  - 150-156
  - 158-190
  - 192-271
  - 273-306
  - 309-313
  - 315-318
  - 320-331
  - 333-339
  - 341-362
  - 364-370
  - 372-382
  - 384-403
  - 405-418
- file: crates/gcore/src/provisioning/hub.rs
  ranges:
  - 4-9
  - 11-20
  - 23-26
  - 28-35
  - 38-41
  - 44-48
  - 51-54
  - 56-66
  - 69-87
  - 89-167
  - 169-279
  - 281-283
  - 286-337
  - 340-344
  - 347-352
  - 355-358
  - 360-396
  - 398-408
  - 411-414
  - 416-428
  - 430-437
  - 440-442
  - 445-447
  - 450-455
  - 458-470
- file: crates/gcore/src/provisioning/mod.rs
  ranges:
  - 53-55
  - 57-115
  - 117-128
  - 130-132
  - 134-136
  - 138-140
  - 142-151
  - 153-166
  - 168-203
- file: crates/gcore/src/provisioning/tests.rs
  ranges:
  - 5-7
  - 9-35
  - 37-41
  - 43-46
  - 49-74
  - 77-89
  - 92-119
  - 122-136
  - 139-151
  - 154-170
  - 173-192
  - 195-217
  - 219-227
  - 230-254
  - 257-294
  - 297-306
  - 308-323
  - 326-363
  - 366-420
  - 423-454
  - 457-487
  - 490-543
  - 546-586
  - 589-652
  - 655-687
  - 690-692
  - 694-703
  - 706-709
  - 711-729
- file: crates/gcore/src/qdrant.rs
  ranges:
  - 20-36
  - 38-47
  - 50-53
  - 56-59
  - 63-67
  - 70-73
  - 77-81
  - 85-89
  - 92-114
  - 117-173
  - 176-194
  - 197-219
  - 222-244
  - 247-306
  - 308-334
  - 337-399
  - 401-407
  - 409-433
  - 435-449
  - 451-461
  - 463-469
  - 471-482
  - 484-491
  - 493-510
  - 512-524
  - 526-528
  - 530-532
  - 534-552
  - 554-572
  - 574-583
- file: crates/gcore/src/qdrant/naming.rs
  ranges:
  - 3-10
  - 13-22
  - 25-43
  - 45-70
  - 77-90
  - 93-110
  - 113-124
- file: crates/gcore/src/qdrant/tests.rs
  ranges:
  - 12-30
  - 33-59
  - 62-99
  - 102-128
  - 131-161
  - 164-207
  - 210-250
  - 253-292
  - 295-376
  - 379-397
  - 400-414
  - 417-494
  - 497-523
  - 525-527
  - 529-556
- file: crates/gcore/src/search.rs
  ranges:
  - '20'
  - 22-36
  - 39-41
  - 45-55
  - 59-63
  - 67-70
  - 76-130
  - 133-156
  - 163-183
  - 186-201
  - 204-223
  - 226-230
  - 233-235
  - 238-246
  - 248-268
  - 271-280
  - 283-296
- file: crates/gcore/src/secrets.rs
  ranges:
  - 18-22
  - 24-30
  - 33-63
  - 66-68
  - 70-103
  - 105-116
  - 118-133
  - 135-137
  - 139-168
  - 175-181
  - 184-189
  - 192-200
  - 203-211
  - 214-221
  - 224-232
  - 236-249
  - 252-257
  - 260-274
  - 277-282
  - 285-290
  - 293-304
  - 307-314
  - 316-324
- file: crates/gcore/src/setup.rs
  ranges:
  - 11-18
  - 26-34
  - 38-43
  - 45-50
  - 53-54
  - 57-64
  - 69-84
  - 90-100
  - 104-107
  - 111-113
  - 118-120
  - 125-132
  - 136-156
  - '159'
  - 162-169
  - 172-181
  - 190-245
  - 248-274
  - 277-315
- file: crates/ghook/schemas/diagnose-output.v2.schema.json
  ranges:
  - 2-79
- file: crates/ghook/schemas/inbox-envelope.v1.schema.json
  ranges:
  - 2-63
- file: crates/ghook/src/cli_config.rs
  ranges:
  - 11-18
  - 20-61
  - 68-74
  - 77-80
  - 83-88
  - 91-96
  - 99-101
  - 104-108
  - 111-116
- file: crates/ghook/src/detach.rs
  ranges:
  - 23-43
- file: crates/ghook/src/diagnose.rs
  ranges:
  - 15-32
  - 42-45
  - 51-60
  - 62-70
  - 72-120
  - 128-134
  - 137-143
  - 146-152
  - 155-161
  - 163-170
  - 172-177
  - 180-192
  - 195-207
  - 210-213
  - 216-221
  - 224-246
  - 249-256
  - 259-266
- file: crates/ghook/src/envelope.rs
  ranges:
  - 24-32
  - 34-52
  - 59-70
  - 73-84
  - 87-109
  - 112-123
  - 126-140
  - 143-162
- file: crates/ghook/src/json_value.rs
  ranges:
  - 3-20
  - 28-52
- file: crates/ghook/src/main.rs
  ranges:
  - 45-49
  - 57-81
  - 83-106
  - 108-124
  - 126-289
  - 291-297
  - 299-301
  - 303-331
  - 333-341
  - 343-413
  - 415-434
  - 436-496
  - 498-520
  - 522-550
  - 552-564
  - 575-578
  - 581-593
  - 596-604
  - 607-647
  - 650-671
  - 674-690
  - 693-709
  - 712-729
  - 732-750
  - 753-769
  - 772-788
  - 791-802
  - 805-811
  - 814-830
  - 833-847
  - 850-865
  - 868-881
  - 884-894
  - 897-910
  - 913-926
  - 929-961
  - 964-973
- file: crates/ghook/src/output.rs
  ranges:
  - 3-5
  - 7-9
- file: crates/ghook/src/planned_shutdown.rs
  ranges:
  - 21-27
  - 29-37
  - 39-50
  - 52-54
  - 56-62
  - 64-75
  - 77-79
  - 81-84
  - 86-113
  - 115-119
  - 121-130
  - 132-134
  - 136-142
  - 144-152
  - 154-160
  - 162-169
  - 171-176
  - 178-184
  - 195-198
  - 201-206
  - 209-219
  - 222-237
  - 240-282
  - 285-291
  - 294-304
  - 307-323
  - 326-328
  - 331-353
  - 356-366
  - 369-399
  - 402-408
- file: crates/ghook/src/source.rs
  ranges:
  - 3-14
  - 20-27
  - 29-35
  - '37'
  - 39-44
  - 46-50
  - 53-82
- file: crates/ghook/src/statusline.rs
  ranges:
  - 25-27
  - 29-35
  - 37-67
  - 69-104
  - 106-119
  - 121-168
  - 170-174
  - 177-183
  - '186'
  - 189-194
  - 197-201
  - 217-222
  - 225-229
  - 232-236
  - 239-245
  - 248-253
  - 256-283
  - 286-310
  - 313-324
  - 327-344
  - 347-371
  - 374-397
- file: crates/ghook/src/terminal_context.rs
  ranges:
  - 18-23
  - 25-32
  - 34-65
  - 71-77
  - 79-84
  - 86-102
  - 104-126
  - 128-133
  - 138-145
  - 153-158
  - 161-164
  - 167-171
  - 174-187
  - 190-198
  - 201-209
  - 212-216
  - 219-237
- file: crates/ghook/src/transport.rs
  ranges:
  - 31-36
  - 40-45
  - 49-55
  - 58-60
  - 63-65
  - 68-74
  - 77-81
  - 87-114
  - 119-125
  - 127-129
  - 137-204
  - 206-221
  - 225-232
  - 242-273
  - 286-290
  - 293-296
  - 299-305
  - 308-314
  - 317-332
  - 335-348
  - 351-404
  - 407-458
  - 461-493
- file: crates/gloc/config.yaml
  ranges:
  - 11-57
- file: crates/gloc/src/backend.rs
  ranges:
  - 7-12
  - 14-23
  - 28-62
  - 67-108
  - 111-129
  - 132-149
  - 153-162
  - 168-175
  - 178-181
  - 184-189
  - 192-201
  - 204-207
  - 210-213
  - 216-219
  - 222-225
  - 228-231
  - 234-243
- file: crates/gloc/src/config.rs
  ranges:
  - 13-22
  - 25-32
  - 34-42
  - 44-46
  - 48-50
  - 53-65
  - 67-138
  - 142-148
  - 155-160
  - 163-168
  - 171-182
  - 185-189
  - 192-207
  - 210-219
  - 222-226
  - 229-232
  - 235-238
  - 241-250
  - 253-262
  - 265-274
  - 277-288
  - 291-299
  - 302-307
  - 310-317
  - 320-327
- file: crates/gloc/src/exec.rs
  ranges:
  - 9-21
  - 24-36
  - 39-45
  - 51-80
  - 87-94
  - 96-109
  - 111-123
  - 126-134
  - 137-144
  - 147-150
  - 153-156
  - 159-163
  - 166-171
  - 174-188
  - 191-194
  - 197-199
- file: crates/gloc/src/main.rs
  ranges:
  - 16-52
  - 54-118
  - 120-130
  - 132-155
  - 157-202
  - 204-213
  - 215-223
  - 225-250
  - 252-255
  - 257-288
  - 294-301
  - 304-318
  - 321-327
- file: crates/gsqz/config.yaml
  ranges:
  - 12-15
  - 17-204
  - 206-214
- file: crates/gsqz/src/command_split.rs
  ranges:
  - 5-85
  - 92-94
  - 97-102
  - 105-107
  - 110-112
  - 115-120
  - 123-128
  - 131-136
  - 139-144
  - 147-152
  - 155-157
  - 160-162
  - 165-167
- file: crates/gsqz/src/compressor.rs
  ranges:
  - 7-12
  - 14-34
  - 36-40
  - 42-52
  - 54-60
  - 62-67
  - 69-76
  - 78-233
  - 235-266
  - 272-274
  - 277-282
  - 285-293
  - 296-304
  - 307-315
  - 318-327
  - 330-345
  - 348-358
  - 361-377
  - 380-402
  - 405-410
  - 413-424
  - 427-431
  - 434-455
  - 458-468
  - 471-481
  - 484-493
  - 496-507
  - 510-523
  - 526-548
  - 551-571
  - 574-583
  - 586-618
  - 621-646
  - 649-666
  - 669-678
  - 681-691
  - 694-702
  - 705-715
- file: crates/gsqz/src/config.rs
  ranges:
  - 26-35
  - 38-47
  - 49-58
  - 60-62
  - 64-66
  - 69-76
  - 79-87
  - 91-166
  - 169-172
  - 175-177
  - 180-189
  - 191-193
  - 195-197
  - '200'
  - 203-205
  - 208-211
  - 214-216
  - 219-224
  - 227-230
  - 232-234
  - 237-240
  - 242-248
  - 250-257
  - 259-326
  - 333-338
  - 341-345
  - 348-353
  - 356-359
  - 362-368
  - 371-374
  - 377-381
  - 384-388
  - 391-398
  - 401-408
  - 411-423
  - 426-436
  - 439-443
  - 446-457
  - 460-473
  - 476-480
  - 483-490
  - 493-503
  - 506-513
  - 516-526
- file: crates/gsqz/src/daemon.rs
  ranges:
  - 11-23
  - 26-28
  - 32-43
  - 46-53
  - 62-76
  - 79-83
  - 90-95
  - 98-105
  - 108-126
- file: crates/gsqz/src/main.rs
  ranges:
  - 25-48
  - 50-65
  - 67-139
  - 141-184
  - 186-276
- file: crates/gsqz/src/primitives/dedup.rs
  ranges:
  - 9-45
  - 52-58
  - 61-70
  - 73-77
  - 80-83
  - 86-89
  - 92-97
  - 100-118
  - 121-126
- file: crates/gsqz/src/primitives/filter.rs
  ranges:
  - 4-15
  - 22-32
  - 35-39
  - 42-45
  - 48-52
  - 55-59
  - 62-72
  - 75-80
- file: crates/gsqz/src/primitives/group.rs
  ranges:
  - 8-21
  - 28-79
  - 99-183
  - 187-243
  - 247-296
  - 304-344
  - 348-387
  - 391-428
  - 434-475
  - 482-525
  - 532-543
  - 546-556
  - 559-567
  - 570-574
  - 577-581
  - 584-587
  - 590-595
  - 598-606
  - 609-623
  - 626-634
  - 637-650
  - 653-665
  - 668-681
  - 684-709
  - 712-716
  - 719-734
  - 737-750
  - 753-770
  - 773-781
  - 784-793
  - 796-805
  - 808-812
  - 815-822
  - 825-834
  - 837-840
  - 843-849
  - 852-861
  - 864-868
  - 871-880
  - 883-887
  - 890-901
  - 904-908
  - 911-921
  - 924-929
  - 932-940
- file: crates/gsqz/src/primitives/match_output.rs
  ranges:
  - 8-33
  - 39-45
  - 47-49
  - 52-56
  - 59-63
  - 66-70
  - 73-77
  - 80-87
  - 90-94
  - 97-101
  - 104-107
  - 110-115
- file: crates/gsqz/src/primitives/prose.rs
  ranges:
  - 5-9
  - 11-20
  - 23-34
  - 50-100
  - 102-109
  - 116-124
  - 187-211
  - 218-278
  - 280-303
  - 310-314
  - 317-321
  - 324-328
  - 331-335
  - 338-343
  - 346-350
  - 353-363
  - 366-370
  - 373-378
  - 381-386
  - 389-393
  - 396-399
  - 402-407
  - 410-418
  - 421-425
  - 428-432
  - 435-439
  - 442-446
- file: crates/gsqz/src/primitives/replace.rs
  ranges:
  - 7-30
  - 36-41
  - 44-48
  - 51-55
  - 58-63
  - 66-70
  - 73-77
  - 80-84
  - 87-90
  - 93-97
- file: crates/gsqz/src/primitives/truncate.rs
  ranges:
  - 5-27
  - 29-67
  - 74-78
  - 81-88
  - 91-106
  - 109-112
  - 115-120
  - 123-128
  - 131-136
  - 139-145
  - 148-157
  - 160-165
  - 168-178
- file: crates/gwiki/contract/gwiki.contract.json
  ranges:
  - 2-964
- file: crates/gwiki/src/ai/chunk.rs
  ranges:
  - 24-30
  - 33-35
  - 38-47
  - 49-56
  - '58'
  - 60-91
  - 93-99
  - 101-113
  - 115-117
  - 120-131
  - 133-197
  - 199-214
  - 216-229
  - 231-245
  - 247-265
  - 267-272
  - 274-281
  - 283-289
  - 291-293
  - '301'
  - 304-310
  - 313-319
  - 322-324
  - 335-343
  - 346-351
  - 354-385
  - 388-403
  - 406-432
  - 435-487
  - 489-492
  - 494-501
  - 503-513
  - 515-517
  - 519-525
  - 527-534
  - 536-539
  - 541-562
  - 564-571
  - 574-584
  - 586-594
  - 596-617
- file: crates/gwiki/src/ai/clients.rs
  ranges:
  - 20-23
  - 25-27
  - 29-33
  - 35-153
  - 155-199
  - 201-219
  - 221-254
  - 256-270
  - 272-274
  - 276-280
  - 282-302
  - 304-313
  - 315-322
  - 324-329
  - 331-357
  - 359-372
  - 384-439
  - 442-451
  - 453-469
  - 471-484
- file: crates/gwiki/src/ai/translate.rs
  ranges:
  - 6-29
  - 31-55
  - 57-87
  - 89-93
  - 95-97
  - 99-110
  - 112-122
  - 124-129
  - 131-133
  - 135-137
  - 147-154
  - 156-170
  - 172-207
  - 210-236
  - 239-259
  - 262-290
  - 293-316
  - 318-325
  - 327-349
- file: crates/gwiki/src/api.rs
  ranges:
  - 11-121
  - 124-127
  - 130-144
  - 147-149
  - 151-154
  - 156-162
  - 166-174
  - 177-182
  - 184-214
  - 218-222
  - 224-262
  - 264-268
  - 272-276
  - 278-286
  - 289-292
  - 294-315
  - 317-321
  - 324-328
  - 331-334
  - 343-359
  - 362-383
  - 386-416
  - 419-436
- file: crates/gwiki/src/audit.rs
  ranges:
  - 33-35
  - 37-73
  - 75-84
  - 87-93
  - 96-103
  - 106-111
  - 114-116
  - 118-141
  - 143-172
  - 174-186
  - 188-195
  - 197-226
  - 228-237
  - 239-244
  - 246-255
  - 257-262
  - 264-280
  - 282-288
  - 290-327
  - 330-335
  - 338-341
  - 348-425
  - 427-433
  - 435-444
  - 446-450
  - 452-454
  - 456-463
  - 465-477
  - 479-487
  - 489-502
  - 504-509
  - 511-516
  - 518-529
  - 531-553
  - 561-595
  - 598-664
  - 667-692
  - 695-722
  - 725-744
  - 747-767
  - 770-778
  - 781-794
  - 797-834
  - 837-844
  - 847-876
  - 879-932
  - 934-943
  - 946-965
- file: crates/gwiki/src/benchmark.rs
  ranges:
  - 30-39
  - 42-48
  - 51-58
  - 61-67
  - 70-75
  - 78-85
  - 88-91
  - 94-99
  - 104-114
  - 117-120
  - 122-145
  - 147-157
  - 159-193
  - 195-249
  - 251-264
  - 266-281
  - 283-299
  - 301-305
  - 307-331
  - 333-342
  - 344-377
  - 379-395
  - 397-489
  - 491-501
  - 503-509
  - 511-513
  - 515-534
  - 536-554
  - 556-562
  - 564-570
  - 572-584
  - 586-602
  - 604-611
  - 613-626
  - 628-631
  - 633-635
  - 637-639
  - 641-643
  - 645-649
  - '657'
  - 659-667
  - 669-671
  - 673-679
  - 681-702
  - 704-719
  - 721-737
  - 740-771
  - 774-818
  - 821-860
  - 863-873
  - 876-881
  - 884-893
- file: crates/gwiki/src/citations.rs
  ranges:
  - 6-14
  - 16-35
  - 37-46
  - 48-69
  - 71-88
  - 90-92
  - 100-124
  - 127-148
  - 151-170
- file: crates/gwiki/src/code_graph.rs
  ranges:
  - 15-18
  - 21-28
  - 31-34
  - 37-40
  - 43-47
  - 49-68
  - 70-79
  - 81-124
  - 126-132
  - 134-162
  - 164-167
  - 169-172
  - 174-181
  - 183-194
  - 196-208
  - 210-220
  - 222-243
  - 245-249
  - 251-253
  - 255-260
  - 262-275
  - 277-291
  - 299-301
  - 304-315
  - 318-331
  - 334-345
  - 348-356
  - 359-424
  - 427-466
  - 469-492
- file: crates/gwiki/src/collect.rs
  ranges:
  - 18-21
  - 24-30
  - 34-37
  - 39-42
  - 44-46
  - 48-140
  - 142-152
  - 154-165
  - 167-179
  - 181-204
  - 206-327
  - 329-352
  - 354-361
  - 363-390
  - 392-398
  - 400-418
  - 420-433
  - 435-480
  - 482-501
  - 503-550
  - 552-572
  - 574-588
  - 590-592
  - 594-604
  - 606-610
  - 612-615
  - 617-622
  - 624-628
  - 630-636
  - 638-642
  - 644-646
  - 648-654
  - 665-671
  - 674-736
  - 739-768
  - 771-781
  - 784-789
  - 792-797
  - 800-815
  - 818-830
  - 833-847
  - 850-866
  - 869-892
- file: crates/gwiki/src/commands/ask.rs
  ranges:
  - 25-46
  - 48-88
  - 90-99
  - 101-119
  - 121-174
  - 176-189
  - 191-241
  - 243-246
  - 248-261
  - 263-272
  - 274-293
  - 295-302
  - 304-305
  - 307-360
  - 362-368
  - 370-379
  - 381-387
  - 389-391
  - 393-422
  - 424-436
  - 438-451
  - 453-486
  - 488-507
  - 509-513
  - 515-533
  - 555-572
  - 576-590
  - 592-602
  - 604-627
  - 629-633
  - 635-639
  - 643-660
  - 662-694
  - 696-735
  - 737-739
  - 741-751
  - 753-803
  - 805-812
  - 828-870
  - 873-924
  - 927-1029
  - 1032-1050
  - 1052-1071
  - 1074-1117
  - 1120-1142
  - 1145-1151
  - 1154-1171
  - 1174-1203
  - 1206-1220
- file: crates/gwiki/src/commands/audit.rs
  ranges:
  - 3-13
- file: crates/gwiki/src/commands/backlinks.rs
  ranges:
  - 10-18
  - 20-28
  - 30-53
  - 55-78
  - 80-99
  - 101-126
- file: crates/gwiki/src/commands/benchmark.rs
  ranges:
  - 11-44
  - 46-73
  - 75-81
  - 83-121
- file: crates/gwiki/src/commands/citation_quality.rs
  ranges:
  - 26-33
  - 36-40
  - 43-49
  - 52-56
  - 59-64
  - 67-70
  - 73-76
  - 79-83
  - 86-89
  - 92-95
  - 98-101
  - 104-107
  - 110-114
  - 116-146
  - 148-162
  - 164-175
  - 177-222
  - 224-264
  - 266-276
  - 278-285
  - 287-302
  - 304-335
  - 337-349
  - 351-383
  - 385-395
  - 397-403
  - 405-416
  - 418-428
  - 430-454
  - 456-470
  - 472-483
  - 485-504
  - 506-517
  - 519-532
  - 534-548
  - 562-572
  - 575-639
  - 642-716
  - 719-769
  - 772-786
  - 789-818
  - 822-841
  - 843-847
  - 849-864
- file: crates/gwiki/src/commands/citation_quality/contradictions.rs
  ranges:
  - 15-18
  - 21-24
  - 27-29
  - 31-67
  - 69-117
  - 119-125
  - 127-163
  - 169-180
  - 182-193
  - 195-226
  - 228-234
  - 236-241
- file: crates/gwiki/src/commands/collect.rs
  ranges:
  - 10-20
  - 22-43
- file: crates/gwiki/src/commands/compile.rs
  ranges:
  - 8-74
- file: crates/gwiki/src/commands/export.rs
  ranges:
  - 4-30
- file: crates/gwiki/src/commands/graph.rs
  ranges:
  - 13-52
  - 54-67
  - 69-90
  - 93-118
  - 129-131
  - 133-138
  - 140-148
  - 150-158
  - 160-162
  - 164-168
  - 172-180
  - 184-186
  - 190-195
  - 199-204
  - 208-217
- file: crates/gwiki/src/commands/graph_context.rs
  ranges:
  - 13-83
  - 85-98
- file: crates/gwiki/src/commands/health.rs
  ranges:
  - 4-19
- file: crates/gwiki/src/commands/index.rs
  ranges:
  - 35-38
  - 40-46
  - 48-52
  - 54-86
  - 88-153
  - 155-191
  - 193-205
  - 207-229
  - 231-254
  - 256-258
  - 260-266
  - 268-272
  - 274-278
  - 280-285
  - 287-311
  - 313-364
  - 366-368
  - 370-372
  - 374-384
  - 386-391
  - 393-428
  - 430-454
  - 456-466
  - 468-509
  - 511-566
  - 568-638
  - 640-650
  - 656-658
  - 660-670
  - 673-680
  - 683-700
  - 703-727
  - 731-736
  - 738-746
- file: crates/gwiki/src/commands/init.rs
  ranges:
  - 9-20
  - 22-40
- file: crates/gwiki/src/commands/librarian.rs
  ranges:
  - 3-11
- file: crates/gwiki/src/commands/lint.rs
  ranges:
  - 3-11
- file: crates/gwiki/src/commands/mod.rs
  ranges:
  - 31-100
  - 102-113
  - 115-139
- file: crates/gwiki/src/commands/read.rs
  ranges:
  - 17-28
  - 30-57
  - 59-85
  - 87-114
  - 116-122
  - 124-152
  - 154-181
  - 183-197
  - 199-211
  - 213-219
  - 221-235
  - 237-241
  - 243-312
  - 314-320
  - 322-329
  - 331-340
  - 342-361
  - 364-378
  - 380-385
  - 387-487
  - 490-493
  - 495-509
  - 512-515
  - 518-522
  - 524-557
  - 566-592
  - 595-608
  - 611-622
- file: crates/gwiki/src/commands/refresh/candidate.rs
  ranges:
  - 15-74
  - 76-173
  - 175-214
  - 216-224
  - 226-245
  - 247-273
  - 275-310
- file: crates/gwiki/src/commands/refresh/mod.rs
  ranges:
  - 29-37
  - 39-49
  - 51-140
- file: crates/gwiki/src/commands/refresh/model.rs
  ranges:
  - 5-9
  - 12-17
  - 19-24
  - 27-38
  - 41-43
  - 45-52
  - 54-69
  - 72-85
  - 88-98
  - 101-107
  - 110-116
  - 119-125
  - 127-137
  - 140-144
  - 146-170
- file: crates/gwiki/src/commands/refresh/render.rs
  ranges:
  - 3-49
  - 51-68
- file: crates/gwiki/src/commands/refresh/selection.rs
  ranges:
  - 4-75
  - 79-82
  - 85-112
  - 115-118
  - 121-124
  - 126-138
  - 140-146
  - 148-152
  - 155-169
  - 171-184
  - 186-209
  - 211-219
  - 221-223
  - 225-231
  - 233-238
  - 247-293
- file: crates/gwiki/src/commands/refresh/tests.rs
  ranges:
  - 7-13
  - 15-31
  - 33-49
  - 51-103
  - 105-121
  - 123-131
  - 134-160
  - 163-185
  - 188-214
  - 217-250
  - 253-316
  - 319-342
  - 345-362
  - 365-370
  - 373-386
  - 389-406
  - 409-420
  - 423-434
  - 437-445
  - 448-464
- file: crates/gwiki/src/commands/refresh/vault.rs
  ranges:
  - 7-9
  - 16-49
  - 51-66
  - 68-101
  - 103-112
- file: crates/gwiki/src/commands/research.rs
  ranges:
  - 5-30
  - 32-74
- file: crates/gwiki/src/commands/review_report.rs
  ranges:
  - 28-105
  - 108-113
  - 115-143
  - 146-154
  - 157-167
  - 170-174
  - 177-183
  - 185-211
  - 213-229
  - 231-241
  - 243-260
  - 262-266
  - 268-294
  - 296-321
  - 323-362
  - 364-398
  - 400-429
  - 431-455
  - 457-470
  - 472-483
  - 485-492
  - 494-529
  - 531-533
  - 535-545
  - 547-561
  - 563-571
  - 573-587
  - 589-602
  - 604-611
  - 613-625
  - 639-706
  - 709-731
  - 734-741
  - 744-755
  - 758-771
- file: crates/gwiki/src/commands/search.rs
  ranges:
  - 23-30
  - 32-69
  - 71-134
  - 136-154
  - 156-162
  - 164-199
  - 201-225
  - 227-231
  - 233-239
  - 241-302
  - 304-315
  - 317-336
  - 343-356
  - 359-376
- file: crates/gwiki/src/commands/setup.rs
  ranges:
  - '18'
  - 20-92
  - 94-111
  - 113-123
  - 125-127
  - 129-180
  - 182-198
  - 200-233
  - 235-242
  - 244-248
  - 250-276
  - 278-292
  - 302-310
  - 313-355
  - 358-386
  - 389-412
  - 415-434
- file: crates/gwiki/src/commands/sources.rs
  ranges:
  - 15-23
  - 25-122
  - 125-138
  - 141-146
  - 148-172
  - 175-181
  - 183-193
  - 195-205
  - 208-213
  - 216-219
  - 221-230
  - 232-260
  - 262-301
  - 303-316
  - 318-340
  - 342-363
  - 365-396
  - 398-441
  - 443-462
  - 464-486
  - 488-490
  - 492-525
  - 527-566
  - 568-573
  - 575-585
  - 587-593
  - 595-616
  - 618-657
  - 659-661
  - 669-695
  - 698-716
  - 719-730
  - 733-738
  - 741-767
  - 770-812
  - 815-828
  - 831-839
  - 841-857
  - 859-874
- file: crates/gwiki/src/commands/status.rs
  ranges:
  - 6-9
  - 11-30
  - 32-36
  - 38-88
  - 90-94
- file: crates/gwiki/src/commands/trust.rs
  ranges:
  - 14-46
  - 48-52
  - 54-94
  - 96-105
  - 108-123
  - 125-197
  - 200-207
  - 209-220
  - 223-227
  - 230-234
  - 237-240
  - 243-246
  - 249-256
  - 258-295
  - 297-319
  - 321-327
  - 329-335
  - 337-357
  - 364-373
  - 376-404
  - 407-480
- file: crates/gwiki/src/compile/collect.rs
  ranges:
  - 10-82
  - 85-90
  - 93-97
  - 99-127
  - 129-142
  - 144-171
  - 173-185
  - 187-195
  - 197-203
  - 207-239
  - 246-269
  - 272-300
- file: crates/gwiki/src/compile/index.rs
  ranges:
  - 16-63
  - 65-94
  - 96-98
  - 100-102
  - 104-106
  - 108-117
  - 119-128
  - 130-132
  - 134-193
  - 195-217
  - 219-245
  - 247-250
  - 252-262
  - 264-270
  - 272-290
  - 292-294
  - 296-330
  - 337-344
- file: crates/gwiki/src/compile/mod.rs
  ranges:
  - 27-32
  - 35-38
  - 41-44
  - 46-53
  - 56-63
  - 66-77
  - 80-85
  - 88-91
  - 94-99
  - 101-191
  - 193-267
  - 270-275
  - 277-290
- file: crates/gwiki/src/compile/render.rs
  ranges:
  - 11-47
  - 49-63
  - 65-105
  - 107-144
  - 146-182
  - 184-186
  - 188-190
- file: crates/gwiki/src/compile/tests.rs
  ranges:
  - 6-24
  - 27-71
  - 74-101
  - 104-130
  - 133-169
  - 172-218
  - 222-242
  - 246-276
  - 279-348
  - 351-378
  - 381-409
  - 412-419
  - 422-441
- file: crates/gwiki/src/contract.rs
  ranges:
  - 6-499
  - 501-503
  - 505-515
  - 517-520
  - 522-528
  - 530-534
  - 536-546
- file: crates/gwiki/src/credibility.rs
  ranges:
  - 7-13
  - 16-22
  - 25-30
  - 33-36
  - 38-58
  - 60-89
  - 91-118
  - 120-135
  - 137-152
  - 154-175
  - 177-189
  - 196-223
- file: crates/gwiki/src/daemon.rs
  ranges:
  - 11-18
  - 26-31
  - 34-40
  - 43-50
  - 53-62
  - 65-74
  - 77-80
  - 82-84
  - '86'
  - 88-97
  - 165-168
  - 170-172
  - 174-247
  - 249-256
  - 258-275
  - 277-295
  - 297-329
  - 331-345
  - 354-356
  - 358-366
  - 368-375
  - 378-418
  - 421-431
  - 434-458
  - 461-480
- file: crates/gwiki/src/document.rs
  ranges:
  - 4-16
  - 18-34
  - 37-40
  - 42-71
  - 74-78
  - 80-96
  - '98'
  - 100-126
  - 133-217
- file: crates/gwiki/src/error.rs
  ranges:
  - 10-66
  - 68-100
  - 102-159
  - 161-176
  - 178-190
  - 192-196
  - 198-202
  - 204-208
  - 216-227
  - 230-240
  - 243-254
  - 257-275
  - 278-287
- file: crates/gwiki/src/events.rs
  ranges:
  - 14-20
  - 23-25
  - 27-75
  - 77-105
  - 114-147
- file: crates/gwiki/src/exports.rs
  ranges:
  - 9-13
  - 16-20
  - 23-27
  - 30-38
  - 41-45
  - 47-55
  - 58-62
  - 87-89
  - 91-101
  - 103-115
  - 117-136
  - 138-173
  - 175-180
  - 182-195
  - 197-219
  - 221-243
  - 245-250
  - 252-265
  - 281-335
  - 338-453
  - 456-472
  - 475-491
- file: crates/gwiki/src/falkor_graph.rs
  ranges:
  - 29-31
  - 33-43
  - 46-49
  - 51-54
  - 56-60
  - 62-78
  - 80-104
  - 106-197
  - 199-240
  - 242-250
  - 252-288
  - 290-296
  - 298-314
  - 316-320
  - 322-328
  - 330-332
  - 334-346
  - 348-350
  - 352-365
  - 367-370
  - 372-402
  - 404-439
  - 441-444
  - 446-473
  - 475-486
  - 488-573
  - 575-607
  - 609-617
  - 619-632
  - 634-636
  - 638-651
  - 653-658
  - 660-666
  - 668-670
  - 672-680
  - 682-697
  - 699-702
  - 704-708
  - 710-714
  - 717-719
  - 722-739
  - 741-745
  - 756-759
  - 762-805
  - 808-819
  - 822-824
  - 827-833
  - 836-841
  - 844-855
  - 858-874
  - 877-886
  - 889-908
  - 911-923
  - 926-978
- file: crates/gwiki/src/frontmatter.rs
  ranges:
  - 10-13
  - 16-30
  - 32-116
  - 119-125
  - 128-130
  - 132-136
  - '138'
  - 140-170
  - 173-191
  - 193-199
  - 201-205
  - 207-221
  - 223-232
  - 234-264
  - 266-286
  - 289-303
  - 306-314
  - 316-329
  - 331-344
  - 346-394
  - 396-398
  - 400-406
  - 408-415
  - 419-434
  - 436-450
  - 457-524
  - 527-546
  - 549-578
  - 581-626
  - 629-659
  - 662-691
- file: crates/gwiki/src/graph/analytics.rs
  ranges:
  - 14-22
  - 24-39
  - '41'
  - 44-51
  - 54-58
  - 61-65
  - 68-71
  - 74-78
  - 81-85
  - 87-91
  - 94-98
  - 100-158
  - 160-181
  - 183-218
  - 220-232
  - 234-242
  - 244-252
  - 254-261
  - 263-271
  - 285-316
  - 319-344
  - 347-362
- file: crates/gwiki/src/graph/context.rs
  ranges:
  - 8-11
  - 13-29
  - 32-39
  - 42-45
  - 48-53
  - 56-61
  - 64-73
  - 76-80
  - 83-88
  - 91-99
  - 102-105
  - 107-153
  - 155-172
  - 174-183
  - 185-201
  - 203-212
  - 214-227
  - 229-242
  - 244-272
  - 274-311
  - 313-320
  - 322-329
  - 331-340
  - 342-390
  - 392-394
  - 407-502
  - 505-557
  - 560-654
  - 656-662
  - 664-670
  - 672-684
  - 686-693
  - 695-714
- file: crates/gwiki/src/graph/export.rs
  ranges:
  - 11-112
  - 114-190
  - 204-317
  - 320-349
- file: crates/gwiki/src/graph/mod.rs
  ranges:
  - 22-26
  - 29-33
  - 36-39
  - 42-47
  - 50-59
  - 62-67
  - 70-72
  - 74-82
  - 85-92
  - 95-103
  - 106-113
  - 116-122
  - 125-127
  - 130-135
  - 138-143
  - 146-148
  - 150-156
  - 158-239
  - 242-244
  - 246-414
  - 416-418
  - 420-422
  - 424-426
  - 428-430
  - 432-440
  - 442-449
  - 451-453
  - 455-464
  - 466-475
  - 477-486
  - 488-497
  - 499-501
  - 503-505
  - 507-513
  - 515-517
  - 519-521
  - 523-532
  - 534-554
  - 556-565
  - 567-593
  - 595-599
  - 601-606
  - 613-679
  - 682-715
  - 718-725
  - 728-771
  - 774-817
  - 820-862
  - 864-870
  - 872-884
  - 886-893
- file: crates/gwiki/src/health.rs
  ranges:
  - 22-34
  - 37-41
  - 44-47
  - 49-53
  - 55-95
  - 97-106
  - 108-132
  - 134-142
  - 144-169
  - 171-188
  - 190-192
  - 194-197
  - 199-211
  - 213-226
  - 228-236
  - 238-240
  - 242-247
  - 249-253
  - 255-262
  - 265-276
  - 279-281
  - 283-287
  - 289-327
  - 329-339
  - 341-350
  - 353-360
  - 362-381
  - 384-386
  - 389-391
  - 393-398
  - 400-403
  - 406-435
  - 439-441
  - 443-459
  - 461-467
  - 469-489
  - 491-504
  - 506-521
  - 523-538
  - 540-560
  - 568-611
  - 614-638
  - 641-654
  - 657-695
  - 698-700
  - 703-712
  - 715-726
  - 729-735
  - 738-745
  - 748-753
  - 756-769
  - 772-807
  - 809-813
  - 815-830
- file: crates/gwiki/src/indexer.rs
  ranges:
  - 16-18
  - 20-26
  - 29-35
  - 37-58
  - 60-68
  - 70-74
  - 76-80
  - 82-87
  - 89-148
  - 150-155
  - 157-197
  - 199-236
  - 238-249
  - 251-264
  - 266-273
  - 275-280
  - 282-324
  - 326-355
  - 357-369
  - 371-373
  - 375-379
  - 381-394
  - 407-413
  - 415-448
  - 451-468
  - 471-509
  - 512-534
  - 537-561
  - 564-594
  - 597-617
  - 620-650
  - 653-686
  - 689-702
- file: crates/gwiki/src/ingest/audio.rs
  ranges:
  - 21-28
  - 31-37
  - 40-54
  - 56-87
  - 89-91
  - 94-96
  - 99-101
  - 104-125
  - 128-137
  - 139-145
  - 148-159
  - 161-202
  - 204-226
  - 228-238
  - 241-250
  - 253-258
  - 261-286
  - 289-299
  - 301-315
  - 318-325
  - 327-335
  - 337-365
  - 385-394
  - 397-403
  - '405'
  - 407-430
  - 433-438
  - 441-463
  - 466-500
  - 502-530
  - 533-537
  - 540-548
  - 551-577
  - 581-587
  - 591-625
  - 629-663
  - 667-693
  - 697-734
  - 738-776
  - 779-810
  - 813-848
  - 851-886
- file: crates/gwiki/src/ingest/document/html.rs
  ranges:
  - 8-39
  - 41-51
  - 53-76
  - 78-96
  - 98-110
  - 112-140
  - 142-148
  - 150-199
  - 201-213
  - 215-223
  - 230-235
  - 238-242
- file: crates/gwiki/src/ingest/document/mod.rs
  ranges:
  - 21-27
  - 30-36
  - 38-46
  - 49-53
  - 56-62
  - 64-66
  - 68-72
  - '74'
  - 77-86
  - 88-100
  - 103-114
  - 116-191
  - 193-201
  - 203-214
  - 216-218
  - 220-222
  - 224-229
- file: crates/gwiki/src/ingest/document/office.rs
  ranges:
  - 39-52
  - 54-56
  - 58-60
  - 62-64
  - 66-68
  - 70-81
  - 83-94
  - 96-109
  - 111-176
  - 178-262
  - 264-267
  - 269-309
  - 311-314
  - 316-393
  - 395-402
  - 404-414
  - 416-430
  - 432-450
  - 452-462
  - 464-469
  - 471-473
  - 475-479
  - 481-486
  - 493-502
  - 505-513
  - 516-521
- file: crates/gwiki/src/ingest/document/render.rs
  ranges:
  - 11-33
  - 36-67
  - 69-93
  - 95-122
  - 124-211
  - 213-228
  - 230-241
  - 248-260
  - 263-274
- file: crates/gwiki/src/ingest/document/tests.rs
  ranges:
  - 9-18
  - 20-25
  - 27-38
  - 40-53
  - 55-59
  - 61-70
  - 72-96
  - 98-118
  - 121-200
  - 203-258
  - 261-263
  - 266-273
  - 276-294
  - 297-317
  - 320-327
  - 330-337
- file: crates/gwiki/src/ingest/file.rs
  ranges:
  - 53-57
  - 60-63
  - 65-78
  - 80-262
  - 264-310
  - 312-336
  - 338-345
  - 347-349
  - 352-354
  - 356-371
  - 373-379
  - 382-414
  - 416-431
  - 433-447
  - 449-461
  - 463-508
  - 521-530
  - 532-538
  - 541-557
  - 560-613
  - 616-638
  - 641-655
  - 658-698
  - 701-728
  - 731-754
  - 757-829
  - 833-867
  - 871-897
  - 901-925
  - 929-953
- file: crates/gwiki/src/ingest/git.rs
  ranges:
  - 15-18
  - 22-27
  - 30-55
  - 58-74
  - 77-109
  - 112-127
  - 130-142
  - 145-154
  - 157-172
  - 181-236
  - 239-247
  - 250-261
- file: crates/gwiki/src/ingest/image.rs
  ranges:
  - 23-31
  - 34-40
  - 43-56
  - 59-70
  - 72-103
  - 106-116
  - 118-167
  - 169-177
  - 179-210
  - 213-218
  - 228-238
  - 241-272
  - 275-297
  - 301-335
  - 338-370
  - 373-375
- file: crates/gwiki/src/ingest/mediawiki.rs
  ranges:
  - 12-20
  - 23-41
  - 44-77
  - 86-123
- file: crates/gwiki/src/ingest/mod.rs
  ranges:
  - 25-29
  - 31-36
  - 38-46
  - 48-57
  - 59-73
  - 75-85
  - 87-107
  - 109-117
  - 120-135
  - 137-143
  - 146-149
  - 151-159
  - 161-171
  - 173-178
  - 180-186
  - 188-195
  - 197-204
  - 206-226
  - 228-230
  - 232-235
  - 237-244
  - 246-248
  - 250-252
  - 254-256
  - 258-260
  - 262-264
  - 266-305
  - 307-357
  - 359-374
  - 376-391
  - 393-410
  - 412-420
  - 422-449
  - 453-458
  - 460-474
  - 476-482
  - 508-517
  - 519-525
  - 527-542
  - 545-556
  - 559-585
  - 588-598
  - 601-618
  - 621-670
  - 673-719
  - 722-727
  - 729-746
  - 748-797
  - 800-833
- file: crates/gwiki/src/ingest/pdf/ingest.rs
  ranges:
  - 23-37
  - 41-52
  - 55-108
  - 111-128
  - 131-146
  - 149-220
  - 223-247
  - 250-257
  - 260-266
- file: crates/gwiki/src/ingest/pdf/markdown.rs
  ranges:
  - 15-89
  - 92-107
  - 110-135
  - 138-156
  - 159-239
  - 242-264
  - 267-272
  - 275-295
  - 298-319
  - 322-328
  - 331-335
  - 338-344
  - 351-360
  - 363-375
- file: crates/gwiki/src/ingest/pdf/mod.rs
  ranges:
  - 22-25
  - 28-34
  - 37-40
- file: crates/gwiki/src/ingest/pdf/render.rs
  ranges:
  - 23-39
  - 42-94
  - 97-100
  - 103-114
  - 117-128
  - 131-133
  - 136-144
  - 147-166
  - 169-174
  - 181-191
  - 195-202
- file: crates/gwiki/src/ingest/pdf/tests.rs
  ranges:
  - '21'
  - 23-27
  - 29-60
  - 63-65
  - 69-74
  - 77-137
  - 140-175
  - 178-182
  - 185-231
  - 234-289
  - 292-324
  - 327-331
  - 334-442
  - 446-453
- file: crates/gwiki/src/ingest/pdf/text.rs
  ranges:
  - 4-25
  - 32-36
  - 39-49
  - 52-54
  - 57-59
  - 62-64
  - 67-69
  - 72-74
  - 77-82
- file: crates/gwiki/src/ingest/pdf/types.rs
  ranges:
  - 11-14
  - 18-24
  - 28-33
  - 37-43
  - 47-49
  - 51-57
  - 60-81
- file: crates/gwiki/src/ingest/url.rs
  ranges:
  - 22-28
  - 31-35
  - 38-42
  - 45-48
  - 50-62
  - 65-74
  - 76-110
  - 112-145
  - 147-157
  - 159-164
  - 166-205
  - 208-210
  - 212-221
  - 223-296
  - 298-302
  - 304-318
  - 320-326
  - 328-355
  - 357-363
  - 365-377
  - 379-412
  - 414-418
  - 420-441
  - 443-445
  - 447-449
  - 451-460
  - 462-487
  - 489-516
  - 518-524
  - 526-538
  - 540-547
  - 549-555
  - 557-573
  - 575-585
  - 587-596
  - 598-632
  - 634-649
  - 651-657
  - 659-661
  - 663-683
  - 685-694
  - 712-752
  - 755-785
  - 788-799
  - 802-844
  - 847-867
  - 870-885
  - 888-908
  - 911-916
  - 918-934
  - 937-940
  - 942-983
- file: crates/gwiki/src/ingest/video/assets.rs
  ranges:
  - 4-23
  - 25-115
  - 118-122
  - 126-206
  - 208-212
  - 214-224
  - 226-242
- file: crates/gwiki/src/ingest/video/metadata.rs
  ranges:
  - 4-8
  - 10-25
  - 27-39
  - 43-57
  - 59-73
  - 76-84
  - 86-127
  - 129-134
- file: crates/gwiki/src/ingest/video/mod.rs
  ranges:
  - 32-45
  - 48-61
  - 64-73
  - 76-94
  - 97-104
  - 107-126
  - 128-163
  - 166-179
  - 181-235
- file: crates/gwiki/src/ingest/video/processing.rs
  ranges:
  - 18-26
  - '28'
  - 30-42
  - 45-64
  - 66-179
  - 181-197
  - 199-209
  - 212-216
  - 218-223
  - 225-329
  - 331-335
- file: crates/gwiki/src/ingest/video/tests.rs
  ranges:
  - 18-55
  - 57-62
  - 64-89
  - 91-111
  - '113'
  - 115-131
  - '133'
  - 135-144
  - '146'
  - 148-160
  - '162'
  - 164-170
  - 172-198
  - 201-273
  - 276-323
  - 326-329
  - 332-350
  - 354-446
  - 449-451
  - 454-461
  - 464-617
  - 620-638
  - 641-656
  - 659-707
  - 710-765
  - 767-799
  - 801-803
  - 805-810
  - 813-843
  - 846-883
  - 886-922
- file: crates/gwiki/src/ingest/wayback.rs
  ranges:
  - 18-25
  - 28-47
  - 50-60
  - 63-75
  - 78-98
  - 101-108
  - 111-118
  - 121-129
  - 132-139
  - 142-145
  - 148-153
  - 156-163
  - 166-171
  - 174-180
  - 183-185
  - 188-215
  - 218-226
  - 229-238
  - 241-266
  - 269-292
  - 295-304
  - 307-313
  - 316-352
  - 361-400
  - 403-413
  - 416-430
  - 433-465
  - 468-475
  - 478-491
  - 494-511
  - 513-516
- file: crates/gwiki/src/librarian.rs
  ranges:
  - 15-20
  - 22-31
  - 33-40
  - 43-50
  - 52-60
  - 63-69
  - 72-76
  - 79-85
  - 88-93
  - 96-100
  - 102-198
  - 200-230
  - 232-239
  - 241-253
  - 255-264
  - 266-272
  - 274-283
  - 285-291
  - 293-303
  - 305-355
  - 357-371
  - 373-390
  - 392-394
  - 396-403
  - 405-434
  - 436-452
  - 454-461
  - 475-562
  - 565-591
  - 594-617
  - 620-639
  - 642-656
  - 660-679
  - 681-685
  - 687-701
- file: crates/gwiki/src/links.rs
  ranges:
  - 4-7
  - 10-19
  - 21-23
  - 25-61
  - 63-72
  - 74-104
  - 106-141
  - 143-149
  - 151-166
  - 168-185
  - 187-202
  - 204-214
  - 216-225
  - 227-233
  - 235-257
  - 259-283
  - 285-289
  - 291-295
  - 297-309
  - 311-315
  - 317-322
  - 324-338
  - 340-342
  - 348-350
  - 352-383
  - 385-400
  - 402-406
  - 408-414
  - 416-418
  - 420-422
  - 429-468
  - 471-488
  - 491-500
  - 503-508
  - 511-517
  - 520-526
  - 529-536
  - 539-553
  - 556-567
- file: crates/gwiki/src/lint.rs
  ranges:
  - 13-22
  - 25-30
  - 33-36
  - 38-103
  - 105-126
  - 129-135
  - 137-169
  - 171-173
  - 175-181
  - 183-195
  - 197-254
  - 256-262
  - 264-270
  - 272-282
  - 284-290
  - 292-306
  - 308-316
  - 318-347
  - 349-365
  - 367-376
  - 378-397
  - 399-433
  - 435-440
  - 442-461
  - 463-476
  - 478-484
  - 487-493
  - 500-532
  - 535-552
  - 555-560
  - 563-568
  - 571-587
  - 589-593
- file: crates/gwiki/src/log.rs
  ranges:
  - 9-15
  - 19-22
  - 25-49
  - 52-90
  - 93-108
  - 111-117
  - 121-128
  - 131-150
  - 153-155
  - 158-164
  - 167-174
  - 183-211
  - 214-233
- file: crates/gwiki/src/main.rs
  ranges:
  - 46-60
  - 63-149
  - 152-167
  - 170-212
  - 215-225
  - 228-243
  - 246-254
  - 256-267
  - 270-285
  - 288-296
  - 304-312
  - 315-318
  - 321-324
  - 327-351
  - 354-372
  - 375-378
  - 381-394
  - 397-411
  - 414-418
  - '425'
  - 429-441
  - 443-450
  - 452-456
  - 458-510
  - 517-536
  - 538-540
  - 542-556
  - 558-709
  - 712-718
  - 722-732
  - 736-743
  - 746-756
  - 758-775
  - 778-794
- file: crates/gwiki/src/markdown.rs
  ranges:
  - 11-19
  - 22-29
  - 32-35
  - 38-41
  - 43-55
  - 57-68
  - 70-77
  - '79'
  - 81-85
  - 87-91
  - 93-116
  - 119-130
  - 132-185
  - 187-206
  - 208-230
  - 232-274
  - 276-296
  - 304-353
  - 356-368
  - 371-387
  - 390-399
- file: crates/gwiki/src/media.rs
  ranges:
  - 13-17
  - 19-22
  - 24-27
  - 29-35
  - 37-43
  - 45-52
  - 55-58
  - 60-69
  - 71-93
  - 95-125
  - 127-139
  - 141-200
  - 202-209
  - 211-213
  - 215-230
  - 232-245
  - 247-250
  - 252-267
  - 269-271
  - 273-283
  - 285-300
  - 302-311
  - 313-340
  - 342-346
  - 355-359
  - 362-366
  - 369-375
  - 379-436
  - 439-449
- file: crates/gwiki/src/models.rs
  ranges:
  - 12-15
  - 17-52
  - 55-61
  - 63-73
  - 76-80
  - 83-97
  - 99-150
  - 152-154
  - 156-158
  - 160-166
  - 168-174
  - 176-191
  - 198-236
  - 239-260
  - 263-284
  - 287-291
- file: crates/gwiki/src/output.rs
  ranges:
  - 10-13
  - 16-19
  - 21-28
  - '30'
  - 32-36
  - 38-42
  - 44-53
  - 55-61
  - 63-66
  - 68-70
  - 73-80
  - 82-99
  - 102-124
  - 127-135
  - 138-144
  - 149-152
  - 154-172
  - 175-185
  - 188-192
  - 195-202
  - 205-209
  - 216-220
  - 223-227
  - 230-236
  - 238-253
  - 256-261
  - 264-270
  - 272-287
  - 289-316
  - 325-416
  - 419-445
- file: crates/gwiki/src/paths.rs
  ranges:
  - 6-22
  - 24-27
  - 29-34
  - 42-47
  - 50-55
  - 58-69
  - 71-86
- file: crates/gwiki/src/provenance.rs
  ranges:
  - 14-22
  - 25-29
  - 32-36
  - 39-47
  - 49-168
  - 170-172
  - 174-205
  - 207-223
  - 230-278
  - 281-304
  - 307-313
- file: crates/gwiki/src/registry.rs
  ranges:
  - 15-20
  - 23-26
  - 29-33
  - 35-102
  - 104-136
  - 138-140
  - 142-144
  - 146-184
  - 186-203
  - 205-211
  - 213-225
  - 233-247
  - 250-322
  - 325-333
- file: crates/gwiki/src/research/mod.rs
  ranges:
  - '41'
  - 44-50
  - 53-59
  - 62-71
  - '74'
  - 77-83
  - 86-97
  - 100-122
  - 126-130
  - 134-142
  - 145-152
  - 156-160
  - 163-166
  - 168-287
  - 289-336
  - 338-353
  - 355-366
- file: crates/gwiki/src/research/model.rs
  ranges:
  - 21-24
  - 26-33
  - 35-97
  - 99-101
  - 103-114
  - 116-118
  - 120-136
  - 138-159
  - 161-163
  - 165-173
  - 175-177
  - 179-209
  - 211-214
  - 217-224
- file: crates/gwiki/src/research/notes.rs
  ranges:
  - 5-16
  - 18-20
  - 22-26
  - 28-99
  - 101-108
  - 110-113
  - 115-126
  - 128-157
  - 159-256
  - 258-300
  - 302-313
  - 315-318
  - 320-364
  - 366-402
  - 404-410
  - 415-423
  - 428-430
  - 432-441
  - 443-449
  - 451-459
- file: crates/gwiki/src/research/outcome.rs
  ranges:
  - 15-24
  - 26-41
  - 43-51
  - 53-89
  - 91-99
  - 101-127
  - 129-147
  - 149-152
  - 154-171
  - 173-188
  - 190-200
  - 202-216
  - 221-228
  - 230-252
  - 254-329
  - 331-334
  - 336-344
  - 346-353
  - 360-372
  - 375-385
  - 388-426
  - 429-444
  - 447-457
  - 460-470
  - 473-482
- file: crates/gwiki/src/research/storage.rs
  ranges:
  - 12-59
  - 61-91
  - 93-95
  - 97-135
  - 137-151
  - 153-155
  - 157-159
  - 168-177
- file: crates/gwiki/src/research/tests.rs
  ranges:
  - 8-21
  - 23-27
  - 29-37
  - 40-46
  - 49-60
  - 63-72
  - 75-108
  - 111-122
  - 125-169
  - 172-212
  - 215-255
  - 258-280
  - 283-323
  - 326-343
  - 346-379
  - 382-405
- file: crates/gwiki/src/research_loop/engine.rs
  ranges:
  - 19-28
  - 31-46
  - 48-160
  - 162-183
  - 185-278
  - 280-292
  - 294-331
  - 333-374
  - 376-407
  - 410-416
  - 419-435
  - 437-538
  - 540-554
  - 556-566
  - 568-573
  - 575-588
  - 590-593
  - 598-601
  - 607-610
  - 613-630
- file: crates/gwiki/src/research_loop/helpers.rs
  ranges:
  - 8-19
  - 21-80
  - 82-85
  - 90-102
  - 104-106
  - 108-119
  - 121-137
  - 139-175
  - 177-187
  - 189-203
  - 205-209
  - 211-221
  - 228-237
- file: crates/gwiki/src/research_loop/tests.rs
  ranges:
  - 12-14
  - 16-22
  - 24-33
  - '35'
  - 37-44
  - '46'
  - 48-57
  - '59'
  - 61-68
  - '70'
  - 72-76
  - '78'
  - 80-85
  - '87'
  - 89-100
  - 103-106
  - 108-122
  - 124-141
  - 144-168
  - 171-200
  - 202-210
  - 213-299
  - 302-363
  - 366-444
  - 447-503
  - 506-561
  - 564-601
  - 604-614
  - 617-628
  - 631-642
  - 645-656
  - 659-662
  - 665-668
  - 671-688
  - 692-706
- file: crates/gwiki/src/research_loop/types.rs
  ranges:
  - 11-17
  - 20-24
  - 27-39
  - 42-45
  - 48-53
  - 55-59
  - 61-66
  - 68-70
  - 72-74
  - 76-78
  - 80-84
  - 87-91
  - 93-95
  - 99-130
  - 133-140
  - 142-173
  - 176-179
  - 182-197
  - 199-206
  - 208-215
  - 219-226
  - 230-237
  - 241-244
  - 246-249
  - 251-254
  - 256-259
  - 261-264
  - 266-269
  - 271-282
- file: crates/gwiki/src/runner.rs
  ranges:
  - 7-9
  - 12-17
- file: crates/gwiki/src/schema.rs
  ranges:
  - '13'
  - 15-24
  - 26-28
  - 30-36
  - 38-57
  - 59-61
  - 63-75
  - 83-118
  - 121-126
- file: crates/gwiki/src/scope.rs
  ranges:
  - 12-16
  - 19-27
  - 29-89
  - 91-94
  - 96-121
  - 123-129
  - 131-152
  - 154-180
  - 182-188
  - 190-206
  - 216-218
  - 220-226
  - 228-236
  - 240-256
  - 259-273
  - 276-312
  - 315-341
- file: crates/gwiki/src/search/bm25.rs
  ranges:
  - 13-17
  - 20-23
  - 26-37
  - 39-44
  - 46-69
  - 71-157
  - 159-162
  - 164-176
  - 178-182
  - 184-186
  - 189-191
  - 195-225
  - 228-284
  - 286-289
  - 291-299
  - 301-303
  - 307-309
  - 312-316
  - 319-326
  - 335-374
  - 377-387
  - 390-395
  - 398-414
  - 417-427
  - 430-444
  - 447-455
  - 457-482
  - 484-504
  - 506-527
  - 529-538
  - 540-551
- file: crates/gwiki/src/search/graph_boost.rs
  ranges:
  - 21-24
  - 26-33
  - 35-39
  - 41-44
  - 46-51
  - '54'
  - 56-66
  - 68-70
  - 72-78
  - 80-90
  - 93-98
  - 101-103
  - 105-109
  - 111-124
  - 126-129
  - 131-146
  - 148-185
  - 188-191
  - 194-197
  - 199-264
  - 266-277
  - 279-301
  - 303-308
  - 310-347
  - 349-362
  - 364-384
  - 386-392
  - 399-427
  - 430-452
  - 455-473
  - 476-489
  - 492-512
  - 514-519
  - 521-526
- file: crates/gwiki/src/search/mod.rs
  ranges:
  - 14-18
  - 20-60
  - 63-67
  - 69-86
  - 89-92
  - 95-100
  - 103-108
  - 111-115
  - 118-131
  - 133-142
  - 144-149
  - 152-157
  - 160-163
  - 166-169
  - 171-180
  - '182'
  - 186-266
  - 268-285
  - 287-300
  - 302-304
  - 311-352
  - 355-386
  - 389-427
  - 429-454
  - 456-475
  - 477-483
  - '485'
  - 487-502
- file: crates/gwiki/src/search/rrf.rs
  ranges:
  - 8-92
  - 94-96
  - 98-108
  - 119-180
  - 183-225
  - 229-240
  - 242-267
- file: crates/gwiki/src/search/semantic.rs
  ranges:
  - 18-22
  - 25-28
  - 30-35
  - 37-54
  - 57-61
  - 63-70
  - 72-163
  - 165-170
  - 172-174
  - 176-182
  - 184-204
  - 206-211
  - 214-226
  - 234-245
  - 250-252
  - 255-261
  - 264-268
  - 271-306
  - 309-323
  - '327'
  - 330-334
  - 337-351
  - 354-365
  - 368-376
  - '379'
  - 381-390
  - 392-396
  - 398-411
  - 413-457
  - 459-461
  - 463-468
  - 470-478
  - 480-509
  - '512'
  - 515-525
  - 528-531
  - 534-541
  - 544-553
  - 556-560
  - 563-571
  - 574-585
  - '588'
  - 591-599
  - '602'
  - 605-614
  - 617-619
  - 622-638
- file: crates/gwiki/src/session.rs
  ranges:
  - 15-18
  - 20-46
  - 48-57
  - 60-64
  - 68-76
  - 79-87
  - 89-95
  - 97-160
  - 163-170
  - 173-183
  - 186-198
  - 200-317
  - 319-338
  - 340-349
  - 351-356
  - 358-365
  - 367-369
  - 371-387
  - 394-427
  - 430-441
  - 444-456
  - 459-467
  - 470-478
  - 481-506
  - 509-530
  - 533-554
- file: crates/gwiki/src/setup.rs
  ranges:
  - 29-35
  - 37-47
  - 50-54
  - 57-61
  - 64-66
  - 68-237
  - 239-267
  - 269-271
  - 273-279
  - 281-287
  - 289-295
  - 297-305
  - 307-329
  - 331-355
  - 363-454
  - 457-480
  - 483-488
  - 491-496
- file: crates/gwiki/src/sources/atomic.rs
  ranges:
  - 7-44
  - 46-56
  - 58-83
  - 85-104
  - 111-116
  - 120-129
- file: crates/gwiki/src/sources/manifest.rs
  ranges:
  - 23-25
  - 27-225
  - 227-265
  - 267-297
  - 299-303
  - 305-312
  - 314-323
- file: crates/gwiki/src/sources/render.rs
  ranges:
  - 15-45
  - 47-58
  - 60-70
  - 72-75
  - 77-124
  - 126-133
  - 135-137
  - 139-145
  - 147-166
  - 168-183
  - 185-190
  - 192-197
  - 199-204
  - 206-208
  - 215-221
  - 224-229
  - 232-234
- file: crates/gwiki/src/sources/tests.rs
  ranges:
  - 8-50
  - 53-113
  - 116-121
  - 124-140
  - 143-160
- file: crates/gwiki/src/sources/types.rs
  ranges:
  - 12-29
  - 31-51
  - 55-58
  - 60-67
  - 71-74
  - 76-83
  - 86-96
  - 98-150
  - 152-162
  - 165-179
  - 183-189
  - 191-198
  - 201-216
  - 218-245
  - 247-249
  - 251-259
  - 261-274
- file: crates/gwiki/src/store.rs
  ranges:
  - 15-21
  - 24-30
  - 33-40
  - 43-49
  - 52-57
  - 60-66
  - 69-73
  - 76-78
  - 80-112
  - 115-121
  - 123-134
  - '136'
  - 138-148
  - 150-159
  - 168-180
  - 182-233
  - 236-240
  - 242-246
  - 249-255
  - 257-264
  - 266-293
  - 297-313
  - 315-371
  - 373-469
  - 471-540
  - 542-586
  - 588-633
  - 635-642
  - 644-669
  - 672-674
  - 676-681
  - 683-688
  - 690-706
  - 708-710
  - 712-718
  - 720-725
  - 727-739
  - 741-744
  - 746-765
  - 767-775
  - 777-785
  - 787-794
  - 796-803
  - 805-809
  - 811-815
  - 817-829
  - 836-843
  - 846-867
  - 870-875
  - 878-922
  - 925-939
- file: crates/gwiki/src/support/config.rs
  ranges:
  - 18-20
  - 22-44
  - 46-61
  - 68-71
  - 73-80
  - 82-86
  - 88-93
  - 96-102
  - 104-111
  - 113-118
  - 120-127
  - 129-136
  - 138-142
  - 144-151
  - 153-168
  - 182-185
  - 187-201
  - 203-212
  - 214-220
  - 223-225
  - 227-232
  - 234-242
  - 245-257
  - 260-279
  - 283-301
  - 304-316
  - 320-332
  - 336-363
- file: crates/gwiki/src/support/counts.rs
  ranges:
  - 4-10
  - 12-20
  - 22-33
  - 36-42
  - 44-54
  - 56-72
  - 79-85
- file: crates/gwiki/src/support/env.rs
  ranges:
  - 21-24
  - 27-30
  - 32-49
  - 51-55
  - 57-66
  - 68-75
  - 77-89
  - 91-98
  - 100-109
  - 111-142
  - 144-154
  - 156-180
  - 182-188
  - 190-200
  - 202-218
  - 220-224
  - 226-234
  - 236-238
  - 251-257
  - 261-285
  - 288-297
  - 299-322
- file: crates/gwiki/src/support/graph.rs
  ranges:
  - 8-55
  - 57-90
  - 92-103
  - 105-107
  - 109-122
  - 124-146
  - 148-154
  - 162-192
  - 195-208
  - 211-236
  - 239-272
- file: crates/gwiki/src/support/postgres.rs
  ranges:
  - 6-39
  - 41-51
- file: crates/gwiki/src/support/scope.rs
  ranges:
  - 12-36
  - 38-42
  - 44-55
  - 60-66
  - 68-76
  - 78-87
  - 89-96
  - 98-104
  - 106-111
  - 114-118
  - 120-131
- file: crates/gwiki/src/support/search.rs
  ranges:
  - 11-13
  - 15-22
  - '24'
  - 26-39
  - 41-43
  - 46-51
  - 53-57
  - 60-161
- file: crates/gwiki/src/support/text.rs
  ranges:
  - 7-13
  - 15-22
  - 24-35
  - 37-49
  - 51-59
  - 61-67
  - 69-71
  - 73-75
  - 77-102
  - 109-114
  - 117-127
- file: crates/gwiki/src/support/time.rs
  ranges:
  - 3-6
  - 8-17
  - 24-39
- file: crates/gwiki/src/synthesis.rs
  ranges:
  - 14-18
  - 20-36
  - 39-43
  - 46-56
  - 59-63
  - 66-70
  - 73-76
  - 80-83
  - 86-89
  - 91-124
  - 126-192
  - 194-232
  - 239-253
  - 255-326
  - 328-356
  - 358-381
  - 383-398
  - 400-405
  - 407-413
  - 415-438
  - 440-454
  - 456-461
  - 463-485
  - 487-497
  - 499-522
  - 524-542
  - 544-558
  - 560-565
  - 567-585
  - 587-611
  - 613-647
  - 649-668
  - 670-680
  - 687-713
  - 716-738
  - 741-746
  - 749-762
  - 765-797
  - 800-828
  - 832-858
  - 861-871
- file: crates/gwiki/src/transcribe.rs
  ranges:
  - 14-18
  - 21-24
  - 27-39
  - 42-45
  - 47-60
  - 62-90
  - 93-98
  - 100-110
  - 113-116
  - 119-122
  - 124-159
  - 161-185
  - 187-214
  - 216-235
  - 237-367
  - 369-375
  - 377-383
  - 392-394
  - 396-427
  - 429-445
  - 448-504
  - 507-566
  - 569-609
- file: crates/gwiki/src/vault.rs
  ranges:
  - 19-22
  - 25-28
  - 55-60
  - 62-99
  - 101-137
  - 140-143
  - 145-151
  - 153-180
  - 182-218
  - 220-230
  - 232-251
  - 258-275
  - 278-303
  - 306-324
  - 327-346
- file: crates/gwiki/src/vector.rs
  ranges:
  - 17-26
  - 29-33
  - 36-40
  - 43-48
  - 50-59
  - '61'
  - 63-67
  - 69-73
  - 75-77
  - 79-99
  - 101-193
  - 195-197
  - 199-205
  - 207-245
  - 247-249
  - 251-253
  - 255-257
  - 260-262
  - 266-282
  - 284-298
  - 301-323
  - 325-330
  - 332-340
  - 342-345
  - 347-354
  - 356-371
  - 373-375
  - 377-381
  - 383-416
  - 425-452
  - 455-522
  - 525-566
  - 570-604
  - 607-617
  - 619-622
  - 624-635
  - 637-640
  - 642-648
  - 651-655
  - 657-660
  - 662-693
  - 695-704
- file: crates/gwiki/src/video.rs
  ranges:
  - 18-21
  - 24-29
  - 32-36
  - 39-43
  - 46-49
  - 52-55
  - 58-61
  - 64-68
  - 70-94
  - 96-103
  - 105-163
  - 165-168
  - 170-178
  - 180-195
  - 197-222
  - 224-280
  - 282-292
  - 294-313
  - 315-326
  - 328-586
  - 588-603
  - 605-612
  - 614-619
  - 621-627
  - 636-663
  - 666-676
  - 679-714
  - 717-752
  - 755-848
  - 851-893
  - 895-911
  - 913-927
- file: crates/gwiki/src/vision.rs
  ranges:
  - 19-23
  - 26-29
  - 31-44
  - 47-52
  - 54-56
  - 59-66
  - 68-72
  - 75-78
  - 80-120
  - 122-234
  - 236-244
  - 246-263
  - 265-283
  - 285-309
  - 311-338
  - 340-359
  - 368-370
  - '372'
  - 374-383
  - 385-392
  - 394-410
  - 413-457
  - 460-488
  - 491-513
  - 516-558
  - 561-592
  - 595-648
  - 651-662
generated_by: gcode-codewiki
trust: generated
freshness: indexed
---

# Architecture Overview

## Subsystems

- [[code/modules/crates/gcode|crates/gcode]] - The crates/gcode subsystem provides a language-aware code indexing, analysis, and retrieval engine that extracts symbol and import facts from multi-language abstract syntax trees, synchronizes them to relational, graph, and vector databases, and exposes a structured command-line interface for hybrid search, dependency traversal, and automated documentation generation.
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:2]
[crates/gcode/assets/import_roots/ruby_require_roots.json:2]
[crates/gcode/build.rs:1-8]
[crates/gcode/contract/gcode.contract.json:2]
[crates/gcode/src/cli.rs:21-44]
- [[code/modules/crates/gcode/assets|crates/gcode/assets]] - This subsystem provides static configuration and JSON asset files mapping dependency and import roots for languages like Elixir and Ruby to enable language-aware code indexing.
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:2]
[crates/gcode/assets/import_roots/ruby_require_roots.json:2]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:3]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:4]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:5]
- [[code/modules/crates/gcode/assets/import_roots|crates/gcode/assets/import_roots]] - This subsystem defines JSON-based configuration files for Elixir dependency and Ruby require roots to map external libraries and API symbols to stable component identifiers within the gcode tool.
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:2]
[crates/gcode/assets/import_roots/ruby_require_roots.json:2]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:3]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:4]
[crates/gcode/assets/import_roots/elixir_dependency_roots.json:5]
- [[code/modules/crates/gcode/contract|crates/gcode/contract]] - The crates/gcode/contract subsystem defines the formal CLI and API contract for the gcode code indexer, detailing its commands, global flags, expected inputs, JSON output structures, and error codes.
[crates/gcode/contract/gcode.contract.json:2]
[crates/gcode/contract/gcode.contract.json:3]
[crates/gcode/contract/gcode.contract.json:4]
[crates/gcode/contract/gcode.contract.json:5-49]
[crates/gcode/contract/gcode.contract.json:7]
- [[code/modules/crates/gcode/src|crates/gcode/src]] - The crates/gcode/src subsystem serves as the core engine of gcode, orchestrating codebase indexing, AST and textual analysis, and dependency graph extraction while managing data synchronization to Postgres, graph, and vector databases to enable high-performance hybrid code search, structural analysis, and documentation generation.
[crates/gcode/src/cli.rs:21-44]
[crates/gcode/src/cli/tests.rs:5-213]
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
- [[code/modules/crates/gcode/src/cli|crates/gcode/src/cli]] - The crates/gcode/src/cli subsystem is responsible for parsing, validating, and routing command-line interface arguments, subcommands, and flags for the gcode codebase indexing, search, and visualization tool.
[crates/gcode/src/cli/tests.rs:5-213]
[crates/gcode/src/cli/tests.rs:216-234]
[crates/gcode/src/cli/tests.rs:237-252]
[crates/gcode/src/cli/tests.rs:255-270]
[crates/gcode/src/cli/tests.rs:273-288]
- [[code/modules/crates/gcode/src/commands|crates/gcode/src/commands]] - The crates/gcode/src/commands subsystem implements the command-line interface execution logic for the gcode tool, orchestrating operations for codebase indexing, code-graph analysis, pattern-matching grep, semantic and symbol-level search, embedding diagnostics, and automated documentation generation via CodeWiki.
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
[crates/gcode/src/commands/codewiki/build_parts/hotspots.rs:5-131]
[crates/gcode/src/commands/codewiki/build_parts/modules.rs:4-136]
- [[code/modules/crates/gcode/src/commands/codewiki|crates/gcode/src/commands/codewiki]] - This subsystem is responsible for analyzing codebase structure, dependencies, contributor history, and complexity to generate and incrementally maintain an AI-assisted CodeWiki documentation system.
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
[crates/gcode/src/commands/codewiki/build_parts/hotspots.rs:5-131]
[crates/gcode/src/commands/codewiki/build_parts/modules.rs:4-136]
- [[code/modules/crates/gcode/src/commands/codewiki/build_parts|crates/gcode/src/commands/codewiki/build_parts]] - This subsystem is responsible for building individual document components for a codebase wiki, including reports and maps for system architecture, changes, individual files, hotspot metrics, modules, developer onboarding, and index snapshots.
[crates/gcode/src/commands/codewiki/build_parts/architecture.rs:5-114]
[crates/gcode/src/commands/codewiki/build_parts/changes.rs:5-101]
[crates/gcode/src/commands/codewiki/build_parts/file.rs:12-15]
[crates/gcode/src/commands/codewiki/build_parts/hotspots.rs:5-131]
[crates/gcode/src/commands/codewiki/build_parts/modules.rs:4-136]
- [[code/modules/crates/gcode/src/commands/graph|crates/gcode/src/commands/graph]] - The crates/gcode/src/commands/graph subsystem manages the lifecycle of the code graph index, including synchronizing, clearing, and rebuilding project data, and exposes query interfaces to resolve symbol relationships like callers, usages, and imports.
[crates/gcode/src/commands/graph/lifecycle.rs:11-13]
[crates/gcode/src/commands/graph/payload.rs:6-37]
[crates/gcode/src/commands/graph/reads.rs:14-20]
[crates/gcode/src/commands/graph/tests.rs:16-30]
[crates/gcode/src/commands/graph/lifecycle.rs:15-53]
- [[code/modules/crates/gcode/src/commands/grep|crates/gcode/src/commands/grep]] - The crates/gcode/src/commands/grep subsystem is responsible for executing pattern matching and regular expression searches within code, incorporating precise word and identifier boundary validation to locate matching text spans.
[crates/gcode/src/commands/grep/grep_matcher.rs:6-9]
[crates/gcode/src/commands/grep/grep_matcher.rs:11-44]
[crates/gcode/src/commands/grep/grep_matcher.rs:12-31]
[crates/gcode/src/commands/grep/grep_matcher.rs:33-43]
[crates/gcode/src/commands/grep/grep_matcher.rs:46-65]
- [[code/modules/crates/gcode/src/commands/snapshots|crates/gcode/src/commands/snapshots]] - This subsystem maintains test assertion snapshots for verifying the outcomes, unsupported file types, and projection synchronization behavior of the code indexing commands. 
- [[code/modules/crates/gcode/src/config|crates/gcode/src/config]] - The crates/gcode/src/config subsystem manages the detection and resolution of project roots and identities while parsing and validating configuration settings for backend services, databases, vector search parameters, and embedding providers.
[crates/gcode/src/config/context.rs:26-31]
[crates/gcode/src/config/services.rs:20-22]
[crates/gcode/src/config/tests.rs:14-22]
[crates/gcode/src/config/context.rs:34]
[crates/gcode/src/config/context.rs:37]
- [[code/modules/crates/gcode/src/db|crates/gcode/src/db]] - This subsystem is responsible for dynamically resolving PostgreSQL database connection URLs via environment variables, local configurations, and authentication brokers, establishing read-only or read-write connections, and querying codebase synchronization facts, imports, symbols, and calls.
[crates/gcode/src/db/mod.rs:16-20]
[crates/gcode/src/db/queries.rs:8-13]
[crates/gcode/src/db/resolution.rs:16-18]
[crates/gcode/src/db/mod.rs:27-31]
[crates/gcode/src/db/mod.rs:33-35]
- [[code/modules/crates/gcode/src/dispatch|crates/gcode/src/dispatch]] - The dispatch subsystem is responsible for early request routing, configuring command logging levels, and mapping commands to their minimum required service dependencies during CLI execution.
[crates/gcode/src/dispatch/tests.rs:5-9]
[crates/gcode/src/dispatch/tests.rs:12-14]
[crates/gcode/src/dispatch/tests.rs:17-22]
[crates/gcode/src/dispatch/tests.rs:25-27]
[crates/gcode/src/dispatch/tests.rs:30-70]
- [[code/modules/crates/gcode/src/graph|crates/gcode/src/graph]] - The graph subsystem manages the code-index graph database layer, facilitating secure Cypher query construction and execution to track code relationships while generating structured, Markdown-rendered architectural analysis reports.
[crates/gcode/src/graph/code_graph/connection.rs:7-12]
[crates/gcode/src/graph/code_graph/lifecycle.rs:18-21]
[crates/gcode/src/graph/code_graph/payload.rs:10-19]
[crates/gcode/src/graph/code_graph/read.rs:45-90]
[crates/gcode/src/graph/code_graph/tests.rs:7-21]
- [[code/modules/crates/gcode/src/graph/code_graph|crates/gcode/src/graph/code_graph]] - The code_graph subsystem manages the lifecycle, querying, and mutation of a graph database index that tracks code relationships—including file imports, symbol definitions, and function calls—to facilitate call graph analysis and blast radius calculations.
[crates/gcode/src/graph/code_graph/connection.rs:7-12]
[crates/gcode/src/graph/code_graph/lifecycle.rs:18-21]
[crates/gcode/src/graph/code_graph/payload.rs:10-19]
[crates/gcode/src/graph/code_graph/read.rs:45-90]
[crates/gcode/src/graph/code_graph/tests.rs:7-21]
- [[code/modules/crates/gcode/src/graph/report|crates/gcode/src/graph/report]] - The graph/report subsystem is responsible for querying, loading, summarizing, and rendering code graph analytics—such as hotspots, dependency frequencies, and bridge edges—into formatted Markdown reports from graph snapshots.
[crates/gcode/src/graph/report/generation.rs:21-23]
[crates/gcode/src/graph/report/loading.rs:18-78]
[crates/gcode/src/graph/report/queries.rs:7-18]
[crates/gcode/src/graph/report/render.rs:8-18]
[crates/gcode/src/graph/report/rows.rs:11-19]
- [[code/modules/crates/gcode/src/index|crates/gcode/src/index]] - The subsystem discovers, filters, and indexes source files across multiple programming languages to parse syntactic facts—including symbols, import bindings, call relations, and content chunks—and persist them into a structural code index.
[crates/gcode/src/index/api.rs:16-23]
[crates/gcode/src/index/chunker.rs:19-62]
[crates/gcode/src/index/hasher.rs:7-9]
[crates/gcode/src/index/import_resolution/context.rs:19-37]
[crates/gcode/src/index/import_resolution/helpers.rs:1-3]
- [[code/modules/crates/gcode/src/index/import_resolution|crates/gcode/src/index/import_resolution]] - The import_resolution subsystem parses and analyzes import statements across multiple programming languages to resolve local and external dependencies and construct a unified module resolution context.
[crates/gcode/src/index/import_resolution/context.rs:19-37]
[crates/gcode/src/index/import_resolution/helpers.rs:1-3]
[crates/gcode/src/index/import_resolution/parser/go_rust.rs:12-40]
[crates/gcode/src/index/import_resolution/parser/java_csharp.rs:8-60]
[crates/gcode/src/index/import_resolution/parser/mod.rs:29-54]
- [[code/modules/crates/gcode/src/index/import_resolution/parser|crates/gcode/src/index/import_resolution/parser]] - This subsystem parses language-specific import and module reference statements across a wide variety of programming languages to extract and normalize raw dependency bindings for unified index resolution.
[crates/gcode/src/index/import_resolution/parser/go_rust.rs:12-40]
[crates/gcode/src/index/import_resolution/parser/java_csharp.rs:8-60]
[crates/gcode/src/index/import_resolution/parser/mod.rs:29-54]
[crates/gcode/src/index/import_resolution/parser/php_kotlin.rs:7-14]
[crates/gcode/src/index/import_resolution/parser/python_js.rs:11-98]
- [[code/modules/crates/gcode/src/index/indexer|crates/gcode/src/index/indexer]] - The `crates/gcode/src/index/indexer` subsystem orchestrates the codebase indexing lifecycle, including file freshness tracking, git-aware overlay reconciliation, and the extraction and persistence of semantic code facts to a database sink.
[crates/gcode/src/index/indexer/file.rs:15-91]
[crates/gcode/src/index/indexer/freshness_probe.rs:37-81]
[crates/gcode/src/index/indexer/lifecycle.rs:16-54]
[crates/gcode/src/index/indexer/overlay.rs:32-35]
[crates/gcode/src/index/indexer/pipeline.rs:27-30]
- [[code/modules/crates/gcode/src/index/parser|crates/gcode/src/index/parser]] - This subsystem is responsible for parsing and extracting function and method call sites during code indexing through both AST-based analysis and textual scanning.
[crates/gcode/src/index/parser/calls.rs:23-32]
[crates/gcode/src/index/parser/calls/ast.rs:17-96]
[crates/gcode/src/index/parser/calls/dart_textual.rs:8-55]
[crates/gcode/src/index/parser/calls/resolution.rs:6-10]
[crates/gcode/src/index/parser/calls/shadowing.rs:6-23]
- [[code/modules/crates/gcode/src/index/parser/calls|crates/gcode/src/index/parser/calls]] - This subsystem is responsible for extracting, parsing, and resolving function and method calls from source code using both AST-based and textual scanning techniques while managing identifier semantics, variable shadowing, and lexical scope boundaries.
[crates/gcode/src/index/parser/calls/ast.rs:17-96]
[crates/gcode/src/index/parser/calls/dart_textual.rs:8-55]
[crates/gcode/src/index/parser/calls/resolution.rs:6-10]
[crates/gcode/src/index/parser/calls/shadowing.rs:6-23]
[crates/gcode/src/index/parser/calls/text.rs:4-20]
- [[code/modules/crates/gcode/src/projection|crates/gcode/src/projection]] - The projection subsystem is responsible for synchronizing graph and vector projections with codebase index changes, tracking sync states, and reporting progress or errors during the projection lifecycle.
[crates/gcode/src/projection/sync.rs:11-14]
[crates/gcode/src/projection/sync.rs:17-21]
[crates/gcode/src/projection/sync.rs:24-29]
[crates/gcode/src/projection/sync.rs:33-37]
[crates/gcode/src/projection/sync.rs:40-43]
- [[code/modules/crates/gcode/src/search|crates/gcode/src/search]] - This subsystem provides comprehensive symbol and code search capabilities by combining Postgres-backed full-text search, graph-based symbol boosting and expansion, and reciprocal rank fusion to merge and rank results.
[crates/gcode/src/search/fts/common.rs:16]
[crates/gcode/src/search/fts/content.rs:13-21]
[crates/gcode/src/search/fts/counts.rs:10-66]
[crates/gcode/src/search/fts/graph.rs:16-50]
[crates/gcode/src/search/fts/symbols.rs:15-18]
- [[code/modules/crates/gcode/src/search/fts|crates/gcode/src/search/fts]] - This subsystem provides database-driven full-text search, counting, snippet generation, and symbol resolution capabilities for code content and symbols while respecting project and overlay visibility constraints.
[crates/gcode/src/search/fts/common.rs:16]
[crates/gcode/src/search/fts/content.rs:13-21]
[crates/gcode/src/search/fts/counts.rs:10-66]
[crates/gcode/src/search/fts/graph.rs:16-50]
[crates/gcode/src/search/fts/symbols.rs:15-18]
- [[code/modules/crates/gcode/src/setup|crates/gcode/src/setup]] - The crates/gcode/src/setup subsystem is responsible for defining database schema contracts, managing DDL execution, and initializing, validating, or resetting Postgres database structures for the standalone code index.
[crates/gcode/src/setup/contracts.rs:5-8]
[crates/gcode/src/setup/ddl.rs:8-10]
[crates/gcode/src/setup/identifiers.rs:5-15]
[crates/gcode/src/setup/postgres.rs:12-57]
[crates/gcode/src/setup/tests.rs:12-55]
- [[code/modules/crates/gcode/src/vector|crates/gcode/src/vector]] - This subsystem is responsible for generating text embeddings, managing the lifecycle and syncing of code symbol vector collections in Qdrant, and executing semantic searches over those vectors.
[crates/gcode/src/vector/code_symbols/embedding.rs:21-23]
[crates/gcode/src/vector/code_symbols/lifecycle.rs:29-37]
[crates/gcode/src/vector/code_symbols/qdrant.rs:18-24]
[crates/gcode/src/vector/code_symbols/repository.rs:6-18]
[crates/gcode/src/vector/code_symbols/search.rs:8-14]
- [[code/modules/crates/gcode/src/vector/code_symbols|crates/gcode/src/vector/code_symbols]] - This subsystem manages the generation of vector embeddings, collection lifecycles within the Qdrant vector database, and semantic search operations for project code symbols.
[crates/gcode/src/vector/code_symbols/embedding.rs:21-23]
[crates/gcode/src/vector/code_symbols/lifecycle.rs:29-37]
[crates/gcode/src/vector/code_symbols/qdrant.rs:18-24]
[crates/gcode/src/vector/code_symbols/repository.rs:6-18]
[crates/gcode/src/vector/code_symbols/search.rs:8-14]
- [[code/modules/crates/gcore|crates/gcore]] - The crates/gcore subsystem serves as Gobby's foundational backend library, responsible for provisioning containerized backing services—including FalkorDB, Qdrant, and PostgreSQL—while orchestrating core utilities for multi-provider AI routing, layered configuration resolution, secure secret management, and database integrations.
[crates/gcore/assets/docker-compose.services.yml:5-117]
[crates/gcore/assets/postgres-pgsearch/version.json:2]
[crates/gcore/src/ai/daemon.rs:19-24]
[crates/gcore/src/ai/embeddings.rs:19-38]
[crates/gcore/src/ai/mod.rs:31-35]
- [[code/modules/crates/gcore/assets|crates/gcore/assets]] - The crates/gcore/assets subsystem manages the project's backend container infrastructure, configuring database services including FalkorDB, Qdrant, and a custom PostgreSQL image with full-text search and audit logging support.
[crates/gcore/assets/docker-compose.services.yml:5-117]
[crates/gcore/assets/postgres-pgsearch/version.json:2]
[crates/gcore/assets/docker-compose.services.yml:6-28]
[crates/gcore/assets/docker-compose.services.yml:7]
[crates/gcore/assets/docker-compose.services.yml:8-10]
- [[code/modules/crates/gcore/assets/postgres-pgsearch|crates/gcore/assets/postgres-pgsearch]] - This subsystem provides the Docker container configuration, database initialization scripts, and multi-architecture build metadata required to deploy a PostgreSQL database preconfigured with the pg_search extension.
[crates/gcore/assets/postgres-pgsearch/version.json:2]
[crates/gcore/assets/postgres-pgsearch/version.json:3]
[crates/gcore/assets/postgres-pgsearch/version.json:4-7]
[crates/gcore/assets/postgres-pgsearch/version.json:5]
[crates/gcore/assets/postgres-pgsearch/version.json:6]
- [[code/modules/crates/gcore/assets/postgres-pgsearch/initdb.d|crates/gcore/assets/postgres-pgsearch/initdb.d]] - This subsystem provides the SQL initialization scripts to automatically configure and enable the pg_search and pgaudit extensions during PostgreSQL database startup. 
- [[code/modules/crates/gcore/assets/postgres-pgsearch/scripts|crates/gcore/assets/postgres-pgsearch/scripts]] - This subsystem provides shell utility scripts to export PostgreSQL audit logs for audit tracking and analysis. 
- [[code/modules/crates/gcore/src|crates/gcore/src]] - The crates/gcore/src subsystem provides the core engine of the application by orchestrating layered configuration resolution, secret decryption, and database hub provisioning (PostgreSQL, Qdrant, and FalkorDB), while exposing unified transport abstractions for daemon-backed AI modalities, graph analytics, and vector search indexing.
[crates/gcore/src/ai/daemon.rs:19-24]
[crates/gcore/src/ai/embeddings.rs:19-38]
[crates/gcore/src/ai/mod.rs:31-35]
[crates/gcore/src/ai/probe.rs:20-23]
[crates/gcore/src/ai/text.rs:9-15]
- [[code/modules/crates/gcore/src/ai|crates/gcore/src/ai]] - The crates/gcore/src/ai subsystem coordinates communication with local or remote AI daemons to execute text generation, embedding creation, audio transcription, and computer vision tasks, managing capability probing, transport routing, and resilient retry logic.
[crates/gcore/src/ai/daemon.rs:19-24]
[crates/gcore/src/ai/embeddings.rs:19-38]
[crates/gcore/src/ai/mod.rs:31-35]
[crates/gcore/src/ai/probe.rs:20-23]
[crates/gcore/src/ai/text.rs:9-15]
- [[code/modules/crates/gcore/src/config|crates/gcore/src/config]] - This subsystem defines core configuration schemas and provides layered resolution logic to retrieve, merge, and decode settings—such as database, indexing, and AI capability bindings—from environment variables, YAML files, and secure configuration stores.
[crates/gcore/src/config/resolve.rs:11-21]
[crates/gcore/src/config/tests.rs:9-11]
[crates/gcore/src/config/types.rs:5-9]
[crates/gcore/src/config/resolve.rs:24-75]
[crates/gcore/src/config/resolve.rs:78-84]
- [[code/modules/crates/gcore/src/provisioning|crates/gcore/src/provisioning]] - The provisioning subsystem manages the bootstrapping, YAML-based configuration, and deployment of Docker services like databases and vector stores to ensure a fully resolved and healthy runtime hub environment.
[crates/gcore/src/provisioning/bootstrap.rs:8-15]
[crates/gcore/src/provisioning/docker.rs:9-18]
[crates/gcore/src/provisioning/hub.rs:4-9]
[crates/gcore/src/provisioning/mod.rs:53-55]
[crates/gcore/src/provisioning/tests.rs:5-7]
- [[code/modules/crates/gcore/src/qdrant|crates/gcore/src/qdrant]] - The Qdrant subsystem manages vector database integration by handling collection naming and validation, search orchestration, batch point upserts, and database lifecycle operations.
[crates/gcore/src/qdrant/naming.rs:3-10]
[crates/gcore/src/qdrant/tests.rs:12-30]
[crates/gcore/src/qdrant/naming.rs:13-22]
[crates/gcore/src/qdrant/naming.rs:25-43]
[crates/gcore/src/qdrant/naming.rs:45-70]
- [[code/modules/crates/ghook|crates/ghook]] - The crates/ghook subsystem implements a resilient Rust-based sidecar utility and defines versioned JSON Schema contracts to intercept, validate, queue, and dispatch CLI tool-use and session-lifecycle hooks to the central Gobby daemon.
[crates/ghook/schemas/diagnose-output.v2.schema.json:2]
[crates/ghook/schemas/inbox-envelope.v1.schema.json:2]
[crates/ghook/src/cli_config.rs:11-18]
[crates/ghook/src/detach.rs:23-43]
[crates/ghook/src/diagnose.rs:15-32]
- [[code/modules/crates/ghook/schemas|crates/ghook/schemas]] - The crates/ghook/schemas subsystem defines JSON schema specifications for validating hook event envelopes and diagnosis output payloads, ensuring structured and type-safe communications between the command-line interface, Git/terminal hooks, and the Gobby daemon.
[crates/ghook/schemas/diagnose-output.v2.schema.json:2]
[crates/ghook/schemas/inbox-envelope.v1.schema.json:2]
[crates/ghook/schemas/diagnose-output.v2.schema.json:3]
[crates/ghook/schemas/diagnose-output.v2.schema.json:4]
[crates/ghook/schemas/diagnose-output.v2.schema.json:5]
- [[code/modules/crates/ghook/src|crates/ghook/src]] - The crates/ghook/src subsystem is a Rust-based Git and CLI hook utility responsible for capturing terminal and execution context, packaging it into validated JSON envelopes, and dispatching these payloads to the Gobby daemon to enforce real-time hook-based execution controls.
[crates/ghook/src/cli_config.rs:11-18]
[crates/ghook/src/detach.rs:23-43]
[crates/ghook/src/diagnose.rs:15-32]
[crates/ghook/src/envelope.rs:24-32]
[crates/ghook/src/json_value.rs:3-20]
- [[code/modules/crates/gloc|crates/gloc]] - The gloc subsystem provides a command-line utility and the core infrastructure required to configure, resolve, validate, and execute local and remote AI models and client environments.
[crates/gloc/config.yaml:11-17]
[crates/gloc/src/backend.rs:7-12]
[crates/gloc/src/config.rs:13-22]
[crates/gloc/src/exec.rs:9-21]
[crates/gloc/src/main.rs:16-52]
- [[code/modules/crates/gloc/src|crates/gloc/src]] - The gloc subsystem provides a command-line interface and configuration engine to manage, probe, and orchestrate the execution of local and remote AI client binaries and backends.
[crates/gloc/src/backend.rs:7-12]
[crates/gloc/src/config.rs:13-22]
[crates/gloc/src/exec.rs:9-21]
[crates/gloc/src/main.rs:16-52]
[crates/gloc/src/backend.rs:14-23]
- [[code/modules/crates/gsqz|crates/gsqz]] - The crates/gsqz subsystem is responsible for optimizing and compressing terminal command outputs through a configurable text-transformation pipeline to minimize token usage in LLM-assisted workflows.
[crates/gsqz/config.yaml:12-15]
[crates/gsqz/src/command_split.rs:5-85]
[crates/gsqz/src/compressor.rs:7-12]
[crates/gsqz/src/config.rs:26-35]
[crates/gsqz/src/daemon.rs:11-23]
- [[code/modules/crates/gsqz/src|crates/gsqz/src]] - The crates/gsqz/src subsystem implements a configurable text-compression utility and daemon client (gsqz) that filters, transforms, and optimizes command and terminal outputs using custom rule-based pipelines and granular text-processing primitives.
[crates/gsqz/src/command_split.rs:5-85]
[crates/gsqz/src/compressor.rs:7-12]
[crates/gsqz/src/config.rs:26-35]
[crates/gsqz/src/daemon.rs:11-23]
[crates/gsqz/src/main.rs:25-48]
- [[code/modules/crates/gsqz/src/primitives|crates/gsqz/src/primitives]] - This subsystem provides core text-manipulation primitives for deduplicating, filtering, grouping, matching, replacing, truncating, and compressing text and log outputs.
[crates/gsqz/src/primitives/dedup.rs:9-45]
[crates/gsqz/src/primitives/filter.rs:4-15]
[crates/gsqz/src/primitives/group.rs:8-21]
[crates/gsqz/src/primitives/match_output.rs:8-33]
[crates/gsqz/src/primitives/prose.rs:5-9]
- [[code/modules/crates/gwiki|crates/gwiki]] - The crates/gwiki subsystem implements the core Rust-based research and knowledge engine, providing CLI contracts, markdown-based skill assets, and an autonomous research loop that integrates multi-format asset ingestion, vector and graph-boosted search, and note compilation into structured, fully cited wikis.
[crates/gwiki/contract/gwiki.contract.json:2]
[crates/gwiki/src/ai/chunk.rs:24-30]
[crates/gwiki/src/ai/clients.rs:20-23]
[crates/gwiki/src/ai/translate.rs:6-29]
[crates/gwiki/src/api.rs:11-121]
- [[code/modules/crates/gwiki/assets/skills|crates/gwiki/assets/skills]] - This subsystem manages the markdown assets defining the agentic skills for auditing, compiling, querying, and researching within the repository. 
- [[code/modules/crates/gwiki/contract|crates/gwiki/contract]] - The crates/gwiki/contract subsystem defines the API contract, commands, parameters, and metadata schemas for the gwiki tool integration.
[crates/gwiki/contract/gwiki.contract.json:2]
[crates/gwiki/contract/gwiki.contract.json:3]
[crates/gwiki/contract/gwiki.contract.json:4]
[crates/gwiki/contract/gwiki.contract.json:5-25]
[crates/gwiki/contract/gwiki.contract.json:7]
- [[code/modules/crates/gwiki/src|crates/gwiki/src]] - The crates/gwiki/src subsystem serves as the core engine of the GWiki research platform, orchestrating multi-source document ingestion, AI-driven research and synthesis, semantic and graph-boosted search, and the compilation of raw research notes into a structured and audited wiki.
[crates/gwiki/src/ai/chunk.rs:24-30]
[crates/gwiki/src/ai/clients.rs:20-23]
[crates/gwiki/src/ai/translate.rs:6-29]
[crates/gwiki/src/api.rs:11-121]
[crates/gwiki/src/audit.rs:33-35]
- [[code/modules/crates/gwiki/src/ai|crates/gwiki/src/ai]] - This subsystem provides AI-powered services for audio chunking, media transcription, segment-wise language translation, and vision extraction through a unified client interface supporting both production and testing environments.
[crates/gwiki/src/ai/chunk.rs:24-30]
[crates/gwiki/src/ai/clients.rs:20-23]
[crates/gwiki/src/ai/translate.rs:6-29]
[crates/gwiki/src/ai/chunk.rs:33-35]
[crates/gwiki/src/ai/chunk.rs:38-47]
- [[code/modules/crates/gwiki/src/commands|crates/gwiki/src/commands]] - The crates/gwiki/src/commands subsystem defines the executable command-line interfaces for the gwiki knowledge base, orchestrating core user actions such as vault initialization, source ingestion, indexing, semantic graph search, and trust and quality auditing.
[crates/gwiki/src/commands/ask.rs:25-46]
[crates/gwiki/src/commands/audit.rs:3-13]
[crates/gwiki/src/commands/backlinks.rs:10-18]
[crates/gwiki/src/commands/benchmark.rs:11-44]
[crates/gwiki/src/commands/citation_quality.rs:26-33]
- [[code/modules/crates/gwiki/src/commands/citation_quality|crates/gwiki/src/commands/citation_quality]] - The citation quality subsystem evaluates the integrity of references by identifying, analyzing, and reporting contradictions between section content and cited source claims.
[crates/gwiki/src/commands/citation_quality/contradictions.rs:15-18]
[crates/gwiki/src/commands/citation_quality/contradictions.rs:21-24]
[crates/gwiki/src/commands/citation_quality/contradictions.rs:27-29]
[crates/gwiki/src/commands/citation_quality/contradictions.rs:31-67]
[crates/gwiki/src/commands/citation_quality/contradictions.rs:69-117]
- [[code/modules/crates/gwiki/src/commands/refresh|crates/gwiki/src/commands/refresh]] - The crates/gwiki/src/commands/refresh subsystem is responsible for identifying, planning, and executing updates to local and URL-based wiki sources by fetching remote content, replaying local files, validating changes, and safely managing cached raw data and assets within the local vault.
[crates/gwiki/src/commands/refresh/candidate.rs:15-74]
[crates/gwiki/src/commands/refresh/mod.rs:29-37]
[crates/gwiki/src/commands/refresh/model.rs:5-9]
[crates/gwiki/src/commands/refresh/render.rs:3-49]
[crates/gwiki/src/commands/refresh/selection.rs:4-75]
- [[code/modules/crates/gwiki/src/compile|crates/gwiki/src/compile]] - The compile subsystem collects and validates source notes, parses their sections, renders them into Obsidian-compatible Markdown page targets, and updates the central wiki index with compiled page links.
[crates/gwiki/src/compile/collect.rs:10-82]
[crates/gwiki/src/compile/index.rs:16-63]
[crates/gwiki/src/compile/mod.rs:27-32]
[crates/gwiki/src/compile/render.rs:11-47]
[crates/gwiki/src/compile/tests.rs:6-24]
- [[code/modules/crates/gwiki/src/graph|crates/gwiki/src/graph]] - This subsystem is responsible for constructing, querying, and analyzing an in-memory knowledge graph that maps wiki documents, code references, and citations, while supporting backlink generation, contextual warning audits, graph analytics, and exporting structured graph reports.
[crates/gwiki/src/graph/analytics.rs:14-22]
[crates/gwiki/src/graph/context.rs:8-11]
[crates/gwiki/src/graph/export.rs:11-112]
[crates/gwiki/src/graph/mod.rs:22-26]
[crates/gwiki/src/graph/analytics.rs:24-39]
- [[code/modules/crates/gwiki/src/ingest|crates/gwiki/src/ingest]] - The gwiki ingest subsystem manages the retrieval, parsing, and extraction of text, metadata, and media assets from diverse sources—including web URLs, repositories, documents, PDFs, audio, and video—into structured markdown and indexed wiki components.
[crates/gwiki/src/ingest/audio.rs:21-28]
[crates/gwiki/src/ingest/document/html.rs:8-39]
[crates/gwiki/src/ingest/document/mod.rs:21-27]
[crates/gwiki/src/ingest/document/office.rs:39-52]
[crates/gwiki/src/ingest/document/render.rs:11-33]
- [[code/modules/crates/gwiki/src/ingest/document|crates/gwiki/src/ingest/document]] - This subsystem is responsible for extracting content and metadata from HTML and Microsoft Office files, converting them into normalized markdown, and managing document ingestion and format degradation.
[crates/gwiki/src/ingest/document/html.rs:8-39]
[crates/gwiki/src/ingest/document/mod.rs:21-27]
[crates/gwiki/src/ingest/document/office.rs:39-52]
[crates/gwiki/src/ingest/document/render.rs:11-33]
[crates/gwiki/src/ingest/document/tests.rs:9-18]
- [[code/modules/crates/gwiki/src/ingest/pdf|crates/gwiki/src/ingest/pdf]] - The crates/gwiki/src/ingest/pdf subsystem is responsible for ingesting PDF documents into GWiki by extracting text layers, rendering pages to images for vision-assisted OCR, and sanitizing and formatting the resulting content into structured Markdown.
[crates/gwiki/src/ingest/pdf/ingest.rs:23-37]
[crates/gwiki/src/ingest/pdf/markdown.rs:15-89]
[crates/gwiki/src/ingest/pdf/mod.rs:22-25]
[crates/gwiki/src/ingest/pdf/render.rs:23-39]
[crates/gwiki/src/ingest/pdf/tests.rs:21]
- [[code/modules/crates/gwiki/src/ingest/video|crates/gwiki/src/ingest/video]] - This subsystem is responsible for ingesting video files by extracting metadata, transcribing audio, sampling individual frames, and analyzing frame content through vision-based descriptions.
[crates/gwiki/src/ingest/video/assets.rs:4-23]
[crates/gwiki/src/ingest/video/metadata.rs:4-8]
[crates/gwiki/src/ingest/video/mod.rs:32-45]
[crates/gwiki/src/ingest/video/processing.rs:18-26]
[crates/gwiki/src/ingest/video/tests.rs:18-55]
- [[code/modules/crates/gwiki/src/research|crates/gwiki/src/research]] - The research subsystem manages the AI-driven research lifecycle by executing information-gathering loops, resolving search and retrieval commands, and validating and atomically persisting structured research outcomes and notes.
[crates/gwiki/src/research/mod.rs:41]
[crates/gwiki/src/research/model.rs:21-24]
[crates/gwiki/src/research/notes.rs:5-16]
[crates/gwiki/src/research/outcome.rs:15-24]
[crates/gwiki/src/research/storage.rs:12-59]
- [[code/modules/crates/gwiki/src/research_loop|crates/gwiki/src/research_loop]] - The research_loop subsystem coordinates an iterative, budget-bounded agent execution engine that searches, reads, and ingests sources to validate code citations and compile verified research notes.
[crates/gwiki/src/research_loop/engine.rs:19-28]
[crates/gwiki/src/research_loop/helpers.rs:8-19]
[crates/gwiki/src/research_loop/tests.rs:12-14]
[crates/gwiki/src/research_loop/types.rs:11-17]
[crates/gwiki/src/research_loop/engine.rs:31-46]
- [[code/modules/crates/gwiki/src/search|crates/gwiki/src/search]] - This subsystem provides multi-modal wiki search functionality by executing keyword, semantic, and graph-boosted queries across diverse backends and merging their results using Reciprocal Rank Fusion.
[crates/gwiki/src/search/bm25.rs:13-17]
[crates/gwiki/src/search/graph_boost.rs:21-24]
[crates/gwiki/src/search/mod.rs:14-18]
[crates/gwiki/src/search/rrf.rs:8-92]
[crates/gwiki/src/search/semantic.rs:18-22]
- [[code/modules/crates/gwiki/src/sources|crates/gwiki/src/sources]] - The crates/gwiki/src/sources subsystem manages the definition, registration, and locking of source manifests, ensures their safe persistence via atomic file operations, and renders their metadata into Markdown-formatted indexes.
[crates/gwiki/src/sources/atomic.rs:7-44]
[crates/gwiki/src/sources/manifest.rs:23-25]
[crates/gwiki/src/sources/render.rs:15-45]
[crates/gwiki/src/sources/tests.rs:8-50]
[crates/gwiki/src/sources/types.rs:12-29]
- [[code/modules/crates/gwiki/src/support|crates/gwiki/src/support]] - The crates/gwiki/src/support subsystem provides shared utility modules and infrastructure for configuration loading, environment parsing, database connectivity, search scoping, text processing, and graph resolution within the GWiki application.
[crates/gwiki/src/support/config.rs:18-20]
[crates/gwiki/src/support/counts.rs:4-10]
[crates/gwiki/src/support/env.rs:21-24]
[crates/gwiki/src/support/graph.rs:8-55]
[crates/gwiki/src/support/postgres.rs:6-39]
- [[code/modules/docs/contracts|docs/contracts]] - The docs/contracts subsystem documents the specifications, command-line contracts, and shared graph schemas that define the interfaces and protocol agreements for the gcode and gwiki integrations. 
- [[code/modules/docs/guides|docs/guides]] - The docs/guides subsystem serves as the central documentation repository, providing comprehensive developer and user guides, system integration contracts, and release runbooks for Gobby's specialized sub-tools and AI-daemon configurations. 
- [[code/modules/docs/plans|docs/plans]] - The docs/plans subsystem maintains release procedures and authoritative design documentation detailing core ecosystem architectural improvements, schema contracts, and CLI integration strategies. 
- [[code/modules/docs/plans/completed|docs/plans/completed]] - The docs/plans/completed subsystem archives the planning, design, and specification documents for successfully completed features and architectural enhancements. 
- [[code/modules/docs/spikes|docs/spikes]] - The docs/spikes subsystem provides documentation for technical spikes and exploratory research analyzing how external process and function calls are invoked across various programming languages. 
- [[code/modules/scripts|scripts]] - The scripts subsystem contains utility shell scripts used to verify and validate codebase correctness and project health. 

