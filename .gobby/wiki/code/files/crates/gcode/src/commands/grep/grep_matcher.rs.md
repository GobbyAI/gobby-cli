...
Defines `GrepMatcher`, a small grep helper that compiles a pattern into a `regex::Regex` with optional fixed-string escaping, case-insensitive matching, and a word-boundary mode. `find_spans` scans a line for non-empty regex matches and returns their byte spans, filtering them through ASCII identifier-boundary checks when `word` matching is enabled. The private helpers compute whether a match is isolated from adjacent identifier characters, and the tests verify whole-word behavior, Unicode handling, regex `\b` support, and error reporting for invalid or empty patterns.
