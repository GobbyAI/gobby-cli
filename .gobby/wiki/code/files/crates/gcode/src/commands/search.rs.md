...
Implements the `gcode` search command for code symbols and content, combining exact matching, full-text BM25 search, semantic vector results, and optional symbol-graph expansion into a single ranked result set. The file also centralizes path/kind/language visibility filtering, reciprocal-rank and exact-match scoring, result formatting, pagination and warning hints, and helper logic for detecting literal-like queries that should be redirected to grep-style or content search.
