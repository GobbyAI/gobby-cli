...
- `describe_image` (function) component `describe_image [function]` (`da280306-74aa-54d8-a56a-bc9f19ff9a9d`) lines 19-35 [crates/gcore/src/ai/vision.rs:19-35]
  ...
  - Purpose: Submits image bytes to an AI vision extraction API endpoint via HTTP POST and returns the parsed response content with model metadata as a VisionResult. [crates/gcore/src/ai/vision.rs:19-35]
- `request_body` (function) component `request_body [function]` (`8573a93a-a983-5869-8ee6-0e70c43302b7`) lines 37-63 [crates/gcore/src/ai/vision.rs:37-63]
  ...
  - Purpose: Encodes image bytes as a base64 data URI and embeds it within a structured JSON request body containing a vision prompt and optional model specification. [crates/gcore/src/ai/vision.rs:37-63]
- `parse_content` (function) component `parse_content [function]` (`7670963e-2e4b-52fa-af31-078c4f7320bb`) lines 65-90 [crates/gcore/src/ai/vision.rs:65-90]
  ...
  - Purpose: Parses content sequentially as JSON or delimited format, returning a `VisionResult` with extracted description and OCR text, or falling back to the trimmed input as description if both formats fail. [crates/gcore/src/ai/vision.rs:65-90]
- `parse_json_content` (function) component `parse_json_content [function]` (`7e24670a-7ed6-5793-a947-7b97283d512e`) lines 92-104 [crates/gcore/src/ai/vision.rs:92-104]
  ...
  - Purpose: Deserializes JSON-formatted content into a struct and returns a tuple of the trimmed description field paired with optionally cleaned OCR text, or None if deserialization fails. [crates/gcore/src/ai/vision.rs:92-104]
- `VisionContent` (class) component `VisionContent [class]` (`bb746a09-7b5c-584a-bd28-525ac6a598e4`) lines 94-98 [crates/gcore/src/ai/vision.rs:94-98]
  ...
  - Purpose: A struct that encapsulates optional `description` and `ocr_text` string fields with serde aliases for flexible deserialization of vision-extracted text content. [crates/gcore/src/ai/vision.rs:94-98]
- `strip_json_fence` (function) component `strip_json_fence [function]` (`35c80297-49e7-5e66-9f40-a5cfd322b377`) lines 106-121 [crates/gcore/src/ai/vision.rs:106-121]
  ...
  - Purpose: Removes markdown code fence delimiters (triple backticks) and optional 'json' language specifier to extract the enclosed content, returning the original string if no fences are present. [crates/gcore/src/ai/vision.rs:106-121]
- `parse_delimited_content` (function) component `parse_delimited_content [function]` (`eaf22cda-d802-5c87-8320-da8bf0a3e9bd`) lines 123-156 [crates/gcore/src/ai/vision.rs:123-156]
  ...
  - Purpose: Parses section-delimited content to extract a required description and optional OCR text, returning `None` if the description is empty. [crates/gcore/src/ai/vision.rs:123-156]
- `parse_section_label` (function) component `parse_section_label [function]` (`187d6eec-5ed7-5079-8f91-59dca52e6761`) lines 158-173 [crates/gcore/src/ai/vision.rs:158-173]
  ...
  - Purpose: Parses a colon-delimited line by normalizing the label portion (trimming, removing leading '#', lowercasing, replacing spaces with underscores) and returns the corresponding `Section` enum variant paired with the remainder content if the label matches 'description' or 'ocr'. [crates/gcore/src/ai/vision.rs:158-173]
- `clean_optional_text` (function) component `clean_optional_text [function]` (`98af5984-bc13-50fc-8075-266e6169d90a`) lines 175-179 [crates/gcore/src/ai/vision.rs:175-179]
  ...
  - Purpose: This function trims whitespace from an optional string and returns `None` if the result is empty or case-insensitively equals "null", otherwise returning the trimmed string. [crates/gcore/src/ai/vision.rs:175-179]
- `sends_image_url_and_parses` (function) component `sends_image_url_and_parses [function]` (`c5125678-2df4-5bbf-b65a-2e9b46a9de54`) lines 190-222 [crates/gcore/src/ai/vision.rs:190-222]
  ...
  - Purpose: Tests that the `describe_image` function correctly encodes images as base64 data URLs in vision API requests and parses the JSON response into structured description, OCR text, and model metadata fields. [crates/gcore/src/ai/vision.rs:190-222]
- `parses_present_empty_json_description` (function) component `parses_present_empty_json_description [function]` (`68e90422-1644-5453-b932-7a013349ed27`) lines 225-234 [crates/gcore/src/ai/vision.rs:225-234]
  ...
  - Purpose: Tests that `parse_content` correctly deserializes JSON with a present but empty description string, null ocr_text, and extracts the model identifier. [crates/gcore/src/ai/vision.rs:225-234]
- `parses_unterminated_json_fence` (function) component `parses_unterminated_json_fence [function]` (`add5d0e2-954d-5f0c-a54c-25917626e112`) lines 237-246 [crates/gcore/src/ai/vision.rs:237-246]
  ...
  - Purpose: Unit test verifying that `parse_content` correctly extracts JSON object fields from an unterminated (unclosed) markdown JSON code fence. [crates/gcore/src/ai/vision.rs:237-246]
- `spawn_server` (function) component `spawn_server [function]` (`3ad0205a-e7d7-51da-9e36-c4c467003126`) lines 248-250 [crates/gcore/src/ai/vision.rs:248-250]
  ...
  - Purpose: Spawns a test JSON response server from the provided static string response, returning a tuple of the server's URL string and RequestHandle, or panicking if instantiation fails. [crates/gcore/src/ai/vision.rs:248-250]
- `request_body_json` (function) component `request_body_json [function]` (`ca792c6f-b010-5711-9d72-fe94dda683f5`) lines 252-255 [crates/gcore/src/ai/vision.rs:252-255]
  ...
  - Purpose: Extracts the HTTP request body following the CRLF-CRLF header delimiter and deserializes it as a JSON `Value`. [crates/gcore/src/ai/vision.rs:252-255]
- `has_header` (function) component `has_header [function]` (`98467993-79b7-59ed-aef8-bd1899fc8bad`) lines 257-264 [crates/gcore/src/ai/vision.rs:257-264]
  ...
  - Purpose: Returns true if the request string contains a header with a case-insensitive ASCII name match and exact trimmed value match. [crates/gcore/src/ai/vision.rs:257-264]
- `test_context` (function) component `test_context [function]` (`567bf261-a3d2-5e8d-a35b-38c1f624a7a8`) lines 266-283 [crates/gcore/src/ai/vision.rs:266-283]
  ...
  - Purpose: Constructs a test AiContext with a single shared binding across all AI service endpoints (embedding, audio transcription/translation, vision extraction, and text generation) configured with a concurrency limit of 1 for sequential execution. [crates/gcore/src/ai/vision.rs:266-283]
- `binding` (function) component `binding [function]` (`fcf5de2c-4dc1-5a01-871a-1991d0fd599b`) lines 285-297 [crates/gcore/src/ai/vision.rs:285-297]
  - Signature: `fn binding(api_base: &str, api_key: Option<&str>) -> CapabilityBinding {`
  - Purpose: Instantiates a `CapabilityBinding` configured for direct routing with the provided API base URL and optional API key, defaulting to the gpt-4.1-mini model. [crates/gcore/src/ai/vision.rs:285-297]
