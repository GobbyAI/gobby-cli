...
- `find_project_root` (function) component `find_project_root [function]` (`773fd042-0667-51b8-acdf-645b26e780e1`) lines 12-24 [crates/gcore/src/project.rs:12-24]
  ...
  - Purpose: Walks upward from 'start' toward the filesystem root and returns the first directory containing a '.gobby/project.json' or '.gobby/gcode.json' file, or 'None' if no such project root exists. [crates/gcore/src/project.rs:12-24]
- `read_project_id` (function) component `read_project_id [function]` (`ca892579-e399-5e47-aef7-91b3d9aab129`) lines 28-51 [crates/gcore/src/project.rs:28-51]
  ...
  - Purpose: Reads the project ID from '.gobby/project.json' if present, falling back to '.gobby/gcode.json' when the primary file is missing or unreadable, and annotates fallback read errors with both paths. [crates/gcore/src/project.rs:28-51]
- `read_project_id_from` (function) component `read_project_id_from [function]` (`f0c8cb4c-fdc0-5645-939f-bc32e6b32c19`) lines 53-62 [crates/gcore/src/project.rs:53-62]
  ...
  - Purpose: Reads the file at 'path' as UTF-8 JSON, parses it, and returns the string value of its '"id"' field or an error if reading, parsing, or locating a string '"id"' fails. [crates/gcore/src/project.rs:53-62]
- `read_project_id_is_non_destructive` (function) component `read_project_id_is_non_destructive [function]` (`299860f6-1f8f-50b3-bda9-35a3313f3900`) lines 70-89 [crates/gcore/src/project.rs:70-89]
  ...
  - Purpose: Reads the project ID from '.gobby/project.json' and verifies the lookup leaves the file contents unchanged. [crates/gcore/src/project.rs:70-89]
- `read_project_id_falls_back_to_gcode_json_root_marker` (function) component `read_project_id_falls_back_to_gcode_json_root_marker [function]` (`c2929b1f-653d-5e4a-8126-5f28cc30ea15`) lines 92-113 [crates/gcore/src/project.rs:92-113]
  ...
  - Purpose: Creates a temporary project layout with a '.gobby/gcode.json' containing an 'id' and verifies that project root detection returns the temp directory and 'read_project_id' falls back to reading the 'id' from that JSON root marker. [crates/gcore/src/project.rs:92-113]
- `missing_project_id_error_mentions_id_key` (function) component `missing_project_id_error_mentions_id_key [function]` (`d0809951-b630-5b2d-a3ee-782cea3cec3e`) lines 116-126 [crates/gcore/src/project.rs:116-126]
  ...
  - Purpose: Creates a temporary '.gobby/project.json' without an 'id' field, calls 'read_project_id', and asserts the resulting error message explicitly says the ''id' field not found'. [crates/gcore/src/project.rs:116-126]
- `read_project_id_falls_back_to_gcode_json_when_project_json_is_bad` (function) component `read_project_id_falls_back_to_gcode_json_when_project_json_is_bad [function]` (`6512e3bc-709d-5ab1-8555-d9f748341576`) lines 129-145 [crates/gcore/src/project.rs:129-145]
  ...
  - Purpose: Verifies that 'read_project_id' ignores an invalid 'project.json' project ID source and falls back to reading '"id"' from '.gobby/gcode.json', returning '"standalone-fallback"'. [crates/gcore/src/project.rs:129-145]
- `read_project_id_falls_back_to_gcode_json_when_project_json_is_malformed` (function) component `read_project_id_falls_back_to_gcode_json_when_project_json_is_malformed [function]` (`c32b60f5-9fe4-562a-9c5b-295b5354f930`) lines 148-164 [crates/gcore/src/project.rs:148-164]
  ...
  - Purpose: Verifies that 'read_project_id' returns the 'id' from '.gobby/gcode.json' when '.gobby/project.json' exists but contains malformed JSON. [crates/gcore/src/project.rs:148-164]
