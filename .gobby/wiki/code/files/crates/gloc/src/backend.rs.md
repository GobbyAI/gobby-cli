...
- `ModelError` (class) component `ModelError [class]` (`959f4302-6ec9-5693-892c-448fab92ce23`) lines 14-23 [crates/gloc/src/backend.rs:14-23]
  ...
  - Purpose: 'ModelError' implements 'fmt::Display' to format each variant as a human-readable error message: missing model, pull failure, warmup failure, or network error. [crates/gloc/src/backend.rs:14-23]
- `ModelError.fmt` (method) component `ModelError.fmt [method]` (`7e263d52-5ed8-5422-a547-87a81d3649ac`) lines 15-22 [crates/gloc/src/backend.rs:15-22]
  ...
  - Purpose: Formats the enum as a human-readable error string by matching each variant and writing a variant-specific message with the embedded model name or error detail. [crates/gloc/src/backend.rs:15-22]
- `ensure_model_ready` (function) component `ensure_model_ready [function]` (`1550bb68-f95d-5cf7-9a78-634164f14e23`) lines 28-62 [crates/gloc/src/backend.rs:28-62]
  ...
  - Purpose: 'ensure_model_ready' is an Ollama-specific readiness gate that checks whether a model is available and loaded, optionally auto-pulls missing models and auto-warms unloaded ones based on settings, and otherwise returns the underlying 'ModelError'. [crates/gloc/src/backend.rs:28-62]
- `ollama_check_model` (function) component `ollama_check_model [function]` (`c3153167-82c9-5ef3-a9f2-0b33df034b8c`) lines 67-108 [crates/gloc/src/backend.rs:67-108]
  ...
  - Purpose: Checks Ollama’s '/api/tags' to verify the named model is downloaded, returns 'NotFound' if absent, and otherwise returns whether '/api/ps' reports it currently loaded in memory, falling back to 'Ok(false)' if the process-list lookup fails. [crates/gloc/src/backend.rs:67-108]
- `ollama_pull_model` (function) component `ollama_pull_model [function]` (`2e3be105-cdbe-547f-90a6-bbe2885de96b`) lines 111-129 [crates/gloc/src/backend.rs:111-129]
  ...
  - Purpose: Sends a non-streaming JSON 'POST' to '{backend.url}/api/pull' with the requested model name using a ureq client configured with 10s connect and 600s read timeouts, and maps any request failure to 'ModelError::PullFailed'. [crates/gloc/src/backend.rs:111-129]
- `ollama_warmup_model` (function) component `ollama_warmup_model [function]` (`cd57587b-8493-53ba-bd7d-73e123d81762`) lines 132-149 [crates/gloc/src/backend.rs:132-149]
  ...
  - Purpose: Sends a JSON 'POST' to '"{backend.url}/api/generate"' with the requested 'model' using a Ureq client configured with 10s connect and 120s read timeouts, returning 'WarmupFailed' on request error and 'Ok(())' otherwise. [crates/gloc/src/backend.rs:132-149]
- `model_name_matches` (function) component `model_name_matches [function]` (`636cea91-7278-5e0b-a985-9e719e252bd3`) lines 153-162 [crates/gloc/src/backend.rs:153-162]
  ...
  - Purpose: Returns 'true' when the JSON entry’s 'name' or 'model' field is a string exactly equal to 'model', equal to 'model:latest', or prefixed by 'model:', and 'false' otherwise. [crates/gloc/src/backend.rs:153-162]
- `unreachable_backend` (function) component `unreachable_backend [function]` (`26bded03-836f-58c7-8409-c46953e9b282`) lines 168-175 [crates/gloc/src/backend.rs:168-175]
  ...
  - Purpose: Returns a 'Backend' fixture with hardcoded dummy values: 'name' set to '"fake"', 'url' set to 'http://127.0.0.1:19999', 'probe' set to '"/"', and an empty 'auth_token'. [crates/gloc/src/backend.rs:168-175]
- `test_detect_backend_none_running` (function) component `test_detect_backend_none_running [function]` (`bc20d012-5207-57d8-87bf-b209fabb7988`) lines 178-181 [crates/gloc/src/backend.rs:178-181]
  ...
  - Purpose: Verifies that 'gobby_core::local_backend::detect_backend' returns 'None' when given only an unreachable backend at port '100'. [crates/gloc/src/backend.rs:178-181]
- `test_validate_backend_unreachable` (function) component `test_validate_backend_unreachable [function]` (`a381ca63-91a0-50da-b4d4-f9274138f5dd`) lines 184-189 [crates/gloc/src/backend.rs:184-189]
  ...
  - Purpose: Verifies that 'gobby_core::local_backend::validate_backend' returns 'false' when given an unreachable backend and a 100ms timeout. [crates/gloc/src/backend.rs:184-189]
- `test_ensure_model_ready_non_ollama_is_noop` (function) component `test_ensure_model_ready_non_ollama_is_noop [function]` (`fb12b714-be3f-5221-a8ad-21e12c2d1c5d`) lines 192-201 [crates/gloc/src/backend.rs:192-201]
  ...
  - Purpose: Verifies that 'ensure_model_ready' returns 'Ok(())' without side effects for a non-Ollama backend ('lmstudio'), even when given an arbitrary model name. [crates/gloc/src/backend.rs:192-201]
- `test_model_name_matches_exact` (function) component `test_model_name_matches_exact [function]` (`fb28a04e-5a8f-559a-afb2-f6d530ca292d`) lines 204-207 [crates/gloc/src/backend.rs:204-207]
  ...
  - Purpose: Verifies that 'model_name_matches' returns 'true' when a JSON entry’s '"name"' field exactly equals the target model name '"qwen3-coder"'. [crates/gloc/src/backend.rs:204-207]
- `test_model_name_matches_with_latest` (function) component `test_model_name_matches_with_latest [function]` (`8eb03b46-e5a9-5a2c-ad9b-f452fbfd72d1`) lines 210-213 [crates/gloc/src/backend.rs:210-213]
  ...
  - Purpose: Verifies that 'model_name_matches' treats the JSON 'name' value '"qwen3-coder:latest"' as matching the base model name '"qwen3-coder"'. [crates/gloc/src/backend.rs:210-213]
- `test_model_name_matches_with_tag` (function) component `test_model_name_matches_with_tag [function]` (`001d13de-e5d1-5ff5-8031-35b5d08aee92`) lines 216-219 [crates/gloc/src/backend.rs:216-219]
  ...
  - Purpose: Verifies that 'model_name_matches' returns true when the JSON entry name is 'glm-4.7:cloud' and the target model name is 'glm-4.7', i.e. it matches a tagged variant by prefix before the colon. [crates/gloc/src/backend.rs:216-219]
- `test_model_name_no_match` (function) component `test_model_name_no_match [function]` (`28f1c2a8-fc17-51ae-b0aa-c942e21f9368`) lines 222-225 [crates/gloc/src/backend.rs:222-225]
  ...
  - Purpose: Verifies that 'model_name_matches' returns 'false' when the JSON entry name is '"llama3"' and the target model name is '"qwen3-coder"'. [crates/gloc/src/backend.rs:222-225]
- `test_model_name_matches_model_field` (function) component `test_model_name_matches_model_field [function]` (`5e9f0915-50ce-5cfd-8e21-a853a3059467`) lines 228-231 [crates/gloc/src/backend.rs:228-231]
  ...
  - Purpose: Verifies that 'model_name_matches' returns true when a JSON entry’s 'model' field is 'qwen3-coder:latest' and the expected model name is 'qwen3-coder'. [crates/gloc/src/backend.rs:228-231]
- `test_model_error_display` (function) component `test_model_error_display [function]` (`0b24632d-b0de-563d-bfad-d9c7a9df0df0`) lines 234-243 [crates/gloc/src/backend.rs:234-243]
  ...
  - Purpose: Verifies that 'ModelError::NotFound("foo")' formats as 'model 'foo' not found' and 'ModelError::PullFailed("timeout")' formats as 'pull failed: timeout'. [crates/gloc/src/backend.rs:234-243]
