...
This module implements the command-line interface (CLI) argument parsing and validation for the gcode tool. It handles command routing and option processing for code search, graph analysis, grep operations, and configuration tasks, while enforcing strict validation for flags, paths, and output formats. A comprehensive test suite verifies parsing logic, flag interactions, help routing, and default format behavior across all supported commands.
