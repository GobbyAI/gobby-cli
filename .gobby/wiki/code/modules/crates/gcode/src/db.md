...
- [[code/files/crates/gcode/src/db/resolution.rs|crates/gcode/src/db/resolution.rs]] - This file implements PostgreSQL database URL resolution for Gobby. It defines small data models for broker responses and bootstrap config, then layers helpers that read the home directory, locate `bootstrap.yaml`, parse bootstrap YAML, normalize empty strings, and validate broker-provided URLs and daemon loopback targets. The main resolution path tries sources in priority order: explicit environment overrides, then daemon/broker discovery via a local CLI token and HTTP request, then bootstrap config, then gcore config, with optional identity and reachability checks to confirm the chosen endpoint. The test-only helpers and fixtures exercise each branch of that resolution chain and its validation errors.
[crates/gcode/src/db/resolution.rs:16-18]
