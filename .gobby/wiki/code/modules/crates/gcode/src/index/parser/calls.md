...
- [[code/files/crates/gcode/src/index/parser/calls/ast.rs|crates/gcode/src/index/parser/calls/ast.rs]] - This file implements AST-based call extraction for indexed source code. `extract_ast_calls` runs a language-specific tree-sitter query, pulls out call/name captures, filters ignored names, resolves qualified callees and member-call qualifiers, and materializes `CallRelation` records with optional semantic resolution. The JavaScript helpers `extract_js_calls` and `js_bindings` provide a JS-specific path for parsing source and import bindings, and the tests verify capture requirements and qualifier handling for member and imported calls.
...
- [[code/files/crates/gcode/src/index/parser/calls/shadowing.rs|crates/gcode/src/index/parser/calls/shadowing.rs]] - This file detects whether an external call is shadowed by a local binding in the caller’s scope before the call site. `external_call_is_shadowed` chooses the name to check based on call syntax, then delegates to `local_name_in_scope_before_call`, which scans the caller’s source prefix for parameters or local bindings after first stripping nested block comments. The helper functions break that scan down into comment removal, parameter-list parsing, assignment/declaration detection, and identifier extraction so the shadowing check can recognize real local bindings while ignoring commented-out code, malformed bindings, and non-simple assignment forms.
