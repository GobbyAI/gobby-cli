...
The `vector` module provides semantic vector indexing and search for code symbols, backed by Qdrant.

Its core `code_symbols` submodule handles the full lifecycle: generating embeddings for code symbols (with configurable embedding backends and AI context resolution), maintaining per-project Qdrant collections, and serving semantic search queries. Key capabilities include:

- **Embedding generation**: `EmbeddingBackend` and related functions embed symbol text and queries, probe vector dimensions, and batch requests, with embedding sources resolved from AI context configuration.
- **Collection lifecycle**: `CodeSymbolVectorLifecycle` manages collection creation, schema compatibility checks, symbol syncing, stale-vector deletion, point upserts, and project/file-scoped clearing.
- **Search**: `search_code_symbols` and `semantic_search` execute vector queries against Qdrant, returning ranked hits with provenance metadata payloads.
- **Symbol fetching and text preparation**: utilities query symbols by file/project predicate and produce masked vector text (stripping Rust comments and literals).

Errors are surfaced via `SearchError` and `VectorLifecycleError`. The module is extensively tested, covering dimension validation, batching, collection naming/isolation, and HTTP-level Qdrant interactions.
...
- [[code/modules/crates/gcode/src/vector/code_symbols|crates/gcode/src/vector/code_symbols]] - The `code_symbols` module provides semantic vector indexing and search for code symbols, backed by Qdrant. It is organized into focused submodules:
