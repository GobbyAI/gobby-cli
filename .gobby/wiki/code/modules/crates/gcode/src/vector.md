...
The vector module provides infrastructure for managing vector embeddings and semantic search for G-code symbols. It orchestrates embedding generation, dimension probing, and integration with external embedding backends. A core lifecycle interface handles Qdrant collection management, including creation, synchronization, cleanup, and schema validation. The module also exposes a repository and search layer for fetching G-code symbols, executing semantic queries, and managing vector payloads, supported by dedicated error types and Qdrant HTTP utilities.
