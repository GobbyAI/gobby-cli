...
The `gwiki/src` module is the implementation root for gwiki, a scope-aware (global/project/topic) wiki and knowledge-base CLI built on a raw-first storage model. It pairs a `main.rs`/`commands` CLI layer with a `contract.rs` flag specification and JSON/text `output` rendering, dispatching subcommands for setup, indexing, ingestion, search, ask, read, refresh, compile, research, review, export, health, trust, librarian, lint, and source management.

Core infrastructure spans persistence and retrieval: a `store` abstraction with in-memory and Postgres BM25 backends, `vector`/`vector` Qdrant semantic search, FalkorDB graph boosting (`falkor_graph`, `graph`), and a hybrid `search` engine that fuses keyword, semantic, and graph sources with degradation tracking. Content flows through the `ingest` pipeline (URLs, files, PDFs, Office docs, audio, video, images, git, MediaWiki, Wayback) with AI-backed transcription, translation, and vision (`ai`, `transcribe`, `vision`, `video`, `media`), then into indexed documents, chunks, links, sources, and provenance.

Higher-level features include note compilation and synthesis into Obsidian-flavored pages (`compile`, `synthesis`), iterative model-driven research loops (`research`, `research_loop`, `session`), citation/credibility auditing and trust reporting (`audit`, `credibility`, `citations`, `provenance`), code-change-triggered page refresh and review (`code_graph`, `review`), link analysis (`links`, `lint`), benchmarking, and daemon capability probing. Shared `support` utilities, scope/registry/schema resolution, frontmatter and markdown parsing, vault layout, error types, and event logging underpin the crate. Degradation-aware, scope-filtered, atomic, and read-only-by-default behavior are recurring design themes throughout.
...
sequenceDiagram
    ...
    participant m_08afca78_922a_529a_a171_cff14243b5e4 as render_audio_transcript_markdown &#91;function&#93;
    participant m_08fc4ede_258b_5357_8655_592eb3fc7c97 as target_map &#91;function&#93;
    ...
    participant m_52619537_64d0_521e_8dfc_758fa37c9f97 as page_targets &#91;function&#93;
    ...
    participant m_d146c789_f60c_50d1_8e50_f47729972a67 as format_timestamp_ms &#91;function&#93;
    participant m_d9d9324e_7d2a_58da_aab3_043f02a67dcb as align_transcript_and_frames &#91;function&#93;
    participant m_e21bb638_9262_5168_ab38_8c6b856bd87e as format_ranges_ms &#91;function&#93;
    ...
    m_08afca78_922a_529a_a171_cff14243b5e4->>m_d146c789_f60c_50d1_8e50_f47729972a67: calls
    m_08afca78_922a_529a_a171_cff14243b5e4->>m_e21bb638_9262_5168_ab38_8c6b856bd87e: calls
    m_08fc4ede_258b_5357_8655_592eb3fc7c97->>m_52619537_64d0_521e_8dfc_758fa37c9f97: calls
```

## Child Modules

- [[code/modules/crates/gwiki/src/ai|crates/gwiki/src/ai]] - The `ai` module provides AI-backed transcription, translation, and vision extraction for gwiki.
...
- [[code/modules/crates/gwiki/src/graph|crates/gwiki/src/graph]] - The `gwiki::graph` module builds and analyzes a wiki knowledge graph linking documents, sources, citations, and code endpoints.
...
- [[code/modules/crates/gwiki/src/search|crates/gwiki/src/search]] - The `gwiki::search` module implements hybrid wiki search that fuses three retrieval backends and reconciles their results into a unified, scope-aware response.
