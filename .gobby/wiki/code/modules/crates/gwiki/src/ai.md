...
The `ai` module provides AI-backed transcription, translation, and vision extraction for gwiki.

`chunk.rs` handles audio chunking for transcription: the `AudioChunker` trait and `MediaAudioChunker` implementation split media into `AudioChunk`s, with logic to decide when chunking is required (`requires_chunking`, `fixed_codec_bytes_for_duration`), transcribe chunks with overlap deduplication and metadata merging, and assemble `ChunkedTranscription` results.

`clients.rs` defines the production clients (`ProductionTranscriptionClient`, `ProductionVisionClient`) that route requests to AI backends, perform transcription, English/target-language translation (batched and indexed via `IndexedTranslation`), and image extraction. It includes route resolution, error mapping (`ai_error_to_wiki_error`), and conversion from core output types.

`translate.rs` orchestrates audio and segment-level translation, with language normalization helpers (`is_english`, `same_lang`, `source_language`), English-marking, and batch mismatch handling.

Both production paths are covered by extensive test fakes (`FakeChunker`, `ScriptedClient`, `TranslatingClient`, `FakeTranslationClient`) and unit tests exercising chunking thresholds, deduplication, translation precedence, and batch retry behavior.
