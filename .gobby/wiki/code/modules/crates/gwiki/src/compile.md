...
The compile module orchestrates the end-to-end transformation of accepted wiki sources into rendered target pages. It manages source collection and note parsing, path and scope validation, wiki index updates, and final markdown rendering. Core types handle compile configurations, bundled content, and safe file operations with locking and race-condition guards. The module ensures secure target resolution, structural formatting, and Obsidian-compatible output, with comprehensive tests covering edge cases, index consistency, and security constraints.
