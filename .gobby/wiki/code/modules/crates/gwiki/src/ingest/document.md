...
- [[code/files/crates/gwiki/src/ingest/document/render.rs|crates/gwiki/src/ingest/document/render.rs]] - This file renders and persists derived Markdown for document ingests, while preserving the original asset path and failure context in the generated metadata. It provides one path for “raw” document snapshots and another for richer derived documents with scope, extraction, and degradation details, both built as Markdown strings with front matter plus a title and body note.
...
- [[code/files/crates/gwiki/src/ingest/document/tests.rs|crates/gwiki/src/ingest/document/tests.rs]] - Test module for document ingestion in `gwiki`, with helpers that build minimal ZIP-based Office files and a small ingest harness against `MemoryWikiStore`. The shared `zip_bytes` helper assembles archive bytes, while the sample/oversized DOCX, PPTX, and XLSX constructors provide fixture inputs for both normal extraction and limit-triggering cases. The tests then verify that Office and HTML sources are converted to markdown and indexed correctly, and that malformed or oversized inputs degrade gracefully with consistent metadata, bounded extraction behavior, and a few HTML/markdown edge cases.
[crates/gwiki/src/ingest/document/tests.rs:9-18]
