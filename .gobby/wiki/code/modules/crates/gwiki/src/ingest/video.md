...
This module implements the video ingestion pipeline for the gwiki system, orchestrating the extraction of metadata, frame sampling, and audio processing from video files. It manages the complete lifecycle of video assets, including temporary frame persistence, automatic cleanup, and graceful handling of degradation scenarios or unavailable FFmpeg dependencies. The pipeline is structured across dedicated submodules for processing workflows, metadata snapshot management, and asset lifecycle control, supported by extensive test coverage that utilizes mock transcription, vision, and media extraction clients to validate ingestion logic, degradation handling, and asset preservation guarantees.
