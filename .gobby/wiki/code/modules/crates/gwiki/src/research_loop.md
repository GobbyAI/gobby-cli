...
This module implements an iterative, AI-driven research engine that orchestrates knowledge acquisition, validation, and documentation. It centers on the `ResearchLoop` and `LoopState` structs, which manage a cycle of observation, model interaction, and action execution. The architecture uses dependency injection to pluggably integrate AI models, search, ask, read, ingest, and note-writing services, supported by a comprehensive suite of mock implementations for testing. Core logic handles model prompt rendering, JSON action parsing, source validation, citation matching, and state tracking for degradations, scaffolds, and write conflicts. Configuration, input types, observation/result structures, and error handling are defined in `types.rs`, while `helpers.rs` provides path resolution, source normalization, and action fingerprinting utilities.
