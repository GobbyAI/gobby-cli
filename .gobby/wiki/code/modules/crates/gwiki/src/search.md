...
- [[code/files/crates/gwiki/src/search/bm25.rs|crates/gwiki/src/search/bm25.rs]] - This file defines the BM25 search request and SQL payload types, plus the backend trait and concrete backends used to execute ranked full-text search over wiki documents and chunks. It builds parameterized BM25 SQL with optional scope filtering, sanitizes and validates search input, then runs the query through a PostgreSQL backend or serves cached in-memory hits, trimming and filtering results to keyword-searchable paths and BM25-sourced entries.
...
- [[code/files/crates/gwiki/src/search/rrf.rs|crates/gwiki/src/search/rrf.rs]] - Implements reciprocal-rank fusion for wiki search results by collecting BM25, semantic, and graph hits, deduplicating them by canonical fusion key, merging missing metadata across duplicate hits, and then assigning fused scores plus source and explanation provenance into a `WikiSearchResponse`, preserving any requested degradations and returning no results when `limit == 0`. The helper `ranked_keys` extracts fusion keys from ranked hit lists, `merge_hit_metadata` backfills incomplete result fields during deduplication, and the tests verify duplicate coalescing, canonical page-key behavior, and invalid-path rejection.
