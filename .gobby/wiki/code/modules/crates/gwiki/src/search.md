...
This module implements a multi-modal search system for the wiki, integrating keyword matching, graph-based boosting, and semantic vector retrieval. It defines abstract backend traits for BM25, graph boosting, and semantic search, with concrete implementations supporting in-memory, PostgreSQL, Falkor, and Qdrant storage. The module orchestrates search workflows by parsing requests, applying scope and source filters, executing parallel backend queries, and fusing results via Reciprocal Rank Fusion (RRF). It also manages query embedding generation, handles backend degradation and unavailability gracefully, and standardizes search responses with provenance, scoring, and error metadata.
