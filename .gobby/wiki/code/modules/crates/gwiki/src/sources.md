...
This module provides the core infrastructure for managing wiki source files, including manifest tracking, atomic file updates, rendering utilities, and type definitions. It exposes `SourceManifest` for reading, writing, locking, and indexing source metadata, alongside atomic file I/O operations for crash-safe writes. The render utilities handle location canonicalization, markdown escaping, and text normalization, while the types module defines essential enums and structs such as `SourceDraft`, `SourceRecord`, and `SourceReplay` that govern ingestion methods, compilation status, and replay configurations. Together, these components enable reliable source management, version tracking, and content processing within the wiki system.
