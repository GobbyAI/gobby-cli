...
- [[code/files/crates/gwiki/src/support/text.rs|crates/gwiki/src/support/text.rs]] - Utilities for normalizing and classifying text in `gwiki`: it tokenizes search queries, scores text against normalized keywords, sanitizes user-supplied code paths into safe repo-relative strings, extracts short display snippets, maps degradation/document/database kinds to canonical labels, and builds display paths and slugs. The helper functions are paired with tests that confirm snippet truncation, path sanitization, and slug fallback/max-length behavior.
