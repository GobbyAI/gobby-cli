...
- **Tree-sitter grammars** (gcode): 21 languages total — Tier 1 (Python/JS/TS/Go/Rust/Java/C/C++/C#/Ruby/PHP/Swift/Kotlin/Scala/Objective-C), Tier 2 (Dart/Elixir/Lua/Bash), Tier 3 (JSON/YAML). Markdown is indexed as content-only repo text, outside tree-sitter AST detection. Adding an AST language requires a new `tree-sitter-*` dep in `crates/gcode/Cargo.toml` and a grammar entry in `index/languages`.
