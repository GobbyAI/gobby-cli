...
impl Config {
    ...
    /// Load config: CLI override → `.gobby/gsqz.yaml` (current directory or
    /// project root) → `<gobby_home>/gsqz.yaml` → compiled-in default.
    ...
    pub fn load(config_override: Option<&Path>) -> Self {
        match gobby_core::layered_config::load_layered_yaml::<Self>("gsqz", config_override) {
