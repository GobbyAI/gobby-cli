...
fn invalid_diagrams(pages: &[WikiPage]) -> Vec<DiagramIssue> {
    let mut issues = Vec::new();
    for page in pages {
...
/// `architecture_diagrams::is_valid_mermaid`: a balanced `` ```mermaid `` fence, a
/// recognised header ([`VALID_DIAGRAM_HEADERS`]), at least one content line, no
/// stray interior fences, and balanced bracket/paren/brace delimiters.
fn is_valid_mermaid(block: &str) -> bool {
    ...
    if !VALID_DIAGRAM_HEADERS.iter().any(|h| header.starts_with(h)) {
        return false;
    }
...
/// Extract the quoted node labels (`id["label"]`) from a Mermaid block. Edge
/// lines (`a --> b`) carry no quoted label, and `subgraph … ["Title"]` lines
/// carry a descriptive group heading rather than a subsystem node, so both
/// contribute nothing to grounding.
fn node_labels(block: &str) -> Vec<String> {
    ...
    for line in block.lines() {
        ...
        while let Some(open) = rest.find("[\"") {
            ...
            labels.push(after[..close].to_string());
            rest = &after[close + 2..];
        }
    }
    labels
...
"#,
    ...
    #[test]
    fn validator_requires_recognised_header() {
        let bad = "```mermaid\npie title Pets\n    \"Dogs\" : 1\n```";
    ...
    fn node_labels_extracts_quoted_labels_only() {
        let block = "```mermaid\nflowchart LR\n    s0[\"parser — builds AST\"]\n    s0 --> s1\n```";
        assert_eq!(node_labels(block), vec!["parser — builds AST".to_string()]);
    }
