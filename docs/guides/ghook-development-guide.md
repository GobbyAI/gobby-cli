...
```text
host AI CLI (Claude Code / Codex / Gemini / Qwen / Droid / Grok)
...
| `cli_config.rs` | Per-CLI registry (claude/codex/gemini/qwen/droid/grok) — which hooks are critical. Compile-time frozen. `grok` uses native snake_case hook names (`session_start`, `session_end`, `pre_compact`, `stop`). |
...
pub struct Envelope {
    ...
    pub source: String,                 // "claude" / "codex" / "gemini" / "qwen" / "droid" / "grok" / passthrough
