...
Includes docker setups, CLI argument parsing, configuration loading, and environment overrides.

- [[code/concepts/foundational-bedrock|Foundational Bedrock]] - Houses shared configurations, CLI contracts, container orchestrations, and community detection graph analytics.
- [[code/concepts/cli-interface-routing|CLI Interface & Routing]] - Defines the command-line flags, options parsing, routing mechanisms, and CLI contract declarations for the gcode utility.
- [[code/concepts/configuration-database-core|Configuration & Database Core]] - Handles workspace root and identity detection, config resolution with environment overrides, and PostgreSQL relational connection pooling.
- [[code/concepts/postgresql-database-schema-setup|PostgreSQL Database Schema Setup]] - Orchestrates direct SQL and DDL provisioning, extensions like pg_search, and schema validation across the PostgreSQL code index.

### Code Fact Extraction & Syntax Parsing

Focuses on exploring repository files, sanitizing paths, and parsing programming-language specific import statements and call bindings.

- [[code/concepts/filesystem-walker-ingestion|Filesystem Walker & Ingestion]] - Responsible for scanning the local filesystem safely, filtering out ignored and machine-generated files, and managing freshness locks.
- [[code/concepts/import-resolution-syntax-parsing|Import Resolution & Syntax Parsing]] - Implements language-agnostic and language-specific parsers to resolve external imports, qualified namespaces, and function call definitions.

### Multi-Modal Searching & Downstream Projections

Coordinates call graphs, vector databases, search rankings, and FalkorDB dependency syncing.

- [[code/concepts/falkordb-graph-synchronization|FalkorDB Graph Synchronization]] - Maintains relational projections representing call graph and dependency trees downstream in a FalkorDB instance.
- [[code/concepts/query-dispatch-search-orchestration|Query Dispatch & Search Orchestration]] - Handles multi-modal search, combining full-text search, semantic vector retrieval, and code graph ranking using reciprocal rank fusion.

### AI-Backed CodeWiki Platform

Manages generating, rendering, and auditing markdown documentation guides compiled via LLM models.

- [[code/concepts/codewiki-ai-documentation-engine|CodeWiki AI Documentation Engine]] - Generates high-quality Markdown codebase overview guides, subsystem diagrams, onboarding steps, and owner records powered by LLMs.

## Narrative Tours

- [[code/narrative/introduction|Introduction]]
- [[code/narrative/architecture|Architecture]]
- [[code/narrative/data-flow|Data Flow]]
