...
| `sync_file` | function | Synchronizes a file’s code-index graph by invoking 'sync_file_graph', then either returns a success payload/report for synced relationships and symbols or emits a skipped payload/message when the file has no graph facts or is missing from the indexed set, formatted as JSON or text based on 'format'. [crates/gcode/src/commands/graph/lifecycle.rs:444-509] |
