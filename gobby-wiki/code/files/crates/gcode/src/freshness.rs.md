...
| `no_freshness_env_short_circuits_project_refresh` | function | Verifies that when 'INFLIGHT_ENV' is set, 'ensure_fresh(&ctx, FreshnessScope::Project)' short-circuits and returns 'FreshnessStatus::Checked' without performing a project refresh. [crates/gcode/src/freshness.rs:312-320] |
...
| `symbol_slice_check_uses_stored_byte_range_hash` | function | Verifies that 'symbol_slice_is_current' validates a symbol against its stored byte range and content hash, returning true for the original slice and false after the file is shifted so the same symbol no longer occupies that byte range. [crates/gcode/src/freshness.rs:386-422] |
