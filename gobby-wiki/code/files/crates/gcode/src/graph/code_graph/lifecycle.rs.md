...
At its core, the file defines the `GraphLifecycleAction` enum [crates/gcode/src/graph/code_graph/lifecycle.rs:18-21], which represents the fundamental lifecycle actions: clearing and rebuilding. This enum associates each action with its corresponding CLI command, daemon API route, and success messages [crates/gcode/src/graph/code_graph/lifecycle.rs:24-29, crates/gcode/src/graph/code_graph/lifecycle.rs:31-36, crates/gcode/src/graph/code_graph/lifecycle.rs:38-43].
