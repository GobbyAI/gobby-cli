...
`crates/gcode/src/index/indexer/overlay.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `porcelain_status_byte_validation_matches_git_v1_codes` | function | Verifies that 'valid_porcelain_status_byte' accepts the Git v1 porcelain status bytes '[' ', 'M', 'A', 'D', 'R', 'C', 'U', '?', '!']' and rejects invalid bytes '[0, 'X', '\n']'. [crates/gcode/src/index/indexer/overlay.rs:460-467] |
| `git_status_timeout_reads_positive_env_seconds` | function | Verifies that 'git_status_timeout()' returns 'Duration::from_secs(2)' when the 'GCODE_GIT_STATUS_TIMEOUT_SECS' environment variable is set to the positive value '"2"'. [crates/gcode/src/index/indexer/overlay.rs:471-475] |
| `git_status_timeout_rejects_invalid_env_values` | function | Verifies that 'git_status_timeout()' falls back to 'DEFAULT_GIT_STATUS_TIMEOUT_SECS' when 'GCODE_GIT_STATUS_TIMEOUT_SECS' is set to invalid values ('"0"', '"-1"', or '"abc"'). [crates/gcode/src/index/indexer/overlay.rs:479-488] |
