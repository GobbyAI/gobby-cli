...
`crates/gcode/src/projection/sync.rs` is documented from its indexed symbols; see the Reference table below and the module page for how it connects to sibling files.
...
_5 more symbol(s) not shown — run `gcode outline crates/gcode/src/projection/sync.rs` for the full list._
