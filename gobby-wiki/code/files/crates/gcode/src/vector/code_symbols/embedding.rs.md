...
`crates/gcode/src/vector/code_symbols/embedding.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `TestSource::resolve_value` | method | Returns a 'String' by mapping the special input '"$secret:EMBEDDING_KEY"' to the hardcoded value '"resolved-embedding-key"' and otherwise cloning the input string unchanged. [crates/gcode/src/vector/code_symbols/embedding.rs:348-353] |
| `resolves_via_shared_routing` | function | Verifies that embedding config resolution uses the direct 'EMBEDDINGS_API_BASE' when routing is 'auto' with an OpenAI-compatible HTTP transport, and returns 'None' when routing is 'daemon' or 'off'. [crates/gcode/src/vector/code_symbols/embedding.rs:357-391] |
| `reads_endpoint_from_shared_binding` | function | Verifies that 'resolve_embedding_config_from_source(None, ...)' correctly reads an embeddings endpoint configuration from shared binding values, including API base, model, resolved secret API key, query prefix, and timeout. [crates/gcode/src/vector/code_symbols/embedding.rs:394-416] |
| `direct_source_uses_resolved_embedding_config` | function | Verifies that resolving an 'AiContext' with direct embedding settings and a supplied 'EmbeddingConfig' yields 'Some(EmbeddingSource::Direct)' containing the exact resolved config. [crates/gcode/src/vector/code_symbols/embedding.rs:419-442] |
