...
`crates/gcode/src/vector/code_symbols/qdrant.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `vector_search_degradation_warning_mentions_missing_qdrant_config` | function | Verifies that 'vector_search_degradation_warning' returns 'Some("semantic vector search skipped: Qdrant is not configured")' when given 'ServiceState::NotConfigured'. [crates/gcode/src/vector/code_symbols/qdrant.rs:434-439] |
| `vector_search_degradation_warning_mentions_unreachable_qdrant` | function | Verifies that 'vector_search_degradation_warning' returns 'Some("semantic vector search skipped: Qdrant is unreachable: connection refused")' when given 'ServiceState::Unreachable { message: "connection refused" }'. [crates/gcode/src/vector/code_symbols/qdrant.rs:442-452] |
