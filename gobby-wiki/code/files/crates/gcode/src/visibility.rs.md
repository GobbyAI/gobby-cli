...
`crates/gcode/src/visibility.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `tombstone_count` | function | Returns the number of tombstone records in 'code_indexed_files' for the current overlay project by counting rows where 'project_id' matches 'ctx.index_scope.overlay_project_id' and 'language' equals 'TOMBSTONE_LANGUAGE', falling back to '0' if the scope is not an overlay or the query/conversion fails. [crates/gcode/src/visibility.rs:541-558] |
| `visible_project_ids_include_overlay_before_parent` | function | Verifies that 'visible_project_ids(&ctx)' returns the overlay project ID before its parent project ID when the index scope is an overlay rooted at the worktree. [crates/gcode/src/visibility.rs:566-587] |
| `symbols_for_file_sql_qualifies_joined_symbol_columns` | function | Verifies that 'symbols_for_files_sql()' generates a query with joined columns qualified by the 'cs' alias in the 'SELECT' list and 'FROM/JOIN' clauses, including 'cs.file_path = ANY($2)', and does not use an unqualified 'SELECT id, project_id, file_path'. [crates/gcode/src/visibility.rs:590-598] |
| `overlay_symbols_for_files_sql_batches_paths_and_preserves_overlay_shadowing` | function | Verifies that 'overlay_symbols_for_files_sql()' generates SQL selecting 'cs.id', 'cs.project_id', and 'cs.file_path', filters file paths with 'ANY($3)', matches 'project_id' in both '$1' and '$2' contexts, includes 'NOT EXISTS' overlay-shadowing logic via 'shadow.project_id = $1 AND shadow.file_path = cs.file_path', and does not mistakenly bind 'cs.file_path' to '$3'. [crates/gcode/src/visibility.rs:601-611] |
