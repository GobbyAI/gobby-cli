...
`crates/gcore/src/provisioning/bootstrap.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `flatten_yaml_errors_include_root_path` | function | Verifies that 'flatten("true")' returns an error for a root scalar and that the formatted error message includes the ''<root>'' path marker. [crates/gcore/src/provisioning/bootstrap.rs:243-247] |
| `flatten_yaml_errors_include_mapping_path_for_non_string_keys` | function | Verifies that flattening YAML with a non-string mapping key fails and that the resulting error message includes the enclosing mapping path (''ai''). [crates/gcore/src/provisioning/bootstrap.rs:250-254] |
| `flatten_yaml_errors_include_scalar_path` | function | This test asserts that flattening YAML with a scalar sequence value fails and that the resulting error message includes the full dotted path ''ai.embeddings.provider''. [crates/gcore/src/provisioning/bootstrap.rs:257-262] |
| `flatten_yaml_depth_errors_include_current_path` | function | Verifies that 'flatten' rejects YAML nesting deeper than 'MAX_YAML_FLATTEN_DEPTH' and that the resulting error message includes the current key path prefix 'k0.k1'. [crates/gcore/src/provisioning/bootstrap.rs:265-275] |
