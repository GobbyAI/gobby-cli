...
`crates/gwiki/src/ai/clients.rs` is documented from its indexed symbols; see the Reference table below and the module page for how it connects to sibling files.
...
| `binding` | function | The 'binding' function constructs and returns a 'CapabilityBinding' struct initialized with the provided 'AiRouting', an optional 'api_base' mapped to an owned 'String', hardcoded test string values for 'model' and 'provider', and all remaining fields set to 'None'. [crates/gwiki/src/ai/clients.rs:471-489] |
