...
`crates/gwiki/src/api.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `ScopeIdentity::fmt` | method | Formats the value by writing its 'kind' as a string, followed by a colon, followed by its 'id', into the provided formatter. [crates/gwiki/src/api.rs:333-335] |
| `CommandOutcome` | class | 'CommandOutcome' is a struct that bundles a list of status messages, a 'CommandResult', and an 8-bit exit code for a command execution. [crates/gwiki/src/api.rs:339-343] |
| `CommandResult` | class | 'CommandResult' is a Rust struct that encapsulates a command’s output as both structured JSON data in 'payload: serde_json::Value' and a raw textual representation in 'text: String'. [crates/gwiki/src/api.rs:346-349] |
| `scope_selection_constructors_express_allowed_states` | function | Verifies that 'ScopeSelection::detect()', 'ScopeSelection::project("/repo")', and 'ScopeSelection::topic("ops")' construct the expected allowed states and identities, including default/global, project-root, and topic-name behavior. [crates/gwiki/src/api.rs:358-374] |
| `target_lang_requires_translate_flag` | function | Verifies that 'IngestFileOptions::target_lang' is ignored unless 'translate' is 'true', in which case 'apply_to_ai_context' sets 'context.bindings.audio_translate.target_lang' to the provided language code. [crates/gwiki/src/api.rs:377-398] |
| `transcription_routing_applies_to_active_audio_capability` | function | Verifies that 'IngestFileOptions::transcription_routing' is applied only to the active audio capability selected by 'translate', routing 'audio_transcribe' when 'translate' is false and 'audio_translate' when 'translate' is true, without modifying the inactive binding. [crates/gwiki/src/api.rs:401-431] |
| `crate_has_no_gcode_dependency` | function | Reads 'Cargo.toml', parses its 'dependencies' table, and asserts that 'gobby-core' is present while 'gobby-code' is absent. [crates/gwiki/src/api.rs:434-451] |
