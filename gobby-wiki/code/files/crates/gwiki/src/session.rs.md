...
`crates/gwiki/src/session.rs` is documented from its indexed symbols; see the Key components below and the module page for how it connects to sibling files.
...
| `ResearchSession::record_compile_state` | method | Stores the provided 'CompileState' in 'self.compile_state' and then persists the updated checkpoint by calling 'self.save_checkpoint()', returning any resulting 'WikiError'. [crates/gwiki/src/session.rs:311-314] |
| `validate_checkpoint_scope_root` | function | Compares the normalized expected scope root against the normalized loaded root derived from the checkpoint vault base, returns the expected path when they match, and otherwise raises 'WikiError::InvalidScope' stating that the checkpoint belongs to a different scope root. [crates/gwiki/src/session.rs:317-336] |
| `comparable_path` | function | Returns an absolute, canonicalized 'PathBuf' for 'path', first resolving a relative 'path' against 'relative_base' when provided, and falling back to the original path if canonicalization fails. [crates/gwiki/src/session.rs:338-347] |
| `checkpoint_vault_root` | function | Returns the grandparent directory of 'checkpoint_path' as a 'PathBuf', or 'None' if the path has fewer than two parent components. [crates/gwiki/src/session.rs:349-354] |
| `new_session_id` | function | Generates a new session ID string in the form 'research-<unix_timestamp_ms>-<first_8_hex_chars_of_uuidv4>', returning it as 'Result<String, WikiError>'. [crates/gwiki/src/session.rs:356-363] |
| `unix_timestamp_ms` | function | Returns the current Unix timestamp in milliseconds by delegating to 'time::unix_timestamp_ms()', propagating its 'Result<u64, WikiError>'. [crates/gwiki/src/session.rs:365-367] |
| `research_prompt` | function | Builds a research prompt string containing the question, worker count, and a fixed instruction to return source-grounded notes for 'raw/research/', optionally appending a bulleted 'Source constraints' section from the provided constraint list. [crates/gwiki/src/session.rs:369-385] |
| `compile_state_is_resumable` | function | Creates a research session, records a 'CompileState' checkpoint, reloads it from disk, and verifies the compile state is persisted with the expected fields and 'write_intent' flag. [crates/gwiki/src/session.rs:392-428] |
| `research_code_citation_rejects_empty_provenance` | function | Verifies that 'ResearchCodeCitation::new' returns 'WikiError::InvalidInput' with 'field: "code_citations"' when given an empty provenance vector. [crates/gwiki/src/session.rs:431-442] |
| `research_code_citation_rejects_parent_path_components` | function | Verifies that 'ResearchCodeCitation::new' rejects a citation path containing parent-directory components like '..' by returning 'WikiError::InvalidInput' for the 'code_citations' field. [crates/gwiki/src/session.rs:445-457] |
| `research_code_citation_deserialization_rejects_empty_provenance` | function | Verifies that deserializing a 'ResearchCodeCitation' with an empty 'provenance' array fails and that the resulting error message mentions 'provenance'. [crates/gwiki/src/session.rs:460-468] |
| `research_code_citation_deserialization_rejects_parent_path_components` | function | Verifies that deserializing a 'ResearchCodeCitation' with a 'file' path containing parent directory components ('".."') fails, and that the resulting error message includes '".."'. [crates/gwiki/src/session.rs:471-479] |
| `load_checkpoint_rejects_legacy_project_vault_relative_scope_root` | function | Verifies that 'ResearchSession::load_checkpoint' rejects a legacy checkpoint whose project scope root is stored as the vault-relative path 'gobby-wiki', returning 'WikiError::InvalidScope' with a detail string mentioning both 'gobby-wiki' and the canonical expected root. [crates/gwiki/src/session.rs:482-507] |
| `load_checkpoint_rejects_mismatched_scope_root` | function | Creates a session checkpoint whose serialized 'ResearchScope' points at a different root directory than the checkpoint location, then verifies 'ResearchSession::load_checkpoint' rejects it with 'WikiError::InvalidScope'. [crates/gwiki/src/session.rs:510-531] |
| `load_checkpoint_normalizes_relative_scope_root_against_checkpoint_vault` | function | Verifies that 'ResearchSession::load_checkpoint' resolves a relative 'scope.root' in the saved checkpoint to the checkpoint vault directory by loading a session whose project root is '"."' and asserting the loaded root equals the canonicalized checkpoint path. [crates/gwiki/src/session.rs:534-556] |
